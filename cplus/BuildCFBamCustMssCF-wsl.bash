...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f libcfbamcustmsscf*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
...
echo ":wq" >> ex.cmd
ex libcfbamcustmsscf/CFBamCustMssCFEngine.cpp < ex.cmd
ex libcfbamcustmsscf/configure.ac < ex.cmd
ex libcfbamcustmsscf/Makefile.am < ex.cmd
ex libcfbamcustmsscf/ChangeLog < ex.cmd
rm ex.cmd
pushd libcfbamcustmsscf
	...
	if [ ! -d /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamcustmsscf ]; then
		sudo mkdir /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamcustmsscf
	fi
	sudo chmod 755 /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamcustmsscf
	autoreconf -fi
	aclocal
	autoconf
	automake
