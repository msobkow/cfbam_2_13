...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f libcfbammsscf*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
pushd libcfbammsscf
	if [ -f Makefile ]; then
		make distclean
	fi
...
pushd libcfbammsscf
	...
	if [ ! -d /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbammsscf ]; then
		sudo mkdir /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbammsscf
	fi
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbammsscf
	autoreconf -fi
	aclocal
	autoconf
	automake
	...
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbammsscf
	sudo sh -c "cd /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbammsscf && chmod 644 *.hpp"
	#sudo chmod 755 /usr/bin/*cfbam*msscf
	sudo chmod 755 /usr/lib/*cfbammsscf.la
	sudo chmod 755 /usr/lib/*cfbammsscf.so*
