...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeRamTableImplementationHandler" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeRamTableImplementationHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeRamTableImplementationHandler::CFBamCustXmlLoaderCafeRamTableImplementationHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeRamTableImplementationHandler::~CFBamCustXmlLoaderCafeRamTableImplementationHandler() {
	}

	void CFBamCustXmlLoaderCafeRamTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeRamTableImplementation" ) {
				static const std::string Msg( "Expected QName to be 'CafeRamTableImplementation'" );
	...
	void CFBamCustXmlLoaderCafeRamTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJRamTableImplementationNull() || tableObj->getOptionalJRamTableImplementationValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJRamTableImplementationValue( text );
