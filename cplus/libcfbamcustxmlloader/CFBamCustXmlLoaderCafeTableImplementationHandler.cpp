...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeTableImplementationHandler" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeTableImplementationHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeTableImplementationHandler::CFBamCustXmlLoaderCafeTableImplementationHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeTableImplementationHandler::~CFBamCustXmlLoaderCafeTableImplementationHandler() {
	}

	void CFBamCustXmlLoaderCafeTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeTableImplementation" ) {
				static const std::string Msg( "Expected QName to be 'CafeTableImplementation'" );
	...
	void CFBamCustXmlLoaderCafeTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJTableImplementationNull() || tableObj->getOptionalJTableImplementationValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJTableImplementationValue( text );
