...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::~CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler() {
	}

	void CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CPlusXMsgTableInclude" ) {
				static const std::string Msg( "Expected QName to be 'CPlusXMsgTableInclude'" );
	...
	void CFBamCustXmlLoaderCPlusXMsgTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalCppXMsgTableIncludeNull() || tableObj->getOptionalCppXMsgTableIncludeValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalCppXMsgTableIncludeValue( text );
