...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderTableColHandler::CLASS_NAME( "CFBamCustXmlLoaderTableColHandler" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderTableColHandler::S_1( "1" );

	CFBamCustXmlLoaderTableColHandler::CFBamCustXmlLoaderTableColHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderTableColHandler::~CFBamCustXmlLoaderTableColHandler() {
	}

	void CFBamCustXmlLoaderTableColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "TableCol" ) {
				static const std::string Msg( "Expected QName to be 'TableCol'" );
			...
			if( natXmlElementName != NULL ) {
				editBuff->setOptionalXmlElementNameValue( *natDbName );
			}
			else {
			...
			if( ( attrDataType != NULL ) && ( attrDataType->length() > 0 ) ) {
				...
				if( refDataType == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
						refTable->getRequiredContainerSchemaDef()->getRequiredName() + S_Dot + refTable->getRequiredName() + S_Dot + *attrName + " could not resolve DataType reference qualified name \"" + *attrDataType + "\" to table Value" );
				}
			}
			else {
				refDataType = NULL;
			...
			origTableCol = dynamic_cast<cfbam::ICFBamTableColObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderTableColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
