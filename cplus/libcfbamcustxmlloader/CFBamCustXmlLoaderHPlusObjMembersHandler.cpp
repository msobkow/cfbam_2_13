...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusObjMembersHandler" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusObjMembersHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusObjMembersHandler::CFBamCustXmlLoaderHPlusObjMembersHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusObjMembersHandler::~CFBamCustXmlLoaderHPlusObjMembersHandler() {
	}

	void CFBamCustXmlLoaderHPlusObjMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusObjMembers" ) {
				static const std::string Msg( "Expected QName to be 'HPlusObjMembers'" );
	...
	void CFBamCustXmlLoaderHPlusObjMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppObjMembersNull() || tableObj->getOptionalHppObjMembersValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppObjMembersValue( text );
