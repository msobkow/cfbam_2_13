...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeRamTableImportHandler" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeRamTableImportHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeRamTableImportHandler::CFBamCustXmlLoaderCafeRamTableImportHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeRamTableImportHandler::~CFBamCustXmlLoaderCafeRamTableImportHandler() {
	}

	void CFBamCustXmlLoaderCafeRamTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeRamTableImport" ) {
				static const std::string Msg( "Expected QName to be 'CafeRamTableImport'" );
	...
	void CFBamCustXmlLoaderCafeRamTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJRamTableImportNull() || tableObj->getOptionalJRamTableImportValue().empty() ) {
				std::string text = curContext->getElementText();
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJRamTableImportValue( text );
