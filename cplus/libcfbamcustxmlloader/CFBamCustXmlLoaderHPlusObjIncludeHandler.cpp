...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusObjIncludeHandler" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusObjIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusObjIncludeHandler::CFBamCustXmlLoaderHPlusObjIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusObjIncludeHandler::~CFBamCustXmlLoaderHPlusObjIncludeHandler() {
	}

	void CFBamCustXmlLoaderHPlusObjIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusObjInclude" ) {
				static const std::string Msg( "Expected QName to be 'HPlusObjInclude'" );
	...
	void CFBamCustXmlLoaderHPlusObjIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppObjIncludeNull() || tableObj->getOptionalHppObjIncludeValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppObjIncludeValue( text );
