...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderTextTypeHandler::CLASS_NAME( "CFBamCustXmlLoaderTextTypeHandler" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderTextTypeHandler::S_1( "1" );

	CFBamCustXmlLoaderTextTypeHandler::CFBamCustXmlLoaderTextTypeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderTextTypeHandler::~CFBamCustXmlLoaderTextTypeHandler() {
	}

	void CFBamCustXmlLoaderTextTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "TextType" ) {
				static const std::string Msg( "Expected QName to be 'TextType'" );
			...
			int32_t* pMaxLen = cflib::CFLibXmlUtil::parseInt32( *attrMaxLen );
			if( pMaxLen != NULL ) {
				natMaxLen = *pMaxLen;
				delete pMaxLen;
				pMaxLen = NULL;
			...
			origTextType = dynamic_cast<cfbam::ICFBamTextTypeObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderTextTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
