...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderBlobColHandler::CLASS_NAME( "CFBamCustXmlLoaderBlobColHandler" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderBlobColHandler::S_1( "1" );

	CFBamCustXmlLoaderBlobColHandler::CFBamCustXmlLoaderBlobColHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderBlobColHandler::~CFBamCustXmlLoaderBlobColHandler() {
	}

	void CFBamCustXmlLoaderBlobColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "BlobCol" ) {
				static const std::string Msg( "Expected QName to be 'BlobCol'" );
			...
			origBlobCol = dynamic_cast<cfbam::ICFBamBlobColObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderBlobColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
