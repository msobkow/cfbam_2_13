...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderUuidGenHandler::CLASS_NAME( "CFBamCustXmlLoaderUuidGenHandler" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderUuidGenHandler::S_1( "1" );

	CFBamCustXmlLoaderUuidGenHandler::CFBamCustXmlLoaderUuidGenHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderUuidGenHandler::~CFBamCustXmlLoaderUuidGenHandler() {
	}

	void CFBamCustXmlLoaderUuidGenHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "UuidGen" ) {
				static const std::string Msg( "Expected QName to be 'UuidGen'" );
			...
			if( ( attrInitValue == NULL ) || ( attrInitValue->length() == 0 ) ) {
				...
				if( 0 == uuid_parse( attrInitValue->c_str(), natInitValue ) ) {
			...
			int32_t* pBlockSize = cflib::CFLibXmlUtil::parseInt32( *attrBlockSize );
			if( pBlockSize != NULL ) {
			...
			origUuidGen = dynamic_cast<cfbam::ICFBamUuidGenObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderUuidGenHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
