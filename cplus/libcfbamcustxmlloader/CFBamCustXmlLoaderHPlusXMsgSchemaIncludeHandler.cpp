...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::~CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler() {
	}

	void CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusXMsgSchemaInclude" ) {
				static const std::string Msg( "Expected QName to be 'HPlusXMsgSchemaInclude'" );
	...
	void CFBamCustXmlLoaderHPlusXMsgSchemaIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( schemaDefObj->isOptionalHppXMsgSchemaIncludeNull() || schemaDefObj->getOptionalHppXMsgSchemaIncludeValue().empty() ) {
				editSchemaDefObj = dynamic_cast<cfbam::ICFBamSchemaDefEditObj*>( schemaDefObj->beginEdit() );
				editSchemaDefObj->setOptionalHppXMsgSchemaIncludeValue( text );
