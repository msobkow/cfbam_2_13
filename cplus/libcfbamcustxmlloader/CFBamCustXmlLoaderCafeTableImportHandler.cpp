...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeTableImportHandler" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeTableImportHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeTableImportHandler::CFBamCustXmlLoaderCafeTableImportHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeTableImportHandler::~CFBamCustXmlLoaderCafeTableImportHandler() {
	}

	void CFBamCustXmlLoaderCafeTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeTableImport" ) {
				static const std::string Msg( "Expected QName to be 'CafeTableImport'" );
	...
	void CFBamCustXmlLoaderCafeTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJTableImportNull() || tableObj->getOptionalJTableImportValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJTableImportValue( text );
