...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderCPlusObjIncludeHandler" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCPlusObjIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderCPlusObjIncludeHandler::CFBamCustXmlLoaderCPlusObjIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCPlusObjIncludeHandler::~CFBamCustXmlLoaderCPlusObjIncludeHandler() {
	}

	void CFBamCustXmlLoaderCPlusObjIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CPlusObjInclude" ) {
				static const std::string Msg( "Expected QName to be 'CPlusObjInclude'" );
	...
	void CFBamCustXmlLoaderCPlusObjIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalCppObjIncludeNull() || tableObj->getOptionalCppObjIncludeValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalCppObjIncludeValue( text );
