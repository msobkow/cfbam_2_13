...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusObjImplementationHandler" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusObjImplementationHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusObjImplementationHandler::CFBamCustXmlLoaderHPlusObjImplementationHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusObjImplementationHandler::~CFBamCustXmlLoaderHPlusObjImplementationHandler() {
	}

	void CFBamCustXmlLoaderHPlusObjImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusObjImplementation" ) {
				static const std::string Msg( "Expected QName to be 'HPlusObjImplementation'" );
	...
	void CFBamCustXmlLoaderHPlusObjImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppObjImplementationNull() || tableObj->getOptionalHppObjImplementationValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppObjImplementationValue( text );
