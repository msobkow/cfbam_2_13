...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusOracleTableImplementationHandler" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::CFBamCustXmlLoaderHPlusOracleTableImplementationHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::~CFBamCustXmlLoaderHPlusOracleTableImplementationHandler() {
	}

	void CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusOracleTableImplementation" ) {
				static const std::string Msg( "Expected QName to be 'HPlusOracleTableImplementation'" );
	...
	void CFBamCustXmlLoaderHPlusOracleTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppOracleTableImplementationNull() || tableObj->getOptionalHppOracleTableImplementationValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppOracleTableImplementationValue( text );
