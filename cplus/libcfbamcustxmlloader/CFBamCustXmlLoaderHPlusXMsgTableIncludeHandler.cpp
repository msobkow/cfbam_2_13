...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::~CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler() {
	}

	void CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusXMsgTableInclude" ) {
				static const std::string Msg( "Expected QName to be 'HPlusXMsgTableInclude'" );
	...
	void CFBamCustXmlLoaderHPlusXMsgTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppXMsgTableIncludeNull() || tableObj->getOptionalHppXMsgTableIncludeValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppXMsgTableIncludeValue( text );
