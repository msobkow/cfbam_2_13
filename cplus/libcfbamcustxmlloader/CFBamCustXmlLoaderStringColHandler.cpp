...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderStringColHandler::CLASS_NAME( "CFBamCustXmlLoaderStringColHandler" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderStringColHandler::S_1( "1" );

	CFBamCustXmlLoaderStringColHandler::CFBamCustXmlLoaderStringColHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderStringColHandler::~CFBamCustXmlLoaderStringColHandler() {
	}

	void CFBamCustXmlLoaderStringColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "StringCol" ) {
				static const std::string Msg( "Expected QName to be 'StringCol'" );
			...
			origStringCol = dynamic_cast<cfbam::ICFBamStringColObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderStringColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
