...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderNumberTypeHandler::CLASS_NAME( "CFBamCustXmlLoaderNumberTypeHandler" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderNumberTypeHandler::S_1( "1" );

	CFBamCustXmlLoaderNumberTypeHandler::CFBamCustXmlLoaderNumberTypeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderNumberTypeHandler::~CFBamCustXmlLoaderNumberTypeHandler() {
	}

	void CFBamCustXmlLoaderNumberTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "NumberType" ) {
				static const std::string Msg( "Expected QName to be 'NumberType'" );
			...
			dynamic_cast<cfbam::ICFBamNumberTypeEditObj*>( tmpBuff->beginEdit() );
			...
			if( ( attrDigits == NULL ) || ( attrDigits->length() <= 0 ) ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
					*attrName + S_Dot + S_Digits );
			}
			if( ( attrPrecis == NULL ) || ( attrPrecis->length() <= 0 ) ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
					*attrName + S_Dot + S_Precis );
			}
			...
			int16_t* pDigits = cflib::CFLibXmlUtil::parseInt16( *attrDigits );
			if( pDigits != NULL ) {
				natDigits = *pDigits;
				delete pDigits;
				pDigits = NULL;
			...
			int16_t* pPrecis = cflib::CFLibXmlUtil::parseInt16( *attrPrecis );
			if( pPrecis != NULL ) {
				natPrecis = *pPrecis;
				delete pPrecis;
				pPrecis = NULL;
			...
			origNumberType = dynamic_cast<cfbam::ICFBamNumberTypeObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderNumberTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
