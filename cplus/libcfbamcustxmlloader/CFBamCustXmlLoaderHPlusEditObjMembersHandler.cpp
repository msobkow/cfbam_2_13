...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusEditObjMembersHandler" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusEditObjMembersHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusEditObjMembersHandler::CFBamCustXmlLoaderHPlusEditObjMembersHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusEditObjMembersHandler::~CFBamCustXmlLoaderHPlusEditObjMembersHandler() {
	}

	void CFBamCustXmlLoaderHPlusEditObjMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusEditObjMembers" ) {
				static const std::string Msg( "Expected QName to be 'HPlusEditObjMembers'" );
	...
	void CFBamCustXmlLoaderHPlusEditObjMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppEditObjMembersNull() || tableObj->getOptionalHppEditObjMembersValue().empty() ) {
				std::string text = curContext->getElementText();
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppEditObjMembersValue( text );
