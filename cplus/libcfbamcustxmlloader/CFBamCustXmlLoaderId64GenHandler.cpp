...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderId64GenHandler::CLASS_NAME( "CFBamCustXmlLoaderId64GenHandler" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderId64GenHandler::S_1( "1" );

	CFBamCustXmlLoaderId64GenHandler::CFBamCustXmlLoaderId64GenHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderId64GenHandler::~CFBamCustXmlLoaderId64GenHandler() {
	}

	void CFBamCustXmlLoaderId64GenHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "Id64Gen" ) {
				static const std::string Msg( "Expected QName to be 'Id64Gen'" );
			...
			if( ( attrSlice == NULL ) || ( attrSlice->length() <= 0 ) ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
					*attrName + S_Dot + S_Slice );
			}
			if( ( attrBlockSize == NULL ) || ( attrBlockSize->length() <= 0 ) ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
					*attrName + S_Dot + S_BlockSize );
			}
			...
			int16_t* pSlice = cflib::CFLibXmlUtil::parseInt16( *attrSlice );
			if( pSlice != NULL ) {
				natSlice = *pSlice;
				delete pSlice;
				pSlice = NULL;
			...
			int64_t* pBlockSize = cflib::CFLibXmlUtil::parseInt64( *attrBlockSize );
			if( pBlockSize != NULL ) {
				natBlockSize = *pBlockSize;
				delete pBlockSize;
				pBlockSize = NULL;
			}
			else {
			...
			if( ( attrDispenser != NULL ) && ( attrDispenser->length() > 0 ) ) {
				refDispenser = dynamic_cast<cfbam::ICFBamTableObj*>( refSchemaDef->getNamedObject( *attrDispenser ) );
			...
			origId64Gen = dynamic_cast<cfbam::ICFBamId64GenObj*>( editBuff->create() );
			editBuff = NULL;
			tmpBuff = NULL;
...
cleanup:
	...
	void CFBamCustXmlLoaderId64GenHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
