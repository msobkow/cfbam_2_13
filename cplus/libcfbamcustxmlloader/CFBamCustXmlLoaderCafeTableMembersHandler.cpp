...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeTableMembersHandler" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeTableMembersHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeTableMembersHandler::CFBamCustXmlLoaderCafeTableMembersHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeTableMembersHandler::~CFBamCustXmlLoaderCafeTableMembersHandler() {
	}

	void CFBamCustXmlLoaderCafeTableMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeTableMembers" ) {
				static const std::string Msg( "Expected QName to be 'CafeTableMembers'" );
	...
	void CFBamCustXmlLoaderCafeTableMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJTableMembersNull() || tableObj->getOptionalJTableMembersValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJTableMembersValue( text );
