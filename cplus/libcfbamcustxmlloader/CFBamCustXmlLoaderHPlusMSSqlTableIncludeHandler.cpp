...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::CLASS_NAME( "CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::S_1( "1" );

	CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::~CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler() {
	}

	void CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "HPlusMSSqlTableInclude" ) {
				static const std::string Msg( "Expected QName to be 'HPlusMSSqlTableInclude'" );
	...
	void CFBamCustXmlLoaderHPlusMSSqlTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalHppMSSqlTableIncludeNull() || tableObj->getOptionalHppMSSqlTableIncludeValue().empty() ) {
				std::string text = curContext->getElementText();
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalHppMSSqlTableIncludeValue( text );
