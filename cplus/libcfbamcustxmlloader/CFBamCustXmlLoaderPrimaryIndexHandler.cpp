...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::CLASS_NAME( "CFBamCustXmlLoaderPrimaryIndexHandler" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderPrimaryIndexHandler::S_1( "1" );

	CFBamCustXmlLoaderPrimaryIndexHandler::CFBamCustXmlLoaderPrimaryIndexHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderPrimaryIndexHandler::~CFBamCustXmlLoaderPrimaryIndexHandler() {
	}

	void CFBamCustXmlLoaderPrimaryIndexHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "PrimaryIndex" ) {
				static const std::string Msg( "Expected QName to be 'PrimaryIndex'" );
			...
			if( refTable->getOptionalLookupPrimaryIndex() != NULL ) {
				throw cflib::CFLibInvalidArgumentException( CLASS_NAME,
					S_ProcName,
			...
			// Save our primary index to the table

			editTable = dynamic_cast<cfbam::ICFBamTableEditObj*>( refTable->beginEdit() );
...
cleanup:
	...
	void CFBamCustXmlLoaderPrimaryIndexHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
