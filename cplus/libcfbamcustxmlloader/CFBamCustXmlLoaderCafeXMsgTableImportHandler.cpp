...
namespace cfbam {
	...
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::CLASS_NAME( "CFBamCustXmlLoaderCafeXMsgTableImportHandler" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_True( "true" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_False( "false" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_Yes( "yes" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_No( "no" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_0( "0" );
	const std::string CFBamCustXmlLoaderCafeXMsgTableImportHandler::S_1( "1" );

	CFBamCustXmlLoaderCafeXMsgTableImportHandler::CFBamCustXmlLoaderCafeXMsgTableImportHandler( cfbam::CFBamCustXmlLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamCustXmlLoaderCafeXMsgTableImportHandler::~CFBamCustXmlLoaderCafeXMsgTableImportHandler() {
	}

	void CFBamCustXmlLoaderCafeXMsgTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		try {
			...
			cQName = NULL;
			if( cppQName != "CafeXMsgTableImport" ) {
				static const std::string Msg( "Expected QName to be 'CafeXMsgTableImport'" );
	...
	void CFBamCustXmlLoaderCafeXMsgTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		try {
			...
			if( tableObj->isOptionalJXMsgTableImportNull() || tableObj->getOptionalJXMsgTableImportValue().empty() ) {
				editTableObj = dynamic_cast<cfbam::ICFBamTableEditObj*>( tableObj->beginEdit() );
				editTableObj->setOptionalJXMsgTableImportValue( text );
