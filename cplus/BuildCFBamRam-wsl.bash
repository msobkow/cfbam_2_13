...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f libcfbamram*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
pushd libcfbamram
	if [ -f Makefile ]; then
		make distclean
	fi
...
pushd libcfbamram
	...
	if [ ! -d /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamram ]; then
		sudo mkdir /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamram
	fi
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamram
	autoreconf -fi
	aclocal
	autoconf
	automake
	...
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamram
	sudo sh -c "cd /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamram && chmod 644 *.hpp"
	#sudo chmod 755 /usr/bin/*cfbam*ram
	sudo chmod 755 /usr/lib/*cfbamram.la
	sudo chmod 755 /usr/lib/*cfbamram.so*
