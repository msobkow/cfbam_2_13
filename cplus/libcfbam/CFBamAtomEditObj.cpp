// Description: C++18 edit object instance implementation for CFBam Atom.
...
namespace cfbam {
	...
	const std::string CFBamAtomEditObj::CLASS_NAME( "CFBamAtomEditObj" );

	CFBamAtomEditObj::CFBamAtomEditObj( cfbam::ICFBamAtomObj* argOrig )
	: ICFBamAtomEditObj(),
	  CFBamValueEditObj( argOrig )
	{
		static const std::string S_ProcName( "CFBamAtomEditObj-construct" );
		static const std::string S_OrigBuff( "origBuff" );
	}

	CFBamAtomEditObj::~CFBamAtomEditObj() {
	}

	const std::string& CFBamAtomEditObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamAtomEditObj::getClassCode() const {
		return( cfbam::CFBamAtomBuff::CLASS_CODE );
	}

	bool CFBamAtomEditObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamAtomBuff::CLASS_CODE ) {
			return( true );
		}
		else {
	...
	std::string CFBamAtomEditObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamAtomEditObj" );
	...
	const std::string CFBamAtomEditObj::getGenDefName() {
		return( cfbam::CFBamAtomBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamAtomEditObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamAtomEditObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamAtomEditObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamAtomEditObj::getObjQualifiedName() {
	...
	std::string CFBamAtomEditObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamAtomEditObj::realize() {
		// We realize this so that it's buffer will get copied to orig during realization
		cfbam::ICFBamAtomObj* retobj = getSchema()->getAtomTableObj()->realizeAtom( dynamic_cast<cfbam::ICFBamAtomObj*>( this ) );
		return( retobj );
	}

	cfbam::ICFBamValueObj* CFBamAtomEditObj::moveUp() {
	...
	cfbam::ICFBamValueObj* CFBamAtomEditObj::moveDown() {
	...
	cfbam::ICFBamValueObj* CFBamAtomEditObj::create() {
		cfbam::ICFBamAtomObj* retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsAtom()->getSchema() )->getAtomTableObj()->createAtom( this );
		// Note that this instance is usually discarded during the creation process,
		// and retobj now references the cached instance of the created object.
		return( retobj );
	}

	cfbam::ICFBamValueEditObj* CFBamAtomEditObj::update() {
		getSchema()->getAtomTableObj()->updateAtom( this );
		return( NULL );
	}

	cfbam::ICFBamValueEditObj* CFBamAtomEditObj::deleteInstance() {
		static const std::string S_MethodName( "deleteInstance" );
		static const std::string S_CannotDeleteNewInstance( "Cannot delete a new instance" );
		if( getIsNew() ) {
			throw cflib::CFLibUsageException( CLASS_NAME, S_MethodName, S_CannotDeleteNewInstance );
		}
		getSchema()->getAtomTableObj()->deleteAtom( this );
		return( NULL );
	}

	cfbam::ICFBamAtomTableObj* CFBamAtomEditObj::getAtomTable() {
		return( orig->getSchema()->getAtomTableObj() );
	}

	cfbam::ICFBamAtomEditObj* CFBamAtomEditObj::getAtomEdit() {
		return( (cfbam::ICFBamAtomEditObj*)this );
	}

	cfbam::ICFBamValueEditObj* CFBamAtomEditObj::beginEdit() {
	...
	cfbam::ICFBamAtomObj* CFBamAtomEditObj::getOrigAsAtom() {
		return( dynamic_cast<cfbam::ICFBamAtomObj*>( orig ) );
	}

	cfbam::CFBamValueBuff* CFBamAtomEditObj::getBuff() {
		return( buff );
	}

	void CFBamAtomEditObj::setBuff( cfbam::CFBamValueBuff* value ) {
		if( buff != value ) {
			CFBamValueEditObj::setBuff( value );
		}
	}

	bool CFBamAtomEditObj::isOptionalDbNameNull() {
		return( getAtomBuff()->isOptionalDbNameNull() );
	}

	const std::string& CFBamAtomEditObj::getOptionalDbNameValue() {
		return( getAtomBuff()->getOptionalDbNameValue() );
	}

	const std::string* CFBamAtomEditObj::getOptionalDbNameReference() {
		return( getAtomBuff()->getOptionalDbNameReference() );
	}

	void CFBamAtomEditObj::setOptionalDbNameNull() {
		if( ! getAtomBuff()->isOptionalDbNameNull() ) {
			getAtomEditBuff()->setOptionalDbNameNull();
		}
	}

	void CFBamAtomEditObj::setOptionalDbNameValue( const std::string& value ) {
		if( getAtomBuff()->isOptionalDbNameNull() ) {
			getAtomEditBuff()->setOptionalDbNameValue( value );
		}
		else if( getAtomBuff()->getOptionalDbNameValue() != value ) {
			getAtomEditBuff()->setOptionalDbNameValue( value );
		}
	}

	void CFBamAtomEditObj::copyBuffToOrig() {
		cfbam::CFBamAtomBuff* origBuff = getOrigAsAtom()->getAtomEditBuff();
		cfbam::CFBamAtomBuff* myBuff = getAtomBuff();
		if( origBuff != myBuff ) {
			*origBuff = *myBuff;
		}
	}

	void CFBamAtomEditObj::copyOrigToBuff() {
		cfbam::CFBamAtomBuff* origBuff = getOrigAsAtom()->getAtomBuff();
		cfbam::CFBamAtomBuff* myBuff = getAtomEditBuff();
		if( origBuff != myBuff ) {
			*myBuff = *origBuff;
		}
	}
