// Description: C++18 edit object instance implementation for CFBam UuidCol.
...
namespace cfbam {
	...
	const std::string CFBamUuidColEditObj::CLASS_NAME( "CFBamUuidColEditObj" );

	CFBamUuidColEditObj::CFBamUuidColEditObj( cfbam::ICFBamUuidColObj* argOrig )
	: ICFBamUuidColEditObj(),
	  CFBamUuidDefEditObj( argOrig )
	{
		static const std::string S_ProcName( "CFBamUuidColEditObj-construct" );
		static const std::string S_OrigBuff( "origBuff" );
	}

	CFBamUuidColEditObj::~CFBamUuidColEditObj() {
	}

	const std::string& CFBamUuidColEditObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamUuidColEditObj::getClassCode() const {
		return( cfbam::CFBamUuidColBuff::CLASS_CODE );
	}

	bool CFBamUuidColEditObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamUuidColBuff::CLASS_CODE ) {
			return( true );
		}
		else {
	...
	std::string CFBamUuidColEditObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamUuidColEditObj" );
		...
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt64( &S_Space, S_TableId, CFBamUuidColEditObj::getRequiredTableId() ) );
		ret.append( S_Postamble );
		return( ret );
	}

	const std::string CFBamUuidColEditObj::getGenDefName() {
		return( cfbam::CFBamUuidColBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamUuidColEditObj::getScope() {
		cfbam::ICFBamTableObj* scope = getRequiredContainerTable();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamUuidColEditObj::getObjScope() {
		cfbam::ICFBamTableObj* scope = getRequiredContainerTable();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamUuidColEditObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidColEditObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidColEditObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamUuidColEditObj::getObjQualifiedName() {
	...
	std::string CFBamUuidColEditObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamUuidColEditObj::realize() {
		// We realize this so that it's buffer will get copied to orig during realization
		cfbam::ICFBamUuidColObj* retobj = getSchema()->getUuidColTableObj()->realizeUuidCol( dynamic_cast<cfbam::ICFBamUuidColObj*>( this ) );
		return( retobj );
	}

	cfbam::ICFBamValueObj* CFBamUuidColEditObj::moveUp() {
	...
	cfbam::ICFBamValueObj* CFBamUuidColEditObj::moveDown() {
	...
	cfbam::ICFBamValueObj* CFBamUuidColEditObj::create() {
		cfbam::ICFBamUuidColObj* retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsUuidCol()->getSchema() )->getUuidColTableObj()->createUuidCol( this );
		// Note that this instance is usually discarded during the creation process,
		// and retobj now references the cached instance of the created object.
		return( retobj );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidColEditObj::update() {
		getSchema()->getUuidColTableObj()->updateUuidCol( this );
		return( NULL );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidColEditObj::deleteInstance() {
		static const std::string S_MethodName( "deleteInstance" );
		static const std::string S_CannotDeleteNewInstance( "Cannot delete a new instance" );
		if( getIsNew() ) {
			throw cflib::CFLibUsageException( CLASS_NAME, S_MethodName, S_CannotDeleteNewInstance );
		}
		getSchema()->getUuidColTableObj()->deleteUuidCol( this );
		return( NULL );
	}

	cfbam::ICFBamUuidColTableObj* CFBamUuidColEditObj::getUuidColTable() {
		return( orig->getSchema()->getUuidColTableObj() );
	}

	cfbam::ICFBamUuidColEditObj* CFBamUuidColEditObj::getUuidColEdit() {
		return( (cfbam::ICFBamUuidColEditObj*)this );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidColEditObj::beginEdit() {
	...
	cfbam::ICFBamUuidColObj* CFBamUuidColEditObj::getOrigAsUuidCol() {
		return( dynamic_cast<cfbam::ICFBamUuidColObj*>( orig ) );
	}

	cfbam::CFBamValueBuff* CFBamUuidColEditObj::getBuff() {
		return( buff );
	}

	void CFBamUuidColEditObj::setBuff( cfbam::CFBamValueBuff* value ) {
		if( buff != value ) {
			CFBamUuidDefEditObj::setBuff( value );
		}
	}

	const int64_t CFBamUuidColEditObj::getRequiredTableId() {
		return( getUuidColBuff()->getRequiredTableId() );
	}

	const int64_t* CFBamUuidColEditObj::getRequiredTableIdReference() {
		return( getUuidColBuff()->getRequiredTableIdReference() );
	}

	void CFBamUuidColEditObj::setRequiredContainerScope( cfbam::ICFBamScopeObj* value ) {
		if( value == NULL ) {
	...
	cfbam::ICFBamTableObj* CFBamUuidColEditObj::getRequiredContainerTable( bool forceRead ) {
		cfbam::ICFBamTableObj* retobj = NULL;
		bool anyMissing = false;
		if( ! anyMissing ) {
			retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsUuidCol()->getSchema() )->getTableTableObj()->readTableByIdIdx( getPKey()->getRequiredTenantId(),
					getUuidColBuff()->getRequiredTableId() );
		}
		return( retobj );
	}

	void CFBamUuidColEditObj::setRequiredContainerTable( cfbam::ICFBamTableObj* value ) {
			
			if( value != NULL ) {
				getPKey()->setRequiredTenantId
( value->getRequiredTenantId() );
				getUuidColEditBuff()->setRequiredTenantId( value->getRequiredTenantId() );
				getUuidColEditBuff()->setRequiredTableId( value->getRequiredId() );
			}

			CFBamUuidDefEditObj::setRequiredContainerScope( value );		}

	void CFBamUuidColEditObj::copyBuffToOrig() {
		cfbam::CFBamUuidColBuff* origBuff = getOrigAsUuidCol()->getUuidColEditBuff();
		cfbam::CFBamUuidColBuff* myBuff = getUuidColBuff();
		if( origBuff != myBuff ) {
			*origBuff = *myBuff;
		}
	}

	void CFBamUuidColEditObj::copyOrigToBuff() {
		cfbam::CFBamUuidColBuff* origBuff = getOrigAsUuidCol()->getUuidColBuff();
		cfbam::CFBamUuidColBuff* myBuff = getUuidColEditBuff();
		if( origBuff != myBuff ) {
			*myBuff = *origBuff;
		}
	}
