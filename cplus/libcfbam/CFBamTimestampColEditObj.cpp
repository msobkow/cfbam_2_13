// Description: C++18 edit object instance implementation for CFBam TimestampCol.
...
namespace cfbam {
	...
	const std::string CFBamTimestampColEditObj::CLASS_NAME( "CFBamTimestampColEditObj" );

	CFBamTimestampColEditObj::CFBamTimestampColEditObj( cfbam::ICFBamTimestampColObj* argOrig )
	: ICFBamTimestampColEditObj(),
	  CFBamTimestampDefEditObj( argOrig )
	{
		static const std::string S_ProcName( "CFBamTimestampColEditObj-construct" );
		static const std::string S_OrigBuff( "origBuff" );
	}

	CFBamTimestampColEditObj::~CFBamTimestampColEditObj() {
	}

	const std::string& CFBamTimestampColEditObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamTimestampColEditObj::getClassCode() const {
		return( cfbam::CFBamTimestampColBuff::CLASS_CODE );
	}

	bool CFBamTimestampColEditObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamTimestampColBuff::CLASS_CODE ) {
			return( true );
		}
		else {
	...
	std::string CFBamTimestampColEditObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamTimestampColEditObj" );
		...
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt64( &S_Space, S_TableId, CFBamTimestampColEditObj::getRequiredTableId() ) );
		ret.append( S_Postamble );
		return( ret );
	}

	const std::string CFBamTimestampColEditObj::getGenDefName() {
		return( cfbam::CFBamTimestampColBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamTimestampColEditObj::getScope() {
		cfbam::ICFBamTableObj* scope = getRequiredContainerTable();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamTimestampColEditObj::getObjScope() {
		cfbam::ICFBamTableObj* scope = getRequiredContainerTable();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamTimestampColEditObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamTimestampColEditObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamTimestampColEditObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamTimestampColEditObj::getObjQualifiedName() {
	...
	std::string CFBamTimestampColEditObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamTimestampColEditObj::realize() {
		// We realize this so that it's buffer will get copied to orig during realization
		cfbam::ICFBamTimestampColObj* retobj = getSchema()->getTimestampColTableObj()->realizeTimestampCol( dynamic_cast<cfbam::ICFBamTimestampColObj*>( this ) );
		return( retobj );
	}

	cfbam::ICFBamValueObj* CFBamTimestampColEditObj::moveUp() {
	...
	cfbam::ICFBamValueObj* CFBamTimestampColEditObj::moveDown() {
	...
	cfbam::ICFBamValueObj* CFBamTimestampColEditObj::create() {
		cfbam::ICFBamTimestampColObj* retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsTimestampCol()->getSchema() )->getTimestampColTableObj()->createTimestampCol( this );
		// Note that this instance is usually discarded during the creation process,
		// and retobj now references the cached instance of the created object.
		return( retobj );
	}

	cfbam::ICFBamValueEditObj* CFBamTimestampColEditObj::update() {
		getSchema()->getTimestampColTableObj()->updateTimestampCol( this );
		return( NULL );
	}

	cfbam::ICFBamValueEditObj* CFBamTimestampColEditObj::deleteInstance() {
		static const std::string S_MethodName( "deleteInstance" );
		static const std::string S_CannotDeleteNewInstance( "Cannot delete a new instance" );
		if( getIsNew() ) {
			throw cflib::CFLibUsageException( CLASS_NAME, S_MethodName, S_CannotDeleteNewInstance );
		}
		getSchema()->getTimestampColTableObj()->deleteTimestampCol( this );
		return( NULL );
	}

	cfbam::ICFBamTimestampColTableObj* CFBamTimestampColEditObj::getTimestampColTable() {
		return( orig->getSchema()->getTimestampColTableObj() );
	}

	cfbam::ICFBamTimestampColEditObj* CFBamTimestampColEditObj::getTimestampColEdit() {
		return( (cfbam::ICFBamTimestampColEditObj*)this );
	}

	cfbam::ICFBamValueEditObj* CFBamTimestampColEditObj::beginEdit() {
	...
	cfbam::ICFBamTimestampColObj* CFBamTimestampColEditObj::getOrigAsTimestampCol() {
		return( dynamic_cast<cfbam::ICFBamTimestampColObj*>( orig ) );
	}

	cfbam::CFBamValueBuff* CFBamTimestampColEditObj::getBuff() {
		return( buff );
	}

	void CFBamTimestampColEditObj::setBuff( cfbam::CFBamValueBuff* value ) {
		if( buff != value ) {
			CFBamTimestampDefEditObj::setBuff( value );
		}
	}

	const int64_t CFBamTimestampColEditObj::getRequiredTableId() {
		return( getTimestampColBuff()->getRequiredTableId() );
	}

	const int64_t* CFBamTimestampColEditObj::getRequiredTableIdReference() {
		return( getTimestampColBuff()->getRequiredTableIdReference() );
	}

	void CFBamTimestampColEditObj::setRequiredContainerScope( cfbam::ICFBamScopeObj* value ) {
		if( value == NULL ) {
	...
	cfbam::ICFBamTableObj* CFBamTimestampColEditObj::getRequiredContainerTable( bool forceRead ) {
		cfbam::ICFBamTableObj* retobj = NULL;
		bool anyMissing = false;
		if( ! anyMissing ) {
			retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsTimestampCol()->getSchema() )->getTableTableObj()->readTableByIdIdx( getPKey()->getRequiredTenantId(),
					getTimestampColBuff()->getRequiredTableId() );
		}
		return( retobj );
	}

	void CFBamTimestampColEditObj::setRequiredContainerTable( cfbam::ICFBamTableObj* value ) {
			
			if( value != NULL ) {
				getPKey()->setRequiredTenantId
( value->getRequiredTenantId() );
				getTimestampColEditBuff()->setRequiredTenantId( value->getRequiredTenantId() );
				getTimestampColEditBuff()->setRequiredTableId( value->getRequiredId() );
			}

			CFBamTimestampDefEditObj::setRequiredContainerScope( value );		}

	void CFBamTimestampColEditObj::copyBuffToOrig() {
		cfbam::CFBamTimestampColBuff* origBuff = getOrigAsTimestampCol()->getTimestampColEditBuff();
		cfbam::CFBamTimestampColBuff* myBuff = getTimestampColBuff();
		if( origBuff != myBuff ) {
			*origBuff = *myBuff;
		}
	}

	void CFBamTimestampColEditObj::copyOrigToBuff() {
		cfbam::CFBamTimestampColBuff* origBuff = getOrigAsTimestampCol()->getTimestampColBuff();
		cfbam::CFBamTimestampColBuff* myBuff = getTimestampColEditBuff();
		if( origBuff != myBuff ) {
			*myBuff = *origBuff;
		}
	}
