// Description: C++18 edit object instance implementation for CFBam UuidGen.
...
namespace cfbam {
	...
	const std::string CFBamUuidGenEditObj::CLASS_NAME( "CFBamUuidGenEditObj" );

	CFBamUuidGenEditObj::CFBamUuidGenEditObj( cfbam::ICFBamUuidGenObj* argOrig )
	: ICFBamUuidGenEditObj(),
	  CFBamUuidTypeEditObj( argOrig )
	{
		static const std::string S_ProcName( "CFBamUuidGenEditObj-construct" );
		static const std::string S_OrigBuff( "origBuff" );
	}

	CFBamUuidGenEditObj::~CFBamUuidGenEditObj() {
	}

	const std::string& CFBamUuidGenEditObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamUuidGenEditObj::getClassCode() const {
		return( cfbam::CFBamUuidGenBuff::CLASS_CODE );
	}

	bool CFBamUuidGenEditObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamUuidGenBuff::CLASS_CODE ) {
			return( true );
		}
		else {
			return( CFBamUuidTypeEditObj::implementsClassCode( value ) );
		}
	}

	std::string CFBamUuidGenEditObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamUuidGenEditObj" );
		...
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt16( &S_Space, S_Slice, CFBamUuidGenEditObj::getRequiredSlice() ) );
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt32( &S_Space, S_BlockSize, CFBamUuidGenEditObj::getRequiredBlockSize() ) );
		ret.append( S_Postamble );
		return( ret );
	}

	const std::string CFBamUuidGenEditObj::getGenDefName() {
		return( cfbam::CFBamUuidGenBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamUuidGenEditObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidGenEditObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidGenEditObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamUuidGenEditObj::getObjQualifiedName() {
	...
	std::string CFBamUuidGenEditObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamUuidGenEditObj::realize() {
		// We realize this so that it's buffer will get copied to orig during realization
		cfbam::ICFBamUuidGenObj* retobj = getSchema()->getUuidGenTableObj()->realizeUuidGen( dynamic_cast<cfbam::ICFBamUuidGenObj*>( this ) );
		return( retobj );
	}

	cfbam::ICFBamValueObj* CFBamUuidGenEditObj::moveUp() {
	...
	cfbam::ICFBamValueObj* CFBamUuidGenEditObj::moveDown() {
	...
	cfbam::ICFBamValueObj* CFBamUuidGenEditObj::create() {
		cfbam::ICFBamUuidGenObj* retobj = dynamic_cast<cfbam::ICFBamSchemaObj*>( getOrigAsUuidGen()->getSchema() )->getUuidGenTableObj()->createUuidGen( this );
		// Note that this instance is usually discarded during the creation process,
		// and retobj now references the cached instance of the created object.
		return( retobj );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidGenEditObj::update() {
		getSchema()->getUuidGenTableObj()->updateUuidGen( this );
		return( NULL );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidGenEditObj::deleteInstance() {
		static const std::string S_MethodName( "deleteInstance" );
		static const std::string S_CannotDeleteNewInstance( "Cannot delete a new instance" );
		if( getIsNew() ) {
			throw cflib::CFLibUsageException( CLASS_NAME, S_MethodName, S_CannotDeleteNewInstance );
		}
		getSchema()->getUuidGenTableObj()->deleteUuidGen( this );
		return( NULL );
	}

	cfbam::ICFBamUuidGenTableObj* CFBamUuidGenEditObj::getUuidGenTable() {
		return( orig->getSchema()->getUuidGenTableObj() );
	}

	cfbam::ICFBamUuidGenEditObj* CFBamUuidGenEditObj::getUuidGenEdit() {
		return( (cfbam::ICFBamUuidGenEditObj*)this );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidGenEditObj::beginEdit() {
	...
	cfbam::ICFBamUuidGenObj* CFBamUuidGenEditObj::getOrigAsUuidGen() {
		return( dynamic_cast<cfbam::ICFBamUuidGenObj*>( orig ) );
	}

	cfbam::CFBamValueBuff* CFBamUuidGenEditObj::getBuff() {
		return( buff );
	}

	void CFBamUuidGenEditObj::setBuff( cfbam::CFBamValueBuff* value ) {
		if( buff != value ) {
			CFBamUuidTypeEditObj::setBuff( value );
		}
	}

	const int16_t CFBamUuidGenEditObj::getRequiredSlice() {
		return( getUuidGenBuff()->getRequiredSlice() );
	}

	const int16_t* CFBamUuidGenEditObj::getRequiredSliceReference() {
		return( getUuidGenBuff()->getRequiredSliceReference() );
	}

	void CFBamUuidGenEditObj::setRequiredSlice( const int16_t value ) {
		if( getUuidGenBuff()->getRequiredSlice() != value ) {
			getUuidGenEditBuff()->setRequiredSlice( value );
		}
	}

	const int32_t CFBamUuidGenEditObj::getRequiredBlockSize() {
		return( getUuidGenBuff()->getRequiredBlockSize() );
	}

	const int32_t* CFBamUuidGenEditObj::getRequiredBlockSizeReference() {
		return( getUuidGenBuff()->getRequiredBlockSizeReference() );
	}

	void CFBamUuidGenEditObj::setRequiredBlockSize( const int32_t value ) {
		if( getUuidGenBuff()->getRequiredBlockSize() != value ) {
			getUuidGenEditBuff()->setRequiredBlockSize( value );
		}
	}

	void CFBamUuidGenEditObj::copyBuffToOrig() {
		cfbam::CFBamUuidGenBuff* origBuff = getOrigAsUuidGen()->getUuidGenEditBuff();
		cfbam::CFBamUuidGenBuff* myBuff = getUuidGenBuff();
		if( origBuff != myBuff ) {
			*origBuff = *myBuff;
		}
	}

	void CFBamUuidGenEditObj::copyOrigToBuff() {
		cfbam::CFBamUuidGenBuff* origBuff = getOrigAsUuidGen()->getUuidGenBuff();
		cfbam::CFBamUuidGenBuff* myBuff = getUuidGenEditBuff();
		if( origBuff != myBuff ) {
			*myBuff = *origBuff;
		}
	}
