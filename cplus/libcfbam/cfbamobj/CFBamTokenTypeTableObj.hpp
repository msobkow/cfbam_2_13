...
namespace cfbam {
	...
	class CFBamTokenTypeTableObj
	: public virtual ICFBamTokenTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>* allTokenType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTokenTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTokenTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamTokenTypeTableObj();
		CFBamTokenTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTokenTypeTableObj();
		...
		virtual cfbam::ICFBamTokenTypeObj* newInstance();

		virtual cfbam::ICFBamTokenTypeEditObj* newEditInstance( cfbam::ICFBamTokenTypeObj* orig );

		virtual cfbam::ICFBamTokenTypeObj* realizeTokenType( cfbam::ICFBamTokenTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTokenType( cfbam::ICFBamTokenTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamTokenTypeObj* createTokenType( cfbam::ICFBamTokenTypeEditObj* Obj );

		virtual cfbam::ICFBamTokenTypeObj* readTokenType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTokenTypeObj* lockTokenType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readAllTokenType( bool forceRead = false );

		virtual cfbam::ICFBamTokenTypeObj* readTokenTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTokenTypeObj* readTokenTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readTokenTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamTokenTypeObj* readCachedTokenType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTokenTypeObj* readCachedTokenTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTokenTypeObj* readCachedTokenTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTokenTypeObj*> readCachedTokenTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTokenTypeObj* updateTokenType( cfbam::ICFBamTokenTypeEditObj* Obj );

		virtual void deleteTokenType( cfbam::ICFBamTokenTypeEditObj* Obj );

		virtual void deleteTokenTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTokenTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTokenTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteTokenTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTokenTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTokenTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTokenTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTokenTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTokenTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTokenTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTokenTypeObj* moveUpTokenType( cfbam::ICFBamTokenTypeObj* Obj );

		virtual cfbam::ICFBamTokenTypeObj* moveDownTokenType( cfbam::ICFBamTokenTypeObj* Obj );

		virtual void reallyDetachFromIndexesTokenType( cfbam::ICFBamTokenTypeObj* Obj );
