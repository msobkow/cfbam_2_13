...
namespace cfbam {
	...
	class CFBamTimeTypeTableObj
	: public virtual ICFBamTimeTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>* allTimeType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTimeTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTimeTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamTimeTypeTableObj();
		CFBamTimeTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTimeTypeTableObj();
		...
		virtual cfbam::ICFBamTimeTypeObj* newInstance();

		virtual cfbam::ICFBamTimeTypeEditObj* newEditInstance( cfbam::ICFBamTimeTypeObj* orig );

		virtual cfbam::ICFBamTimeTypeObj* realizeTimeType( cfbam::ICFBamTimeTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTimeType( cfbam::ICFBamTimeTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamTimeTypeObj* createTimeType( cfbam::ICFBamTimeTypeEditObj* Obj );

		virtual cfbam::ICFBamTimeTypeObj* readTimeType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTimeTypeObj* lockTimeType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readAllTimeType( bool forceRead = false );

		virtual cfbam::ICFBamTimeTypeObj* readTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTimeTypeObj* readTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamTimeTypeObj* readCachedTimeType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTimeTypeObj* readCachedTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTimeTypeObj* readCachedTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimeTypeObj*> readCachedTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTimeTypeObj* updateTimeType( cfbam::ICFBamTimeTypeEditObj* Obj );

		virtual void deleteTimeType( cfbam::ICFBamTimeTypeEditObj* Obj );

		virtual void deleteTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTimeTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTimeTypeObj* moveUpTimeType( cfbam::ICFBamTimeTypeObj* Obj );

		virtual cfbam::ICFBamTimeTypeObj* moveDownTimeType( cfbam::ICFBamTimeTypeObj* Obj );

		virtual void reallyDetachFromIndexesTimeType( cfbam::ICFBamTimeTypeObj* Obj );
