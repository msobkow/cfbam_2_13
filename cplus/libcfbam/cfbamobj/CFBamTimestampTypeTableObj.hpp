...
namespace cfbam {
	...
	class CFBamTimestampTypeTableObj
	: public virtual ICFBamTimestampTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>* allTimestampType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTimestampTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTimestampTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamTimestampTypeTableObj();
		CFBamTimestampTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTimestampTypeTableObj();
		...
		virtual cfbam::ICFBamTimestampTypeObj* newInstance();

		virtual cfbam::ICFBamTimestampTypeEditObj* newEditInstance( cfbam::ICFBamTimestampTypeObj* orig );

		virtual cfbam::ICFBamTimestampTypeObj* realizeTimestampType( cfbam::ICFBamTimestampTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTimestampType( cfbam::ICFBamTimestampTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamTimestampTypeObj* createTimestampType( cfbam::ICFBamTimestampTypeEditObj* Obj );

		virtual cfbam::ICFBamTimestampTypeObj* readTimestampType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTimestampTypeObj* lockTimestampType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readAllTimestampType( bool forceRead = false );

		virtual cfbam::ICFBamTimestampTypeObj* readTimestampTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTimestampTypeObj* readTimestampTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readTimestampTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamTimestampTypeObj* readCachedTimestampType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTimestampTypeObj* readCachedTimestampTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTimestampTypeObj* readCachedTimestampTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimestampTypeObj*> readCachedTimestampTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTimestampTypeObj* updateTimestampType( cfbam::ICFBamTimestampTypeEditObj* Obj );

		virtual void deleteTimestampType( cfbam::ICFBamTimestampTypeEditObj* Obj );

		virtual void deleteTimestampTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTimestampTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTimestampTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteTimestampTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTimestampTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTimestampTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTimestampTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTimestampTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTimestampTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTimestampTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTimestampTypeObj* moveUpTimestampType( cfbam::ICFBamTimestampTypeObj* Obj );

		virtual cfbam::ICFBamTimestampTypeObj* moveDownTimestampType( cfbam::ICFBamTimestampTypeObj* Obj );

		virtual void reallyDetachFromIndexesTimestampType( cfbam::ICFBamTimestampTypeObj* Obj );
