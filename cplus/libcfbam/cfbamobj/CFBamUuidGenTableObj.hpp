...
namespace cfbam {
	...
	class CFBamUuidGenTableObj
	: public virtual ICFBamUuidGenTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>* allUuidGen;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamUuidGenObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamUuidTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidGenObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamUuidGenTableObj();
		CFBamUuidGenTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamUuidGenTableObj();
		...
		virtual cfbam::ICFBamUuidGenObj* newInstance();

		virtual cfbam::ICFBamUuidGenEditObj* newEditInstance( cfbam::ICFBamUuidGenObj* orig );

		virtual cfbam::ICFBamUuidGenObj* realizeUuidGen( cfbam::ICFBamUuidGenObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeUuidGen( cfbam::ICFBamUuidGenObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamUuidGenObj* createUuidGen( cfbam::ICFBamUuidGenEditObj* Obj );

		virtual cfbam::ICFBamUuidGenObj* readUuidGen( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamUuidGenObj* lockUuidGen( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readAllUuidGen( bool forceRead = false );

		virtual cfbam::ICFBamUuidGenObj* readUuidGenByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamUuidGenObj* readUuidGenByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readUuidGenBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamUuidGenObj* readCachedUuidGen( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamUuidGenObj* readCachedUuidGenByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamUuidGenObj* readCachedUuidGenByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamUuidGenObj*> readCachedUuidGenBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamUuidGenObj* updateUuidGen( cfbam::ICFBamUuidGenEditObj* Obj );

		virtual void deleteUuidGen( cfbam::ICFBamUuidGenEditObj* Obj );

		virtual void deleteUuidGenByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteUuidGenByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteUuidGenByValTentIdx( const int64_t TenantId );

		virtual void deleteUuidGenByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteUuidGenByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteUuidGenByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteUuidGenByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteUuidGenByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteUuidGenByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteUuidGenBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamUuidGenObj* moveUpUuidGen( cfbam::ICFBamUuidGenObj* Obj );

		virtual cfbam::ICFBamUuidGenObj* moveDownUuidGen( cfbam::ICFBamUuidGenObj* Obj );

		virtual void reallyDetachFromIndexesUuidGen( cfbam::ICFBamUuidGenObj* Obj );
