...
namespace cfbam {
	...
	class CFBamStringColTableObj
	: public virtual ICFBamStringColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>* allStringCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamStringColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamStringColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamStringColTableObj();
		CFBamStringColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamStringColTableObj();
		...
		virtual cfbam::ICFBamStringColObj* newInstance();

		virtual cfbam::ICFBamStringColEditObj* newEditInstance( cfbam::ICFBamStringColObj* orig );

		virtual cfbam::ICFBamStringColObj* realizeStringCol( cfbam::ICFBamStringColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeStringCol( cfbam::ICFBamStringColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamStringColObj* createStringCol( cfbam::ICFBamStringColEditObj* Obj );

		virtual cfbam::ICFBamStringColObj* readStringCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamStringColObj* lockStringCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamStringColObj*> readAllStringCol( bool forceRead = false );

		virtual cfbam::ICFBamStringColObj* readStringColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamStringColObj* readStringColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringColObj*> readStringColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamStringColObj* readCachedStringCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamStringColObj* readCachedStringColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamStringColObj* readCachedStringColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamStringColObj*> readCachedStringColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamStringColObj* updateStringCol( cfbam::ICFBamStringColEditObj* Obj );

		virtual void deleteStringCol( cfbam::ICFBamStringColEditObj* Obj );

		virtual void deleteStringColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteStringColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteStringColByValTentIdx( const int64_t TenantId );

		virtual void deleteStringColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteStringColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteStringColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteStringColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteStringColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteStringColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteStringColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamStringColObj* moveUpStringCol( cfbam::ICFBamStringColObj* Obj );

		virtual cfbam::ICFBamStringColObj* moveDownStringCol( cfbam::ICFBamStringColObj* Obj );

		virtual void reallyDetachFromIndexesStringCol( cfbam::ICFBamStringColObj* Obj );
