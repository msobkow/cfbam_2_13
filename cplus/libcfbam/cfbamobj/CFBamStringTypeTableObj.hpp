...
namespace cfbam {
	...
	class CFBamStringTypeTableObj
	: public virtual ICFBamStringTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>* allStringType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamStringTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamStringTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamStringTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamStringTypeTableObj();
		CFBamStringTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamStringTypeTableObj();
		...
		virtual cfbam::ICFBamStringTypeObj* newInstance();

		virtual cfbam::ICFBamStringTypeEditObj* newEditInstance( cfbam::ICFBamStringTypeObj* orig );

		virtual cfbam::ICFBamStringTypeObj* realizeStringType( cfbam::ICFBamStringTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeStringType( cfbam::ICFBamStringTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamStringTypeObj* createStringType( cfbam::ICFBamStringTypeEditObj* Obj );

		virtual cfbam::ICFBamStringTypeObj* readStringType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamStringTypeObj* lockStringType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readAllStringType( bool forceRead = false );

		virtual cfbam::ICFBamStringTypeObj* readStringTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamStringTypeObj* readStringTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readStringTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamStringTypeObj* readCachedStringType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamStringTypeObj* readCachedStringTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamStringTypeObj* readCachedStringTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamStringTypeObj*> readCachedStringTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamStringTypeObj* updateStringType( cfbam::ICFBamStringTypeEditObj* Obj );

		virtual void deleteStringType( cfbam::ICFBamStringTypeEditObj* Obj );

		virtual void deleteStringTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteStringTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteStringTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteStringTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteStringTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteStringTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteStringTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteStringTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteStringTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteStringTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamStringTypeObj* moveUpStringType( cfbam::ICFBamStringTypeObj* Obj );

		virtual cfbam::ICFBamStringTypeObj* moveDownStringType( cfbam::ICFBamStringTypeObj* Obj );

		virtual void reallyDetachFromIndexesStringType( cfbam::ICFBamStringTypeObj* Obj );
