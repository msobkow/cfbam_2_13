...
namespace cfbam {
	...
	class CFBamParamTableObj
	: public virtual ICFBamParamTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>* members;
		std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>* allParam;
		std::map< cfbam::CFBamParamByUNameIdxKey,
			cfbam::ICFBamParamObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamParamByValTentIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamParamByServerMethodIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByServerMethodIdx;
		std::map< cfbam::CFBamParamByDefSchemaIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamParamByServerTypeIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByServerTypeIdx;
		std::map< cfbam::CFBamParamByPrevIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamParamByNextIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamParamByContPrevIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamParamByContNextIdxKey,
			std::map<cfbam::CFBamParamPKey, cfbam::ICFBamParamObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamParamTableObj();
		CFBamParamTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamParamTableObj();
		...
		virtual cfbam::ICFBamParamObj* newInstance();

		virtual cfbam::ICFBamParamEditObj* newEditInstance( cfbam::ICFBamParamObj* orig );

		virtual cfbam::ICFBamParamObj* realizeParam( cfbam::ICFBamParamObj* Obj );
		...
		virtual void deepDisposeByServerMethodIdx( const int64_t TenantId,
			const int64_t ServerMethodId );
		virtual void deepDisposeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );
		virtual void deepDisposeByServerTypeIdx( const int64_t* TypeTenantId,
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeParam( cfbam::ICFBamParamObj* Obj );
		virtual cfbam::ICFBamParamObj* createParam( cfbam::ICFBamParamEditObj* Obj );

		virtual cfbam::ICFBamParamObj* readParam( cfbam::CFBamParamPKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamParamObj* lockParam( cfbam::CFBamParamPKey* pkey );

		virtual std::vector<cfbam::ICFBamParamObj*> readAllParam( bool forceRead = false );

		virtual cfbam::ICFBamParamObj* readParamByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamParamObj* readParamByUNameIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByServerMethodIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByServerTypeIdx( const int64_t* TypeTenantId,
			const int64_t* TypeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByContPrevIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamParamObj*> readParamByContNextIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamParamObj* readCachedParam( cfbam::CFBamParamPKey* pkey );

		virtual cfbam::ICFBamParamObj* readCachedParamByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamParamObj* readCachedParamByUNameIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByServerMethodIdx( const int64_t TenantId,
			const int64_t ServerMethodId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByServerTypeIdx( const int64_t* TypeTenantId,
			const int64_t* TypeId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByContPrevIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamParamObj*> readCachedParamByContNextIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* NextId );

		virtual cfbam::ICFBamParamObj* updateParam( cfbam::ICFBamParamEditObj* Obj );

		virtual void deleteParam( cfbam::ICFBamParamEditObj* Obj );

		virtual void deleteParamByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteParamByUNameIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const std::string& Name );

		virtual void deleteParamByValTentIdx( const int64_t TenantId );

		virtual void deleteParamByServerMethodIdx( const int64_t TenantId,
			const int64_t ServerMethodId );

		virtual void deleteParamByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteParamByServerTypeIdx( const int64_t* TypeTenantId,
			const int64_t* TypeId );

		virtual void deleteParamByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteParamByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteParamByContPrevIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* PrevId );

		virtual void deleteParamByContNextIdx( const int64_t TenantId,
			const int64_t ServerMethodId,
			const int64_t* NextId );

		virtual cfbam::ICFBamParamObj* moveUpParam( cfbam::ICFBamParamObj* Obj );

		virtual cfbam::ICFBamParamObj* moveDownParam( cfbam::ICFBamParamObj* Obj );

		virtual void reallyDetachFromIndexesParam( cfbam::ICFBamParamObj* Obj );
