...
namespace cfbam {
	...
	class CFBamFloatColTableObj
	: public virtual ICFBamFloatColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>* allFloatCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamFloatColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamFloatColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamFloatColTableObj();
		CFBamFloatColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamFloatColTableObj();
		...
		virtual cfbam::ICFBamFloatColObj* newInstance();

		virtual cfbam::ICFBamFloatColEditObj* newEditInstance( cfbam::ICFBamFloatColObj* orig );

		virtual cfbam::ICFBamFloatColObj* realizeFloatCol( cfbam::ICFBamFloatColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeFloatCol( cfbam::ICFBamFloatColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamFloatColObj* createFloatCol( cfbam::ICFBamFloatColEditObj* Obj );

		virtual cfbam::ICFBamFloatColObj* readFloatCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamFloatColObj* lockFloatCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readAllFloatCol( bool forceRead = false );

		virtual cfbam::ICFBamFloatColObj* readFloatColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamFloatColObj* readFloatColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readFloatColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamFloatColObj* readCachedFloatCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamFloatColObj* readCachedFloatColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamFloatColObj* readCachedFloatColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamFloatColObj*> readCachedFloatColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamFloatColObj* updateFloatCol( cfbam::ICFBamFloatColEditObj* Obj );

		virtual void deleteFloatCol( cfbam::ICFBamFloatColEditObj* Obj );

		virtual void deleteFloatColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteFloatColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteFloatColByValTentIdx( const int64_t TenantId );

		virtual void deleteFloatColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteFloatColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteFloatColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteFloatColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteFloatColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteFloatColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteFloatColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamFloatColObj* moveUpFloatCol( cfbam::ICFBamFloatColObj* Obj );

		virtual cfbam::ICFBamFloatColObj* moveDownFloatCol( cfbam::ICFBamFloatColObj* Obj );

		virtual void reallyDetachFromIndexesFloatCol( cfbam::ICFBamFloatColObj* Obj );
