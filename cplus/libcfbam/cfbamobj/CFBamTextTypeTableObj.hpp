...
namespace cfbam {
	...
	class CFBamTextTypeTableObj
	: public virtual ICFBamTextTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>* allTextType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTextTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTextTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamTextTypeTableObj();
		CFBamTextTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTextTypeTableObj();
		...
		virtual cfbam::ICFBamTextTypeObj* newInstance();

		virtual cfbam::ICFBamTextTypeEditObj* newEditInstance( cfbam::ICFBamTextTypeObj* orig );

		virtual cfbam::ICFBamTextTypeObj* realizeTextType( cfbam::ICFBamTextTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTextType( cfbam::ICFBamTextTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamTextTypeObj* createTextType( cfbam::ICFBamTextTypeEditObj* Obj );

		virtual cfbam::ICFBamTextTypeObj* readTextType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTextTypeObj* lockTextType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readAllTextType( bool forceRead = false );

		virtual cfbam::ICFBamTextTypeObj* readTextTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTextTypeObj* readTextTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readTextTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamTextTypeObj* readCachedTextType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTextTypeObj* readCachedTextTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTextTypeObj* readCachedTextTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTextTypeObj*> readCachedTextTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTextTypeObj* updateTextType( cfbam::ICFBamTextTypeEditObj* Obj );

		virtual void deleteTextType( cfbam::ICFBamTextTypeEditObj* Obj );

		virtual void deleteTextTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTextTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTextTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteTextTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTextTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTextTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTextTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTextTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTextTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTextTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTextTypeObj* moveUpTextType( cfbam::ICFBamTextTypeObj* Obj );

		virtual cfbam::ICFBamTextTypeObj* moveDownTextType( cfbam::ICFBamTextTypeObj* Obj );

		virtual void reallyDetachFromIndexesTextType( cfbam::ICFBamTextTypeObj* Obj );
