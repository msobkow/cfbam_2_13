...
namespace cfbam {
	...
	class CFBamTableColTableObj
	: public virtual ICFBamTableColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>* allTableCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTableColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTableColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByTableIdx;
		std::map< cfbam::CFBamTableColByDataIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTableColObj*>*>* indexByDataIdx;
	...
	public:
		...
		CFBamTableColTableObj();
		CFBamTableColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTableColTableObj();
		...
		virtual cfbam::ICFBamTableColObj* newInstance();

		virtual cfbam::ICFBamTableColEditObj* newEditInstance( cfbam::ICFBamTableColObj* orig );

		virtual cfbam::ICFBamTableColObj* realizeTableCol( cfbam::ICFBamTableColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTableCol( cfbam::ICFBamTableColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual void deepDisposeByDataIdx( const int64_t* DataTenantId,
			const int64_t* DataId );
		virtual cfbam::ICFBamTableColObj* createTableCol( cfbam::ICFBamTableColEditObj* Obj );

		virtual cfbam::ICFBamTableColObj* readTableCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTableColObj* lockTableCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTableColObj*> readAllTableCol( bool forceRead = false );

		virtual cfbam::ICFBamTableColObj* readTableColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTableColObj* readTableColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableColObj*> readTableColByDataIdx( const int64_t* DataTenantId,
			const int64_t* DataId,
			bool forceRead = false );

		virtual cfbam::ICFBamTableColObj* readCachedTableCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTableColObj* readCachedTableColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTableColObj* readCachedTableColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual std::vector<cfbam::ICFBamTableColObj*> readCachedTableColByDataIdx( const int64_t* DataTenantId,
			const int64_t* DataId );

		virtual cfbam::ICFBamTableColObj* updateTableCol( cfbam::ICFBamTableColEditObj* Obj );

		virtual void deleteTableCol( cfbam::ICFBamTableColEditObj* Obj );

		virtual void deleteTableColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTableColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTableColByValTentIdx( const int64_t TenantId );

		virtual void deleteTableColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTableColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTableColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTableColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTableColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTableColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTableColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual void deleteTableColByDataIdx( const int64_t* DataTenantId,
			const int64_t* DataId );

		virtual cfbam::ICFBamTableColObj* moveUpTableCol( cfbam::ICFBamTableColObj* Obj );

		virtual cfbam::ICFBamTableColObj* moveDownTableCol( cfbam::ICFBamTableColObj* Obj );

		virtual void reallyDetachFromIndexesTableCol( cfbam::ICFBamTableColObj* Obj );
