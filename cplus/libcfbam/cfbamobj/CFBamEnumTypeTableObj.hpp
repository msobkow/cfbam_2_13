...
namespace cfbam {
	...
	class CFBamEnumTypeTableObj
	: public virtual ICFBamEnumTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>* allEnumType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamEnumTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamEnumTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamEnumTypeTableObj();
		CFBamEnumTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamEnumTypeTableObj();
		...
		virtual cfbam::ICFBamEnumTypeObj* newInstance();

		virtual cfbam::ICFBamEnumTypeEditObj* newEditInstance( cfbam::ICFBamEnumTypeObj* orig );

		virtual cfbam::ICFBamEnumTypeObj* realizeEnumType( cfbam::ICFBamEnumTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeEnumType( cfbam::ICFBamEnumTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamEnumTypeObj* createEnumType( cfbam::ICFBamEnumTypeEditObj* Obj );

		virtual cfbam::ICFBamEnumTypeObj* readEnumType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamEnumTypeObj* lockEnumType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readAllEnumType( bool forceRead = false );

		virtual cfbam::ICFBamEnumTypeObj* readEnumTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamEnumTypeObj* readEnumTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readEnumTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamEnumTypeObj* readCachedEnumType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamEnumTypeObj* readCachedEnumTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamEnumTypeObj* readCachedEnumTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamEnumTypeObj*> readCachedEnumTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamEnumTypeObj* updateEnumType( cfbam::ICFBamEnumTypeEditObj* Obj );

		virtual void deleteEnumType( cfbam::ICFBamEnumTypeEditObj* Obj );

		virtual void deleteEnumTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteEnumTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteEnumTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteEnumTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteEnumTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteEnumTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteEnumTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteEnumTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteEnumTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteEnumTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamEnumTypeObj* moveUpEnumType( cfbam::ICFBamEnumTypeObj* Obj );

		virtual cfbam::ICFBamEnumTypeObj* moveDownEnumType( cfbam::ICFBamEnumTypeObj* Obj );

		virtual void reallyDetachFromIndexesEnumType( cfbam::ICFBamEnumTypeObj* Obj );
