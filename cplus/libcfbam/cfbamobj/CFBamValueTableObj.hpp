...
namespace cfbam {
	...
	class CFBamValueTableObj
	: public virtual ICFBamValueTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>* allValue;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamValueObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamValueObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamValueTableObj();
		CFBamValueTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamValueTableObj();
		...
		virtual cfbam::ICFBamValueObj* newInstance();

		virtual cfbam::ICFBamValueEditObj* newEditInstance( cfbam::ICFBamValueObj* orig );

		virtual cfbam::ICFBamValueObj* constructByClassCode( const classcode_t argClassCode );

		virtual cfbam::ICFBamValueObj* realizeValue( cfbam::ICFBamValueObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeValue( cfbam::ICFBamValueObj* Obj );
		virtual cfbam::ICFBamValueObj* createValue( cfbam::ICFBamValueEditObj* Obj );

		virtual cfbam::ICFBamValueObj* readValue( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamValueObj* lockValue( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamValueObj*> readAllValue( bool forceRead = false );

		virtual cfbam::ICFBamValueObj* readValueByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamValueObj* readValueByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamValueObj*> readValueByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamValueObj* readCachedValue( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamValueObj* readCachedValueByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamValueObj* readCachedValueByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamValueObj*> readCachedValueByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamValueObj* readValueByLookupUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual cfbam::ICFBamValueObj* readCachedValueByLookupUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual cfbam::ICFBamValueObj* updateValue( cfbam::ICFBamValueEditObj* Obj );

		virtual void deleteValue( cfbam::ICFBamValueEditObj* Obj );

		virtual void deleteValueByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteValueByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteValueByValTentIdx( const int64_t TenantId );

		virtual void deleteValueByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteValueByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteValueByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteValueByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteValueByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteValueByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamValueObj* moveUpValue( cfbam::ICFBamValueObj* Obj );

		virtual cfbam::ICFBamValueObj* moveDownValue( cfbam::ICFBamValueObj* Obj );

		virtual void reallyDetachFromIndexesValue( cfbam::ICFBamValueObj* Obj );
