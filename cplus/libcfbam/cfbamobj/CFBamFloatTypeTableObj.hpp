...
namespace cfbam {
	...
	class CFBamFloatTypeTableObj
	: public virtual ICFBamFloatTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>* allFloatType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamFloatTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamFloatTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamFloatTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamFloatTypeTableObj();
		CFBamFloatTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamFloatTypeTableObj();
		...
		virtual cfbam::ICFBamFloatTypeObj* newInstance();

		virtual cfbam::ICFBamFloatTypeEditObj* newEditInstance( cfbam::ICFBamFloatTypeObj* orig );

		virtual cfbam::ICFBamFloatTypeObj* realizeFloatType( cfbam::ICFBamFloatTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeFloatType( cfbam::ICFBamFloatTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamFloatTypeObj* createFloatType( cfbam::ICFBamFloatTypeEditObj* Obj );

		virtual cfbam::ICFBamFloatTypeObj* readFloatType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamFloatTypeObj* lockFloatType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readAllFloatType( bool forceRead = false );

		virtual cfbam::ICFBamFloatTypeObj* readFloatTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamFloatTypeObj* readFloatTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readFloatTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamFloatTypeObj* readCachedFloatType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamFloatTypeObj* readCachedFloatTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamFloatTypeObj* readCachedFloatTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamFloatTypeObj*> readCachedFloatTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamFloatTypeObj* updateFloatType( cfbam::ICFBamFloatTypeEditObj* Obj );

		virtual void deleteFloatType( cfbam::ICFBamFloatTypeEditObj* Obj );

		virtual void deleteFloatTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteFloatTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteFloatTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteFloatTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteFloatTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteFloatTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteFloatTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteFloatTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteFloatTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteFloatTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamFloatTypeObj* moveUpFloatType( cfbam::ICFBamFloatTypeObj* Obj );

		virtual cfbam::ICFBamFloatTypeObj* moveDownFloatType( cfbam::ICFBamFloatTypeObj* Obj );

		virtual void reallyDetachFromIndexesFloatType( cfbam::ICFBamFloatTypeObj* Obj );
