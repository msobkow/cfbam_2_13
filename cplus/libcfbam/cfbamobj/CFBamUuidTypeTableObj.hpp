...
namespace cfbam {
	...
	class CFBamUuidTypeTableObj
	: public virtual ICFBamUuidTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>* allUuidType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamUuidTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamUuidTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamUuidTypeTableObj();
		CFBamUuidTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamUuidTypeTableObj();
		...
		virtual cfbam::ICFBamUuidTypeObj* newInstance();

		virtual cfbam::ICFBamUuidTypeEditObj* newEditInstance( cfbam::ICFBamUuidTypeObj* orig );

		virtual cfbam::ICFBamUuidTypeObj* realizeUuidType( cfbam::ICFBamUuidTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeUuidType( cfbam::ICFBamUuidTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamUuidTypeObj* createUuidType( cfbam::ICFBamUuidTypeEditObj* Obj );

		virtual cfbam::ICFBamUuidTypeObj* readUuidType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamUuidTypeObj* lockUuidType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readAllUuidType( bool forceRead = false );

		virtual cfbam::ICFBamUuidTypeObj* readUuidTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamUuidTypeObj* readUuidTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readUuidTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamUuidTypeObj* readCachedUuidType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamUuidTypeObj* readCachedUuidTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamUuidTypeObj* readCachedUuidTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamUuidTypeObj*> readCachedUuidTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamUuidTypeObj* updateUuidType( cfbam::ICFBamUuidTypeEditObj* Obj );

		virtual void deleteUuidType( cfbam::ICFBamUuidTypeEditObj* Obj );

		virtual void deleteUuidTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteUuidTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteUuidTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteUuidTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteUuidTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteUuidTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteUuidTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteUuidTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteUuidTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteUuidTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamUuidTypeObj* moveUpUuidType( cfbam::ICFBamUuidTypeObj* Obj );

		virtual cfbam::ICFBamUuidTypeObj* moveDownUuidType( cfbam::ICFBamUuidTypeObj* Obj );

		virtual void reallyDetachFromIndexesUuidType( cfbam::ICFBamUuidTypeObj* Obj );
