...
namespace cfbam {
	...
	class CFBamTextColTableObj
	: public virtual ICFBamTextColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>* allTextCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTextColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTextColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamTextColTableObj();
		CFBamTextColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTextColTableObj();
		...
		virtual cfbam::ICFBamTextColObj* newInstance();

		virtual cfbam::ICFBamTextColEditObj* newEditInstance( cfbam::ICFBamTextColObj* orig );

		virtual cfbam::ICFBamTextColObj* realizeTextCol( cfbam::ICFBamTextColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTextCol( cfbam::ICFBamTextColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamTextColObj* createTextCol( cfbam::ICFBamTextColEditObj* Obj );

		virtual cfbam::ICFBamTextColObj* readTextCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTextColObj* lockTextCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTextColObj*> readAllTextCol( bool forceRead = false );

		virtual cfbam::ICFBamTextColObj* readTextColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTextColObj* readTextColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextColObj*> readTextColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTextColObj* readCachedTextCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTextColObj* readCachedTextColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTextColObj* readCachedTextColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTextColObj*> readCachedTextColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTextColObj* updateTextCol( cfbam::ICFBamTextColEditObj* Obj );

		virtual void deleteTextCol( cfbam::ICFBamTextColEditObj* Obj );

		virtual void deleteTextColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTextColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTextColByValTentIdx( const int64_t TenantId );

		virtual void deleteTextColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTextColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTextColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTextColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTextColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTextColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTextColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTextColObj* moveUpTextCol( cfbam::ICFBamTextColObj* Obj );

		virtual cfbam::ICFBamTextColObj* moveDownTextCol( cfbam::ICFBamTextColObj* Obj );

		virtual void reallyDetachFromIndexesTextCol( cfbam::ICFBamTextColObj* Obj );
