...
namespace cfbam {
	...
	class CFBamDateDefTableObj
	: public virtual ICFBamDateDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>* allDateDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamDateDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamDateDefTableObj();
		CFBamDateDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamDateDefTableObj();
		...
		virtual cfbam::ICFBamDateDefObj* newInstance();

		virtual cfbam::ICFBamDateDefEditObj* newEditInstance( cfbam::ICFBamDateDefObj* orig );

		virtual cfbam::ICFBamDateDefObj* realizeDateDef( cfbam::ICFBamDateDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeDateDef( cfbam::ICFBamDateDefObj* Obj );
		virtual cfbam::ICFBamDateDefObj* createDateDef( cfbam::ICFBamDateDefEditObj* Obj );

		virtual cfbam::ICFBamDateDefObj* readDateDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamDateDefObj* lockDateDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readAllDateDef( bool forceRead = false );

		virtual cfbam::ICFBamDateDefObj* readDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamDateDefObj* readDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamDateDefObj* readCachedDateDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamDateDefObj* readCachedDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamDateDefObj* readCachedDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateDefObj*> readCachedDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamDateDefObj* updateDateDef( cfbam::ICFBamDateDefEditObj* Obj );

		virtual void deleteDateDef( cfbam::ICFBamDateDefEditObj* Obj );

		virtual void deleteDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteDateDefByValTentIdx( const int64_t TenantId );

		virtual void deleteDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamDateDefObj* moveUpDateDef( cfbam::ICFBamDateDefObj* Obj );

		virtual cfbam::ICFBamDateDefObj* moveDownDateDef( cfbam::ICFBamDateDefObj* Obj );

		virtual void reallyDetachFromIndexesDateDef( cfbam::ICFBamDateDefObj* Obj );
