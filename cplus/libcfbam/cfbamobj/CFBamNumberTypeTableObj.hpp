...
namespace cfbam {
	...
	class CFBamNumberTypeTableObj
	: public virtual ICFBamNumberTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>* allNumberType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamNumberTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamNumberTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamNumberTypeTableObj();
		CFBamNumberTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamNumberTypeTableObj();
		...
		virtual cfbam::ICFBamNumberTypeObj* newInstance();

		virtual cfbam::ICFBamNumberTypeEditObj* newEditInstance( cfbam::ICFBamNumberTypeObj* orig );

		virtual cfbam::ICFBamNumberTypeObj* realizeNumberType( cfbam::ICFBamNumberTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeNumberType( cfbam::ICFBamNumberTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamNumberTypeObj* createNumberType( cfbam::ICFBamNumberTypeEditObj* Obj );

		virtual cfbam::ICFBamNumberTypeObj* readNumberType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamNumberTypeObj* lockNumberType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readAllNumberType( bool forceRead = false );

		virtual cfbam::ICFBamNumberTypeObj* readNumberTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamNumberTypeObj* readNumberTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readNumberTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamNumberTypeObj* readCachedNumberType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamNumberTypeObj* readCachedNumberTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamNumberTypeObj* readCachedNumberTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamNumberTypeObj*> readCachedNumberTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamNumberTypeObj* updateNumberType( cfbam::ICFBamNumberTypeEditObj* Obj );

		virtual void deleteNumberType( cfbam::ICFBamNumberTypeEditObj* Obj );

		virtual void deleteNumberTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteNumberTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteNumberTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteNumberTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteNumberTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteNumberTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteNumberTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteNumberTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteNumberTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteNumberTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamNumberTypeObj* moveUpNumberType( cfbam::ICFBamNumberTypeObj* Obj );

		virtual cfbam::ICFBamNumberTypeObj* moveDownNumberType( cfbam::ICFBamNumberTypeObj* Obj );

		virtual void reallyDetachFromIndexesNumberType( cfbam::ICFBamNumberTypeObj* Obj );
