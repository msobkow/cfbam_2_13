...
namespace cfbam {
	...
	class CFBamDateColTableObj
	: public virtual ICFBamDateColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>* allDateCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamDateColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamDateColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamDateColTableObj();
		CFBamDateColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamDateColTableObj();
		...
		virtual cfbam::ICFBamDateColObj* newInstance();

		virtual cfbam::ICFBamDateColEditObj* newEditInstance( cfbam::ICFBamDateColObj* orig );

		virtual cfbam::ICFBamDateColObj* realizeDateCol( cfbam::ICFBamDateColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeDateCol( cfbam::ICFBamDateColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamDateColObj* createDateCol( cfbam::ICFBamDateColEditObj* Obj );

		virtual cfbam::ICFBamDateColObj* readDateCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamDateColObj* lockDateCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamDateColObj*> readAllDateCol( bool forceRead = false );

		virtual cfbam::ICFBamDateColObj* readDateColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamDateColObj* readDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateColObj*> readDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamDateColObj* readCachedDateCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamDateColObj* readCachedDateColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamDateColObj* readCachedDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDateColObj*> readCachedDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamDateColObj* updateDateCol( cfbam::ICFBamDateColEditObj* Obj );

		virtual void deleteDateCol( cfbam::ICFBamDateColEditObj* Obj );

		virtual void deleteDateColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteDateColByValTentIdx( const int64_t TenantId );

		virtual void deleteDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamDateColObj* moveUpDateCol( cfbam::ICFBamDateColObj* Obj );

		virtual cfbam::ICFBamDateColObj* moveDownDateCol( cfbam::ICFBamDateColObj* Obj );

		virtual void reallyDetachFromIndexesDateCol( cfbam::ICFBamDateColObj* Obj );
