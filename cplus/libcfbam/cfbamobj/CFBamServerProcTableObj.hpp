...
namespace cfbam {
	...
	class CFBamServerProcTableObj
	: public virtual ICFBamServerProcTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamScopePKey, cfbam::ICFBamServerProcObj*>* members;
		std::map<cfbam::CFBamScopePKey, cfbam::ICFBamServerProcObj*>* allServerProc;
		std::map< cfbam::CFBamScopeByTenantIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamServerProcObj*>*>* indexByTenantIdx;
		std::map< cfbam::CFBamServerMethodByUNameIdxKey,
			cfbam::ICFBamServerProcObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamServerMethodByMethTableIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamServerProcObj*>*>* indexByMethTableIdx;
		std::map< cfbam::CFBamServerMethodByDefSchemaIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamServerProcObj*>*>* indexByDefSchemaIdx;
	...
	public:
		...
		CFBamServerProcTableObj();
		CFBamServerProcTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamServerProcTableObj();
		...
		virtual cfbam::ICFBamServerProcObj* newInstance();

		virtual cfbam::ICFBamServerProcEditObj* newEditInstance( cfbam::ICFBamServerProcObj* orig );

		virtual cfbam::ICFBamServerProcObj* realizeServerProc( cfbam::ICFBamServerProcObj* Obj );

		virtual void deepDisposeByIdIdx( const int64_t TenantId,
			const int64_t Id );
		virtual void deepDisposeByTenantIdx( const int64_t TenantId );
		virtual void reallyDeepDisposeServerProc( cfbam::ICFBamServerProcObj* Obj );
		...
		virtual cfbam::ICFBamServerProcObj* createServerProc( cfbam::ICFBamServerProcEditObj* Obj );

		virtual cfbam::ICFBamServerProcObj* readServerProc( cfbam::CFBamScopePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamServerProcObj* lockServerProc( cfbam::CFBamScopePKey* pkey );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readAllServerProc( bool forceRead = false );

		virtual cfbam::ICFBamServerProcObj* readServerProcByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readServerProcByTenantIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual cfbam::ICFBamServerProcObj* readServerProcByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readServerProcByMethTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readServerProcByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual cfbam::ICFBamServerProcObj* readCachedServerProc( cfbam::CFBamScopePKey* pkey );

		virtual cfbam::ICFBamServerProcObj* readCachedServerProcByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readCachedServerProcByTenantIdx( const int64_t TenantId );

		virtual cfbam::ICFBamServerProcObj* readCachedServerProcByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readCachedServerProcByMethTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual std::vector<cfbam::ICFBamServerProcObj*> readCachedServerProcByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual cfbam::ICFBamServerProcObj* updateServerProc( cfbam::ICFBamServerProcEditObj* Obj );

		virtual void deleteServerProc( cfbam::ICFBamServerProcEditObj* Obj );

		virtual void deleteServerProcByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteServerProcByTenantIdx( const int64_t TenantId );

		virtual void deleteServerProcByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name );

		virtual void deleteServerProcByMethTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual void deleteServerProcByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void reallyDetachFromIndexesServerProc( cfbam::ICFBamServerProcObj* Obj );
