...
namespace cfbam {
	...
	class CFBamTokenColTableObj
	: public virtual ICFBamTokenColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>* allTokenCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTokenColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTokenColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamTokenColTableObj();
		CFBamTokenColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTokenColTableObj();
		...
		virtual cfbam::ICFBamTokenColObj* newInstance();

		virtual cfbam::ICFBamTokenColEditObj* newEditInstance( cfbam::ICFBamTokenColObj* orig );

		virtual cfbam::ICFBamTokenColObj* realizeTokenCol( cfbam::ICFBamTokenColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTokenCol( cfbam::ICFBamTokenColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamTokenColObj* createTokenCol( cfbam::ICFBamTokenColEditObj* Obj );

		virtual cfbam::ICFBamTokenColObj* readTokenCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTokenColObj* lockTokenCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readAllTokenCol( bool forceRead = false );

		virtual cfbam::ICFBamTokenColObj* readTokenColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTokenColObj* readTokenColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readTokenColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTokenColObj* readCachedTokenCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTokenColObj* readCachedTokenColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTokenColObj* readCachedTokenColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTokenColObj*> readCachedTokenColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTokenColObj* updateTokenCol( cfbam::ICFBamTokenColEditObj* Obj );

		virtual void deleteTokenCol( cfbam::ICFBamTokenColEditObj* Obj );

		virtual void deleteTokenColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTokenColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTokenColByValTentIdx( const int64_t TenantId );

		virtual void deleteTokenColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTokenColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTokenColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTokenColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTokenColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTokenColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTokenColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTokenColObj* moveUpTokenCol( cfbam::ICFBamTokenColObj* Obj );

		virtual cfbam::ICFBamTokenColObj* moveDownTokenCol( cfbam::ICFBamTokenColObj* Obj );

		virtual void reallyDetachFromIndexesTokenCol( cfbam::ICFBamTokenColObj* Obj );
