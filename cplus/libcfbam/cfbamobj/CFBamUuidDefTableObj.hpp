...
namespace cfbam {
	...
	class CFBamUuidDefTableObj
	: public virtual ICFBamUuidDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>* allUuidDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamUuidDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamUuidDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamUuidDefTableObj();
		CFBamUuidDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamUuidDefTableObj();
		...
		virtual cfbam::ICFBamUuidDefObj* newInstance();

		virtual cfbam::ICFBamUuidDefEditObj* newEditInstance( cfbam::ICFBamUuidDefObj* orig );

		virtual cfbam::ICFBamUuidDefObj* realizeUuidDef( cfbam::ICFBamUuidDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeUuidDef( cfbam::ICFBamUuidDefObj* Obj );
		virtual cfbam::ICFBamUuidDefObj* createUuidDef( cfbam::ICFBamUuidDefEditObj* Obj );

		virtual cfbam::ICFBamUuidDefObj* readUuidDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamUuidDefObj* lockUuidDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readAllUuidDef( bool forceRead = false );

		virtual cfbam::ICFBamUuidDefObj* readUuidDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamUuidDefObj* readUuidDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readUuidDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamUuidDefObj* readCachedUuidDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamUuidDefObj* readCachedUuidDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamUuidDefObj* readCachedUuidDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamUuidDefObj*> readCachedUuidDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamUuidDefObj* updateUuidDef( cfbam::ICFBamUuidDefEditObj* Obj );

		virtual void deleteUuidDef( cfbam::ICFBamUuidDefEditObj* Obj );

		virtual void deleteUuidDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteUuidDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteUuidDefByValTentIdx( const int64_t TenantId );

		virtual void deleteUuidDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteUuidDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteUuidDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteUuidDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteUuidDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteUuidDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamUuidDefObj* moveUpUuidDef( cfbam::ICFBamUuidDefObj* Obj );

		virtual cfbam::ICFBamUuidDefObj* moveDownUuidDef( cfbam::ICFBamUuidDefObj* Obj );

		virtual void reallyDetachFromIndexesUuidDef( cfbam::ICFBamUuidDefObj* Obj );
