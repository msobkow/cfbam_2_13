...
namespace cfbam {
	...
	class CFBamTZTimeColTableObj
	: public virtual ICFBamTZTimeColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>* allTZTimeCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTZTimeColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTZTimeColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamTZTimeColTableObj();
		CFBamTZTimeColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTZTimeColTableObj();
		...
		virtual cfbam::ICFBamTZTimeColObj* newInstance();

		virtual cfbam::ICFBamTZTimeColEditObj* newEditInstance( cfbam::ICFBamTZTimeColObj* orig );

		virtual cfbam::ICFBamTZTimeColObj* realizeTZTimeCol( cfbam::ICFBamTZTimeColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTZTimeCol( cfbam::ICFBamTZTimeColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamTZTimeColObj* createTZTimeCol( cfbam::ICFBamTZTimeColEditObj* Obj );

		virtual cfbam::ICFBamTZTimeColObj* readTZTimeCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTZTimeColObj* lockTZTimeCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readAllTZTimeCol( bool forceRead = false );

		virtual cfbam::ICFBamTZTimeColObj* readTZTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTZTimeColObj* readTZTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readTZTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTZTimeColObj* readCachedTZTimeCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTZTimeColObj* readCachedTZTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTZTimeColObj* readCachedTZTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZTimeColObj*> readCachedTZTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTZTimeColObj* updateTZTimeCol( cfbam::ICFBamTZTimeColEditObj* Obj );

		virtual void deleteTZTimeCol( cfbam::ICFBamTZTimeColEditObj* Obj );

		virtual void deleteTZTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTZTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTZTimeColByValTentIdx( const int64_t TenantId );

		virtual void deleteTZTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTZTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTZTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTZTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTZTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTZTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTZTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTZTimeColObj* moveUpTZTimeCol( cfbam::ICFBamTZTimeColObj* Obj );

		virtual cfbam::ICFBamTZTimeColObj* moveDownTZTimeCol( cfbam::ICFBamTZTimeColObj* Obj );

		virtual void reallyDetachFromIndexesTZTimeCol( cfbam::ICFBamTZTimeColObj* Obj );
