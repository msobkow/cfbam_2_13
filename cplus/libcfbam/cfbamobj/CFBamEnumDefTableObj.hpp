...
namespace cfbam {
	...
	class CFBamEnumDefTableObj
	: public virtual ICFBamEnumDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>* allEnumDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamEnumDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamEnumDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamEnumDefTableObj();
		CFBamEnumDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamEnumDefTableObj();
		...
		virtual cfbam::ICFBamEnumDefObj* newInstance();

		virtual cfbam::ICFBamEnumDefEditObj* newEditInstance( cfbam::ICFBamEnumDefObj* orig );

		virtual cfbam::ICFBamEnumDefObj* realizeEnumDef( cfbam::ICFBamEnumDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeEnumDef( cfbam::ICFBamEnumDefObj* Obj );
		virtual cfbam::ICFBamEnumDefObj* createEnumDef( cfbam::ICFBamEnumDefEditObj* Obj );

		virtual cfbam::ICFBamEnumDefObj* readEnumDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamEnumDefObj* lockEnumDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readAllEnumDef( bool forceRead = false );

		virtual cfbam::ICFBamEnumDefObj* readEnumDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamEnumDefObj* readEnumDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readEnumDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamEnumDefObj* readCachedEnumDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamEnumDefObj* readCachedEnumDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamEnumDefObj* readCachedEnumDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamEnumDefObj*> readCachedEnumDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamEnumDefObj* updateEnumDef( cfbam::ICFBamEnumDefEditObj* Obj );

		virtual void deleteEnumDef( cfbam::ICFBamEnumDefEditObj* Obj );

		virtual void deleteEnumDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteEnumDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteEnumDefByValTentIdx( const int64_t TenantId );

		virtual void deleteEnumDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteEnumDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteEnumDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteEnumDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteEnumDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteEnumDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamEnumDefObj* moveUpEnumDef( cfbam::ICFBamEnumDefObj* Obj );

		virtual cfbam::ICFBamEnumDefObj* moveDownEnumDef( cfbam::ICFBamEnumDefObj* Obj );

		virtual void reallyDetachFromIndexesEnumDef( cfbam::ICFBamEnumDefObj* Obj );
