...
namespace cfbam {
	...
	class CFBamTokenDefTableObj
	: public virtual ICFBamTokenDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>* allTokenDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTokenDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTokenDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamTokenDefTableObj();
		CFBamTokenDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTokenDefTableObj();
		...
		virtual cfbam::ICFBamTokenDefObj* newInstance();

		virtual cfbam::ICFBamTokenDefEditObj* newEditInstance( cfbam::ICFBamTokenDefObj* orig );

		virtual cfbam::ICFBamTokenDefObj* realizeTokenDef( cfbam::ICFBamTokenDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTokenDef( cfbam::ICFBamTokenDefObj* Obj );
		virtual cfbam::ICFBamTokenDefObj* createTokenDef( cfbam::ICFBamTokenDefEditObj* Obj );

		virtual cfbam::ICFBamTokenDefObj* readTokenDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTokenDefObj* lockTokenDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readAllTokenDef( bool forceRead = false );

		virtual cfbam::ICFBamTokenDefObj* readTokenDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTokenDefObj* readTokenDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readTokenDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamTokenDefObj* readCachedTokenDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTokenDefObj* readCachedTokenDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTokenDefObj* readCachedTokenDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTokenDefObj*> readCachedTokenDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTokenDefObj* updateTokenDef( cfbam::ICFBamTokenDefEditObj* Obj );

		virtual void deleteTokenDef( cfbam::ICFBamTokenDefEditObj* Obj );

		virtual void deleteTokenDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTokenDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTokenDefByValTentIdx( const int64_t TenantId );

		virtual void deleteTokenDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTokenDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTokenDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTokenDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTokenDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTokenDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTokenDefObj* moveUpTokenDef( cfbam::ICFBamTokenDefObj* Obj );

		virtual cfbam::ICFBamTokenDefObj* moveDownTokenDef( cfbam::ICFBamTokenDefObj* Obj );

		virtual void reallyDetachFromIndexesTokenDef( cfbam::ICFBamTokenDefObj* Obj );
