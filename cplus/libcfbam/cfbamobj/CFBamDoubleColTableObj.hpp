...
namespace cfbam {
	...
	class CFBamDoubleColTableObj
	: public virtual ICFBamDoubleColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>* allDoubleCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamDoubleColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamDoubleColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDoubleColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamDoubleColTableObj();
		CFBamDoubleColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamDoubleColTableObj();
		...
		virtual cfbam::ICFBamDoubleColObj* newInstance();

		virtual cfbam::ICFBamDoubleColEditObj* newEditInstance( cfbam::ICFBamDoubleColObj* orig );

		virtual cfbam::ICFBamDoubleColObj* realizeDoubleCol( cfbam::ICFBamDoubleColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeDoubleCol( cfbam::ICFBamDoubleColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamDoubleColObj* createDoubleCol( cfbam::ICFBamDoubleColEditObj* Obj );

		virtual cfbam::ICFBamDoubleColObj* readDoubleCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamDoubleColObj* lockDoubleCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readAllDoubleCol( bool forceRead = false );

		virtual cfbam::ICFBamDoubleColObj* readDoubleColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamDoubleColObj* readDoubleColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readDoubleColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamDoubleColObj* readCachedDoubleCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamDoubleColObj* readCachedDoubleColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamDoubleColObj* readCachedDoubleColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDoubleColObj*> readCachedDoubleColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamDoubleColObj* updateDoubleCol( cfbam::ICFBamDoubleColEditObj* Obj );

		virtual void deleteDoubleCol( cfbam::ICFBamDoubleColEditObj* Obj );

		virtual void deleteDoubleColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteDoubleColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteDoubleColByValTentIdx( const int64_t TenantId );

		virtual void deleteDoubleColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteDoubleColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteDoubleColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteDoubleColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteDoubleColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteDoubleColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteDoubleColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamDoubleColObj* moveUpDoubleCol( cfbam::ICFBamDoubleColObj* Obj );

		virtual cfbam::ICFBamDoubleColObj* moveDownDoubleCol( cfbam::ICFBamDoubleColObj* Obj );

		virtual void reallyDetachFromIndexesDoubleCol( cfbam::ICFBamDoubleColObj* Obj );
