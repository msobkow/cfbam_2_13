...
namespace cfbam {
	...
	class CFBamTZDateDefTableObj
	: public virtual ICFBamTZDateDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>* allTZDateDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTZDateDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamTZDateDefTableObj();
		CFBamTZDateDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTZDateDefTableObj();
		...
		virtual cfbam::ICFBamTZDateDefObj* newInstance();

		virtual cfbam::ICFBamTZDateDefEditObj* newEditInstance( cfbam::ICFBamTZDateDefObj* orig );

		virtual cfbam::ICFBamTZDateDefObj* realizeTZDateDef( cfbam::ICFBamTZDateDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTZDateDef( cfbam::ICFBamTZDateDefObj* Obj );
		virtual cfbam::ICFBamTZDateDefObj* createTZDateDef( cfbam::ICFBamTZDateDefEditObj* Obj );

		virtual cfbam::ICFBamTZDateDefObj* readTZDateDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTZDateDefObj* lockTZDateDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readAllTZDateDef( bool forceRead = false );

		virtual cfbam::ICFBamTZDateDefObj* readTZDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTZDateDefObj* readTZDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readTZDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamTZDateDefObj* readCachedTZDateDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTZDateDefObj* readCachedTZDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTZDateDefObj* readCachedTZDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZDateDefObj*> readCachedTZDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTZDateDefObj* updateTZDateDef( cfbam::ICFBamTZDateDefEditObj* Obj );

		virtual void deleteTZDateDef( cfbam::ICFBamTZDateDefEditObj* Obj );

		virtual void deleteTZDateDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTZDateDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTZDateDefByValTentIdx( const int64_t TenantId );

		virtual void deleteTZDateDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTZDateDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTZDateDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTZDateDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTZDateDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTZDateDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTZDateDefObj* moveUpTZDateDef( cfbam::ICFBamTZDateDefObj* Obj );

		virtual cfbam::ICFBamTZDateDefObj* moveDownTZDateDef( cfbam::ICFBamTZDateDefObj* Obj );

		virtual void reallyDetachFromIndexesTZDateDef( cfbam::ICFBamTZDateDefObj* Obj );
