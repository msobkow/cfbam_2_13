...
namespace cfbam {
	...
	class CFBamBlobTypeTableObj
	: public virtual ICFBamBlobTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>* allBlobType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamBlobTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamBlobTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBlobTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamBlobTypeTableObj();
		CFBamBlobTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamBlobTypeTableObj();
		...
		virtual cfbam::ICFBamBlobTypeObj* newInstance();

		virtual cfbam::ICFBamBlobTypeEditObj* newEditInstance( cfbam::ICFBamBlobTypeObj* orig );

		virtual cfbam::ICFBamBlobTypeObj* realizeBlobType( cfbam::ICFBamBlobTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeBlobType( cfbam::ICFBamBlobTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamBlobTypeObj* createBlobType( cfbam::ICFBamBlobTypeEditObj* Obj );

		virtual cfbam::ICFBamBlobTypeObj* readBlobType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamBlobTypeObj* lockBlobType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readAllBlobType( bool forceRead = false );

		virtual cfbam::ICFBamBlobTypeObj* readBlobTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamBlobTypeObj* readBlobTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readBlobTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamBlobTypeObj* readCachedBlobType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamBlobTypeObj* readCachedBlobTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamBlobTypeObj* readCachedBlobTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamBlobTypeObj*> readCachedBlobTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamBlobTypeObj* updateBlobType( cfbam::ICFBamBlobTypeEditObj* Obj );

		virtual void deleteBlobType( cfbam::ICFBamBlobTypeEditObj* Obj );

		virtual void deleteBlobTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteBlobTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteBlobTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteBlobTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteBlobTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteBlobTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteBlobTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteBlobTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteBlobTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteBlobTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamBlobTypeObj* moveUpBlobType( cfbam::ICFBamBlobTypeObj* Obj );

		virtual cfbam::ICFBamBlobTypeObj* moveDownBlobType( cfbam::ICFBamBlobTypeObj* Obj );

		virtual void reallyDetachFromIndexesBlobType( cfbam::ICFBamBlobTypeObj* Obj );
