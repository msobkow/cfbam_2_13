...
namespace cfbam {
	...
	class CFBamTextDefTableObj
	: public virtual ICFBamTextDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>* allTextDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTextDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTextDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamTextDefTableObj();
		CFBamTextDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTextDefTableObj();
		...
		virtual cfbam::ICFBamTextDefObj* newInstance();

		virtual cfbam::ICFBamTextDefEditObj* newEditInstance( cfbam::ICFBamTextDefObj* orig );

		virtual cfbam::ICFBamTextDefObj* realizeTextDef( cfbam::ICFBamTextDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTextDef( cfbam::ICFBamTextDefObj* Obj );
		virtual cfbam::ICFBamTextDefObj* createTextDef( cfbam::ICFBamTextDefEditObj* Obj );

		virtual cfbam::ICFBamTextDefObj* readTextDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTextDefObj* lockTextDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readAllTextDef( bool forceRead = false );

		virtual cfbam::ICFBamTextDefObj* readTextDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTextDefObj* readTextDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readTextDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamTextDefObj* readCachedTextDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTextDefObj* readCachedTextDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTextDefObj* readCachedTextDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTextDefObj*> readCachedTextDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTextDefObj* updateTextDef( cfbam::ICFBamTextDefEditObj* Obj );

		virtual void deleteTextDef( cfbam::ICFBamTextDefEditObj* Obj );

		virtual void deleteTextDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTextDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTextDefByValTentIdx( const int64_t TenantId );

		virtual void deleteTextDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTextDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTextDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTextDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTextDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTextDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTextDefObj* moveUpTextDef( cfbam::ICFBamTextDefObj* Obj );

		virtual cfbam::ICFBamTextDefObj* moveDownTextDef( cfbam::ICFBamTextDefObj* Obj );

		virtual void reallyDetachFromIndexesTextDef( cfbam::ICFBamTextDefObj* Obj );
