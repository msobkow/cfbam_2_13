...
// Description: C++18 Object interface for CFBam EnumDef.
...
namespace cfbam {
	...
	public:
		ICFBamEnumDefObj();
		virtual ~ICFBamEnumDefObj();
		virtual cfbam::ICFBamValueEditObj* beginEdit() = 0;

		virtual cfbam::ICFBamEnumDefEditObj* getEnumDefEdit() = 0;

		inline cfbam::CFBamEnumDefBuff* getEnumDefBuff() {
			return( static_cast<cfbam::CFBamEnumDefBuff*>( getBuff() ) );
		};

		inline cfbam::CFBamEnumDefBuff* getEnumDefEditBuff() {
			return( dynamic_cast<cfbam::CFBamEnumDefBuff*>( getBuff() ) );
		};

		virtual std::vector<cfbam::ICFBamEnumTagObj*> getRequiredComponentsTag( bool forceRead = false ) = 0;
