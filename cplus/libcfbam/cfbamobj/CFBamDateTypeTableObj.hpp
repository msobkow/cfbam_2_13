...
namespace cfbam {
	...
	class CFBamDateTypeTableObj
	: public virtual ICFBamDateTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>* allDateType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamDateTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamDateTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamDateTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamDateTypeTableObj();
		CFBamDateTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamDateTypeTableObj();
		...
		virtual cfbam::ICFBamDateTypeObj* newInstance();

		virtual cfbam::ICFBamDateTypeEditObj* newEditInstance( cfbam::ICFBamDateTypeObj* orig );

		virtual cfbam::ICFBamDateTypeObj* realizeDateType( cfbam::ICFBamDateTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeDateType( cfbam::ICFBamDateTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamDateTypeObj* createDateType( cfbam::ICFBamDateTypeEditObj* Obj );

		virtual cfbam::ICFBamDateTypeObj* readDateType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamDateTypeObj* lockDateType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readAllDateType( bool forceRead = false );

		virtual cfbam::ICFBamDateTypeObj* readDateTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamDateTypeObj* readDateTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readDateTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamDateTypeObj* readCachedDateType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamDateTypeObj* readCachedDateTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamDateTypeObj* readCachedDateTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamDateTypeObj*> readCachedDateTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamDateTypeObj* updateDateType( cfbam::ICFBamDateTypeEditObj* Obj );

		virtual void deleteDateType( cfbam::ICFBamDateTypeEditObj* Obj );

		virtual void deleteDateTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteDateTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteDateTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteDateTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteDateTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteDateTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteDateTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteDateTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteDateTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteDateTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamDateTypeObj* moveUpDateType( cfbam::ICFBamDateTypeObj* Obj );

		virtual cfbam::ICFBamDateTypeObj* moveDownDateType( cfbam::ICFBamDateTypeObj* Obj );

		virtual void reallyDetachFromIndexesDateType( cfbam::ICFBamDateTypeObj* Obj );
