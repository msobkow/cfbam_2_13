...
namespace cfbam {
	...
	class CFBamNumberColTableObj
	: public virtual ICFBamNumberColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>* allNumberCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamNumberColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamNumberColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamNumberColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamNumberColTableObj();
		CFBamNumberColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamNumberColTableObj();
		...
		virtual cfbam::ICFBamNumberColObj* newInstance();

		virtual cfbam::ICFBamNumberColEditObj* newEditInstance( cfbam::ICFBamNumberColObj* orig );

		virtual cfbam::ICFBamNumberColObj* realizeNumberCol( cfbam::ICFBamNumberColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeNumberCol( cfbam::ICFBamNumberColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamNumberColObj* createNumberCol( cfbam::ICFBamNumberColEditObj* Obj );

		virtual cfbam::ICFBamNumberColObj* readNumberCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamNumberColObj* lockNumberCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readAllNumberCol( bool forceRead = false );

		virtual cfbam::ICFBamNumberColObj* readNumberColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamNumberColObj* readNumberColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readNumberColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamNumberColObj* readCachedNumberCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamNumberColObj* readCachedNumberColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamNumberColObj* readCachedNumberColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamNumberColObj*> readCachedNumberColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamNumberColObj* updateNumberCol( cfbam::ICFBamNumberColEditObj* Obj );

		virtual void deleteNumberCol( cfbam::ICFBamNumberColEditObj* Obj );

		virtual void deleteNumberColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteNumberColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteNumberColByValTentIdx( const int64_t TenantId );

		virtual void deleteNumberColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteNumberColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteNumberColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteNumberColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteNumberColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteNumberColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteNumberColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamNumberColObj* moveUpNumberCol( cfbam::ICFBamNumberColObj* Obj );

		virtual cfbam::ICFBamNumberColObj* moveDownNumberCol( cfbam::ICFBamNumberColObj* Obj );

		virtual void reallyDetachFromIndexesNumberCol( cfbam::ICFBamNumberColObj* Obj );
