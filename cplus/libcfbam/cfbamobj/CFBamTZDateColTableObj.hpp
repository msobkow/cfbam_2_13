...
namespace cfbam {
	...
	class CFBamTZDateColTableObj
	: public virtual ICFBamTZDateColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>* allTZDateCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTZDateColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTZDateColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZDateColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamTZDateColTableObj();
		CFBamTZDateColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTZDateColTableObj();
		...
		virtual cfbam::ICFBamTZDateColObj* newInstance();

		virtual cfbam::ICFBamTZDateColEditObj* newEditInstance( cfbam::ICFBamTZDateColObj* orig );

		virtual cfbam::ICFBamTZDateColObj* realizeTZDateCol( cfbam::ICFBamTZDateColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTZDateCol( cfbam::ICFBamTZDateColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamTZDateColObj* createTZDateCol( cfbam::ICFBamTZDateColEditObj* Obj );

		virtual cfbam::ICFBamTZDateColObj* readTZDateCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTZDateColObj* lockTZDateCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readAllTZDateCol( bool forceRead = false );

		virtual cfbam::ICFBamTZDateColObj* readTZDateColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTZDateColObj* readTZDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readTZDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTZDateColObj* readCachedTZDateCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTZDateColObj* readCachedTZDateColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTZDateColObj* readCachedTZDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZDateColObj*> readCachedTZDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTZDateColObj* updateTZDateCol( cfbam::ICFBamTZDateColEditObj* Obj );

		virtual void deleteTZDateCol( cfbam::ICFBamTZDateColEditObj* Obj );

		virtual void deleteTZDateColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTZDateColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTZDateColByValTentIdx( const int64_t TenantId );

		virtual void deleteTZDateColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTZDateColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTZDateColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTZDateColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTZDateColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTZDateColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTZDateColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTZDateColObj* moveUpTZDateCol( cfbam::ICFBamTZDateColObj* Obj );

		virtual cfbam::ICFBamTZDateColObj* moveDownTZDateCol( cfbam::ICFBamTZDateColObj* Obj );

		virtual void reallyDetachFromIndexesTZDateCol( cfbam::ICFBamTZDateColObj* Obj );
