...
namespace cfbam {
	...
	class CFBamBoolTypeTableObj
	: public virtual ICFBamBoolTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>* allBoolType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamBoolTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamBoolTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamBoolTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamBoolTypeTableObj();
		CFBamBoolTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamBoolTypeTableObj();
		...
		virtual cfbam::ICFBamBoolTypeObj* newInstance();

		virtual cfbam::ICFBamBoolTypeEditObj* newEditInstance( cfbam::ICFBamBoolTypeObj* orig );

		virtual cfbam::ICFBamBoolTypeObj* realizeBoolType( cfbam::ICFBamBoolTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeBoolType( cfbam::ICFBamBoolTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamBoolTypeObj* createBoolType( cfbam::ICFBamBoolTypeEditObj* Obj );

		virtual cfbam::ICFBamBoolTypeObj* readBoolType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamBoolTypeObj* lockBoolType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readAllBoolType( bool forceRead = false );

		virtual cfbam::ICFBamBoolTypeObj* readBoolTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamBoolTypeObj* readBoolTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readBoolTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamBoolTypeObj* readCachedBoolType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamBoolTypeObj* readCachedBoolTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamBoolTypeObj* readCachedBoolTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamBoolTypeObj*> readCachedBoolTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamBoolTypeObj* updateBoolType( cfbam::ICFBamBoolTypeEditObj* Obj );

		virtual void deleteBoolType( cfbam::ICFBamBoolTypeEditObj* Obj );

		virtual void deleteBoolTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteBoolTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteBoolTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteBoolTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteBoolTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteBoolTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteBoolTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteBoolTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteBoolTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteBoolTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamBoolTypeObj* moveUpBoolType( cfbam::ICFBamBoolTypeObj* Obj );

		virtual cfbam::ICFBamBoolTypeObj* moveDownBoolType( cfbam::ICFBamBoolTypeObj* Obj );

		virtual void reallyDetachFromIndexesBoolType( cfbam::ICFBamBoolTypeObj* Obj );
