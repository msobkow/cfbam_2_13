...
namespace cfbam {
	...
	class CFBamTableTableObj
	: public virtual ICFBamTableTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>* members;
		std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>* allTable;
		std::map< cfbam::CFBamScopeByTenantIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByTenantIdx;
		std::map< cfbam::CFBamTableBySchemaDefIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexBySchemaDefIdx;
		std::map< cfbam::CFBamTableByDefSchemaIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamTableByUNameIdxKey,
			cfbam::ICFBamTableObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamTableBySchemaCdIdxKey,
			cfbam::ICFBamTableObj*>* indexBySchemaCdIdx;
		std::map< cfbam::CFBamTableByPrimaryIndexIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByPrimaryIndexIdx;
		std::map< cfbam::CFBamTableByLookupIndexIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByLookupIndexIdx;
		std::map< cfbam::CFBamTableByAltIndexIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByAltIndexIdx;
		std::map< cfbam::CFBamTableByQualTableIdxKey,
			std::map<cfbam::CFBamScopePKey, cfbam::ICFBamTableObj*>*>* indexByQualTableIdx;
	...
	public:
		...
		CFBamTableTableObj();
		CFBamTableTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTableTableObj();
		...
		virtual cfbam::ICFBamTableObj* newInstance();

		virtual cfbam::ICFBamTableEditObj* newEditInstance( cfbam::ICFBamTableObj* orig );

		virtual cfbam::ICFBamTableObj* realizeTable( cfbam::ICFBamTableObj* Obj );

		virtual void deepDisposeByIdIdx( const int64_t TenantId,
			const int64_t Id );
		virtual void deepDisposeByTenantIdx( const int64_t TenantId );
		virtual void reallyDeepDisposeTable( cfbam::ICFBamTableObj* Obj );
		virtual void deepDisposeBySchemaDefIdx( const int64_t TenantId,
		...
		virtual void deepDisposeBySchemaCdIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& TableClassCode );
		virtual void deepDisposeByPrimaryIndexIdx( const int64_t* PrimaryIndexTenantId,
			const int64_t* PrimaryIndexId );
		virtual void deepDisposeByLookupIndexIdx( const int64_t* LookupIndexTenantId,
			const int64_t* LookupIndexId );
		virtual void deepDisposeByAltIndexIdx( const int64_t* AltIndexTenantId,
			const int64_t* AltIndexId );
		virtual void deepDisposeByQualTableIdx( const int64_t* QualifyingTenantId,
			const int64_t* QualifyingTableId );
		virtual cfbam::ICFBamTableObj* createTable( cfbam::ICFBamTableEditObj* Obj );

		virtual cfbam::ICFBamTableObj* readTable( cfbam::CFBamScopePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTableObj* lockTable( cfbam::CFBamScopePKey* pkey );

		virtual std::vector<cfbam::ICFBamTableObj*> readAllTable( bool forceRead = false );

		virtual cfbam::ICFBamTableObj* readTableByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByTenantIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableBySchemaDefIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual cfbam::ICFBamTableObj* readTableByUNameIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& Name, bool forceRead = false );

		virtual cfbam::ICFBamTableObj* readTableBySchemaCdIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& TableClassCode, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByPrimaryIndexIdx( const int64_t* PrimaryIndexTenantId,
			const int64_t* PrimaryIndexId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByLookupIndexIdx( const int64_t* LookupIndexTenantId,
			const int64_t* LookupIndexId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByAltIndexIdx( const int64_t* AltIndexTenantId,
			const int64_t* AltIndexId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTableObj*> readTableByQualTableIdx( const int64_t* QualifyingTenantId,
			const int64_t* QualifyingTableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTableObj* readCachedTable( cfbam::CFBamScopePKey* pkey );

		virtual cfbam::ICFBamTableObj* readCachedTableByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByTenantIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableBySchemaDefIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual cfbam::ICFBamTableObj* readCachedTableByUNameIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& Name );

		virtual cfbam::ICFBamTableObj* readCachedTableBySchemaCdIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& TableClassCode );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByPrimaryIndexIdx( const int64_t* PrimaryIndexTenantId,
			const int64_t* PrimaryIndexId );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByLookupIndexIdx( const int64_t* LookupIndexTenantId,
			const int64_t* LookupIndexId );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByAltIndexIdx( const int64_t* AltIndexTenantId,
			const int64_t* AltIndexId );

		virtual std::vector<cfbam::ICFBamTableObj*> readCachedTableByQualTableIdx( const int64_t* QualifyingTenantId,
			const int64_t* QualifyingTableId );

		virtual cfbam::ICFBamTableObj* readTableByLookupUNameIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& Name, bool forceRead = false );

		virtual cfbam::ICFBamTableObj* readCachedTableByLookupUNameIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& Name );

		virtual cfbam::ICFBamTableObj* updateTable( cfbam::ICFBamTableEditObj* Obj );

		virtual void deleteTable( cfbam::ICFBamTableEditObj* Obj );

		virtual void deleteTableByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTableByTenantIdx( const int64_t TenantId );

		virtual void deleteTableBySchemaDefIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual void deleteTableByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTableByUNameIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& Name );

		virtual void deleteTableBySchemaCdIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			const std::string& TableClassCode );

		virtual void deleteTableByPrimaryIndexIdx( const int64_t* PrimaryIndexTenantId,
			const int64_t* PrimaryIndexId );

		virtual void deleteTableByLookupIndexIdx( const int64_t* LookupIndexTenantId,
			const int64_t* LookupIndexId );

		virtual void deleteTableByAltIndexIdx( const int64_t* AltIndexTenantId,
			const int64_t* AltIndexId );

		virtual void deleteTableByQualTableIdx( const int64_t* QualifyingTenantId,
			const int64_t* QualifyingTableId );

		virtual void reallyDetachFromIndexesTable( cfbam::ICFBamTableObj* Obj );
