...
namespace cfbam {
	...
	class CFBamChainTableObj
	: public virtual ICFBamChainTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>* members;
		std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>* allChain;
		std::map< cfbam::CFBamChainByTenantIdxKey,
			std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>*>* indexByTenantIdx;
		std::map< cfbam::CFBamChainByChainTableIdxKey,
			std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>*>* indexByChainTableIdx;
		std::map< cfbam::CFBamChainByDefSchemaIdxKey,
			std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamChainByUNameIdxKey,
			cfbam::ICFBamChainObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamChainByPrevRelIdxKey,
			std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>*>* indexByPrevRelIdx;
		std::map< cfbam::CFBamChainByNextRelIdxKey,
			std::map<cfbam::CFBamChainPKey, cfbam::ICFBamChainObj*>*>* indexByNextRelIdx;
	...
	public:
		...
		CFBamChainTableObj();
		CFBamChainTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamChainTableObj();
		...
		virtual cfbam::ICFBamChainObj* newInstance();

		virtual cfbam::ICFBamChainEditObj* newEditInstance( cfbam::ICFBamChainObj* orig );

		virtual cfbam::ICFBamChainObj* realizeChain( cfbam::ICFBamChainObj* Obj );

		virtual void deepDisposeByIdIdx( const int64_t TenantId,
			const int64_t Id );
		virtual void deepDisposeByTenantIdx( const int64_t TenantId );
		virtual void deepDisposeByChainTableIdx( const int64_t TenantId,
		...
		virtual void deepDisposeByPrevRelIdx( const int64_t PrevRelationTenantId,
			const int64_t PrevRelationId );
		virtual void deepDisposeByNextRelIdx( const int64_t NextRelationTenantId,
			const int64_t NextRelationId );
		virtual void reallyDeepDisposeChain( cfbam::ICFBamChainObj* Obj );
		virtual cfbam::ICFBamChainObj* createChain( cfbam::ICFBamChainEditObj* Obj );

		virtual cfbam::ICFBamChainObj* readChain( cfbam::CFBamChainPKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamChainObj* lockChain( cfbam::CFBamChainPKey* pkey );

		virtual std::vector<cfbam::ICFBamChainObj*> readAllChain( bool forceRead = false );

		virtual cfbam::ICFBamChainObj* readChainByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamChainObj*> readChainByTenantIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamChainObj*> readChainByChainTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamChainObj*> readChainByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual cfbam::ICFBamChainObj* readChainByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamChainObj*> readChainByPrevRelIdx( const int64_t PrevRelationTenantId,
			const int64_t PrevRelationId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamChainObj*> readChainByNextRelIdx( const int64_t NextRelationTenantId,
			const int64_t NextRelationId,
			bool forceRead = false );

		virtual cfbam::ICFBamChainObj* readCachedChain( cfbam::CFBamChainPKey* pkey );

		virtual cfbam::ICFBamChainObj* readCachedChainByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual std::vector<cfbam::ICFBamChainObj*> readCachedChainByTenantIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamChainObj*> readCachedChainByChainTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual std::vector<cfbam::ICFBamChainObj*> readCachedChainByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual cfbam::ICFBamChainObj* readCachedChainByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamChainObj*> readCachedChainByPrevRelIdx( const int64_t PrevRelationTenantId,
			const int64_t PrevRelationId );

		virtual std::vector<cfbam::ICFBamChainObj*> readCachedChainByNextRelIdx( const int64_t NextRelationTenantId,
			const int64_t NextRelationId );

		virtual cfbam::ICFBamChainObj* readChainByLookupUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name, bool forceRead = false );

		virtual cfbam::ICFBamChainObj* readCachedChainByLookupUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name );

		virtual cfbam::ICFBamChainObj* updateChain( cfbam::ICFBamChainEditObj* Obj );

		virtual void deleteChain( cfbam::ICFBamChainEditObj* Obj );

		virtual void deleteChainByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteChainByTenantIdx( const int64_t TenantId );

		virtual void deleteChainByChainTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual void deleteChainByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteChainByUNameIdx( const int64_t TenantId,
			const int64_t TableId,
			const std::string& Name );

		virtual void deleteChainByPrevRelIdx( const int64_t PrevRelationTenantId,
			const int64_t PrevRelationId );

		virtual void deleteChainByNextRelIdx( const int64_t NextRelationTenantId,
			const int64_t NextRelationId );

		virtual void reallyDetachFromIndexesChain( cfbam::ICFBamChainObj* Obj );
