...
namespace cfbam {
	...
	class CFBamTimestampDefTableObj
	: public virtual ICFBamTimestampDefTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>* allTimestampDef;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTimestampDefObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimestampDefObj*>*>* indexByContNextIdx;
	...
	public:
		...
		CFBamTimestampDefTableObj();
		CFBamTimestampDefTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTimestampDefTableObj();
		...
		virtual cfbam::ICFBamTimestampDefObj* newInstance();

		virtual cfbam::ICFBamTimestampDefEditObj* newEditInstance( cfbam::ICFBamTimestampDefObj* orig );

		virtual cfbam::ICFBamTimestampDefObj* realizeTimestampDef( cfbam::ICFBamTimestampDefObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTimestampDef( cfbam::ICFBamTimestampDefObj* Obj );
		virtual cfbam::ICFBamTimestampDefObj* createTimestampDef( cfbam::ICFBamTimestampDefEditObj* Obj );

		virtual cfbam::ICFBamTimestampDefObj* readTimestampDef( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTimestampDefObj* lockTimestampDef( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readAllTimestampDef( bool forceRead = false );

		virtual cfbam::ICFBamTimestampDefObj* readTimestampDefByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTimestampDefObj* readTimestampDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readTimestampDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual cfbam::ICFBamTimestampDefObj* readCachedTimestampDef( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTimestampDefObj* readCachedTimestampDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTimestampDefObj* readCachedTimestampDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimestampDefObj*> readCachedTimestampDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTimestampDefObj* updateTimestampDef( cfbam::ICFBamTimestampDefEditObj* Obj );

		virtual void deleteTimestampDef( cfbam::ICFBamTimestampDefEditObj* Obj );

		virtual void deleteTimestampDefByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTimestampDefByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTimestampDefByValTentIdx( const int64_t TenantId );

		virtual void deleteTimestampDefByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTimestampDefByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTimestampDefByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTimestampDefByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTimestampDefByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTimestampDefByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual cfbam::ICFBamTimestampDefObj* moveUpTimestampDef( cfbam::ICFBamTimestampDefObj* Obj );

		virtual cfbam::ICFBamTimestampDefObj* moveDownTimestampDef( cfbam::ICFBamTimestampDefObj* Obj );

		virtual void reallyDetachFromIndexesTimestampDef( cfbam::ICFBamTimestampDefObj* Obj );
