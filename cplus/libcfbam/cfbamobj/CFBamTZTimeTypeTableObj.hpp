...
namespace cfbam {
	...
	class CFBamTZTimeTypeTableObj
	: public virtual ICFBamTZTimeTypeTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>* allTZTimeType;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTZTimeTypeObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTZTimeTypeBySchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTZTimeTypeObj*>*>* indexBySchemaIdx;
	...
	public:
		...
		CFBamTZTimeTypeTableObj();
		CFBamTZTimeTypeTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTZTimeTypeTableObj();
		...
		virtual cfbam::ICFBamTZTimeTypeObj* newInstance();

		virtual cfbam::ICFBamTZTimeTypeEditObj* newEditInstance( cfbam::ICFBamTZTimeTypeObj* orig );

		virtual cfbam::ICFBamTZTimeTypeObj* realizeTZTimeType( cfbam::ICFBamTZTimeTypeObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTZTimeType( cfbam::ICFBamTZTimeTypeObj* Obj );
		virtual void deepDisposeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );
		virtual cfbam::ICFBamTZTimeTypeObj* createTZTimeType( cfbam::ICFBamTZTimeTypeEditObj* Obj );

		virtual cfbam::ICFBamTZTimeTypeObj* readTZTimeType( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTZTimeTypeObj* lockTZTimeType( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readAllTZTimeType( bool forceRead = false );

		virtual cfbam::ICFBamTZTimeTypeObj* readTZTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTZTimeTypeObj* readTZTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readTZTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId,
			bool forceRead = false );

		virtual cfbam::ICFBamTZTimeTypeObj* readCachedTZTimeType( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTZTimeTypeObj* readCachedTZTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTZTimeTypeObj* readCachedTZTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTZTimeTypeObj*> readCachedTZTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTZTimeTypeObj* updateTZTimeType( cfbam::ICFBamTZTimeTypeEditObj* Obj );

		virtual void deleteTZTimeType( cfbam::ICFBamTZTimeTypeEditObj* Obj );

		virtual void deleteTZTimeTypeByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTZTimeTypeByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTZTimeTypeByValTentIdx( const int64_t TenantId );

		virtual void deleteTZTimeTypeByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTZTimeTypeByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTZTimeTypeByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTZTimeTypeByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTZTimeTypeByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTZTimeTypeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTZTimeTypeBySchemaIdx( const int64_t TenantId,
			const int64_t SchemaDefId );

		virtual cfbam::ICFBamTZTimeTypeObj* moveUpTZTimeType( cfbam::ICFBamTZTimeTypeObj* Obj );

		virtual cfbam::ICFBamTZTimeTypeObj* moveDownTZTimeType( cfbam::ICFBamTZTimeTypeObj* Obj );

		virtual void reallyDetachFromIndexesTZTimeType( cfbam::ICFBamTZTimeTypeObj* Obj );
