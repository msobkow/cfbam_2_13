...
namespace cfbam {
	...
	class CFBamTimeColTableObj
	: public virtual ICFBamTimeColTableObj
	{
	protected:
		cfbam::ICFBamSchemaObj* schema;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>* members;
		std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>* allTimeCol;
		std::map< cfbam::CFBamValueByUNameIdxKey,
			cfbam::ICFBamTimeColObj*>* indexByUNameIdx;
		std::map< cfbam::CFBamValueByValTentIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByValTentIdx;
		std::map< cfbam::CFBamValueByScopeIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByScopeIdx;
		std::map< cfbam::CFBamValueByDefSchemaIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByDefSchemaIdx;
		std::map< cfbam::CFBamValueByPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByPrevIdx;
		std::map< cfbam::CFBamValueByNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByNextIdx;
		std::map< cfbam::CFBamValueByContPrevIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByContPrevIdx;
		std::map< cfbam::CFBamValueByContNextIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByContNextIdx;
		std::map< cfbam::CFBamTimeColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey, cfbam::ICFBamTimeColObj*>*>* indexByTableIdx;
	...
	public:
		...
		CFBamTimeColTableObj();
		CFBamTimeColTableObj( cfbam::ICFBamSchemaObj* argSchema );
		virtual ~CFBamTimeColTableObj();
		...
		virtual cfbam::ICFBamTimeColObj* newInstance();

		virtual cfbam::ICFBamTimeColEditObj* newEditInstance( cfbam::ICFBamTimeColObj* orig );

		virtual cfbam::ICFBamTimeColObj* realizeTimeCol( cfbam::ICFBamTimeColObj* Obj );
		...
		virtual void deepDisposeByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );
		virtual void reallyDeepDisposeTimeCol( cfbam::ICFBamTimeColObj* Obj );
		virtual void deepDisposeByTableIdx( const int64_t TenantId,
			const int64_t TableId );
		virtual cfbam::ICFBamTimeColObj* createTimeCol( cfbam::ICFBamTimeColEditObj* Obj );

		virtual cfbam::ICFBamTimeColObj* readTimeCol( cfbam::CFBamValuePKey* pkey, bool forceRead = false );

		virtual cfbam::ICFBamTimeColObj* lockTimeCol( cfbam::CFBamValuePKey* pkey );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readAllTimeCol( bool forceRead = false );

		virtual cfbam::ICFBamTimeColObj* readTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id, bool forceRead = false );

		virtual cfbam::ICFBamTimeColObj* readTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name, bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByValTentIdx( const int64_t TenantId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId,
			bool forceRead = false );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId,
			bool forceRead = false );

		virtual cfbam::ICFBamTimeColObj* readCachedTimeCol( cfbam::CFBamValuePKey* pkey );

		virtual cfbam::ICFBamTimeColObj* readCachedTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual cfbam::ICFBamTimeColObj* readCachedTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByValTentIdx( const int64_t TenantId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual std::vector<cfbam::ICFBamTimeColObj*> readCachedTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTimeColObj* updateTimeCol( cfbam::ICFBamTimeColEditObj* Obj );

		virtual void deleteTimeCol( cfbam::ICFBamTimeColEditObj* Obj );

		virtual void deleteTimeColByIdIdx( const int64_t TenantId,
			const int64_t Id );

		virtual void deleteTimeColByUNameIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name );

		virtual void deleteTimeColByValTentIdx( const int64_t TenantId );

		virtual void deleteTimeColByScopeIdx( const int64_t TenantId,
			const int64_t ScopeId );

		virtual void deleteTimeColByDefSchemaIdx( const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId );

		virtual void deleteTimeColByPrevIdx( const int64_t* PrevTenantId,
			const int64_t* PrevId );

		virtual void deleteTimeColByNextIdx( const int64_t* NextTenantId,
			const int64_t* NextId );

		virtual void deleteTimeColByContPrevIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId );

		virtual void deleteTimeColByContNextIdx( const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId );

		virtual void deleteTimeColByTableIdx( const int64_t TenantId,
			const int64_t TableId );

		virtual cfbam::ICFBamTimeColObj* moveUpTimeCol( cfbam::ICFBamTimeColObj* Obj );

		virtual cfbam::ICFBamTimeColObj* moveDownTimeCol( cfbam::ICFBamTimeColObj* Obj );

		virtual void reallyDetachFromIndexesTimeCol( cfbam::ICFBamTimeColObj* Obj );
