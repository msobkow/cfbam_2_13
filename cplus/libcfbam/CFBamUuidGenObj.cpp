// Description: C++18 base object instance implementation for CFBam UuidGen.
...
namespace cfbam {
	...
	const std::string CFBamUuidGenObj::CLASS_NAME( "CFBamUuidGenObj" );
	const classcode_t CFBamUuidGenObj::CLASS_CODE = 0xa86eL;

	const int16_t CFBamUuidGenObj::getRequiredSlice() {
		return( getUuidGenBuff()->getRequiredSlice() );
	}

	const int16_t* CFBamUuidGenObj::getRequiredSliceReference() {
		return( getUuidGenBuff()->getRequiredSliceReference() );
	}

	const int32_t CFBamUuidGenObj::getRequiredBlockSize() {
		return( getUuidGenBuff()->getRequiredBlockSize() );
	}

	const int32_t* CFBamUuidGenObj::getRequiredBlockSizeReference() {
		return( getUuidGenBuff()->getRequiredBlockSizeReference() );
	}


	CFBamUuidGenObj::CFBamUuidGenObj()
	: ICFBamUuidGenObj(),
	  CFBamUuidTypeObj()
	{
	}

	CFBamUuidGenObj::CFBamUuidGenObj( cfbam::ICFBamSchemaObj* argSchema )
	: ICFBamUuidGenObj(),
	  CFBamUuidTypeObj( argSchema )
	{
	}
	...
	cfbam::CFBamValueBuff* CFBamUuidGenObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfbam::CFBamUuidGenBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableUuidGen()->readDerivedByIdIdx( dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFBamUuidGenObj::setBuff( cfbam::CFBamValueBuff* value ) {
	...
	const std::string& CFBamUuidGenObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamUuidGenObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFBamUuidGenObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamUuidGenBuff::CLASS_CODE ) {
			return( true );
		}
		else {
			return( CFBamUuidTypeObj::implementsClassCode( value ) );
		}
	}

	std::string CFBamUuidGenObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamUuidGenObj" );
		...
		if( ! CFBamUuidDefObj::isOptionalInitValueNull() ) {
			ret.append( cflib::CFLibXmlUtil::formatRequiredUuid( &S_Space, S_InitValue, CFBamUuidDefObj::getOptionalInitValueValue() ) );
		}
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt64( &S_Space, S_SchemaDefId, CFBamUuidTypeObj::getRequiredSchemaDefId() ) );
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt16( &S_Space, S_Slice, CFBamUuidGenObj::getRequiredSlice() ) );
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt32( &S_Space, S_BlockSize, CFBamUuidGenObj::getRequiredBlockSize() ) );
		ret.append( S_Postamble );
		return( ret );
	}

	std::string CFBamUuidGenObj::getObjName() {
	...
	const std::string CFBamUuidGenObj::getGenDefName() {
		return( cfbam::CFBamUuidGenBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamUuidGenObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidGenObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamUuidGenObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamUuidGenObj::getObjQualifiedName() {
	...
	std::string CFBamUuidGenObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamUuidGenObj::realize() {
		cfbam::ICFBamUuidGenObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->realizeUuidGen(
			(cfbam::ICFBamUuidGenObj*)this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamUuidGenObj::read( bool forceRead ) {
		cfbam::ICFBamUuidGenObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->readUuidGenByIdIdx( getUuidGenBuff()->getRequiredTenantId(),
		getUuidGenBuff()->getRequiredId(), forceRead );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamUuidGenObj::moveUp() {
		cfbam::ICFBamUuidGenObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->moveUpUuidGen( this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamUuidGenObj::moveDown() {
		cfbam::ICFBamUuidGenObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->moveDownUuidGen( this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueEditObj* CFBamUuidGenObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfbam::ICFBamUuidGenObj* lockobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->lockUuidGen( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getUuidGenTableObj()->newEditInstance( this );
		return( dynamic_cast<cfbam::ICFBamValueEditObj*>( edit ) );
	}

	cfbam::ICFBamUuidGenEditObj* CFBamUuidGenObj::getUuidGenEdit() {
		return( dynamic_cast<cfbam::ICFBamUuidGenEditObj*>( edit ) );
	}
