...
namespace cfbam {
	...
	const std::string CFBamSecSessionTableObj::CLASS_NAME( "CFBamSecSessionTableObj" );
	const std::string CFBamSecSessionTableObj::TABLE_NAME( "SecSession" );
	const std::string CFBamSecSessionTableObj::TABLE_DBNAME( "SecSess" );

	CFBamSecSessionTableObj::CFBamSecSessionTableObj() {
		schema = NULL;
	...
	CFBamSecSessionTableObj::CFBamSecSessionTableObj( cfsec::ICFSecSchemaObj* argSchema ) {
	...
	cfsec::ICFSecSchemaObj* CFBamSecSessionTableObj::getSchema() {
		return( schema );
	}

	void CFBamSecSessionTableObj::setSchema( cfsec::ICFSecSchemaObj* value ) {
		schema = dynamic_cast<cfbam::ICFBamSchemaObj*>( value );
	}

	const std::string CFBamSecSessionTableObj::getTableName() {
		return( TABLE_NAME );
	}

	const std::string CFBamSecSessionTableObj::getTableDbName() {
		return( TABLE_DBNAME );
	}
	const classcode_t* CFBamSecSessionTableObj::getObjQualifyingClassCode() {
		return( NULL );
	}


	void CFBamSecSessionTableObj::minimizeMemory() {
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::newInstance() {
		cfsec::ICFSecSecSessionObj* inst = dynamic_cast<cfsec::ICFSecSecSessionObj*>( new CFBamSecSessionObj( schema ) );
		return( inst );
	}

	cfsec::ICFSecSecSessionEditObj* CFBamSecSessionTableObj::newEditInstance( cfsec::ICFSecSecSessionObj* orig ) {
		cfsec::ICFSecSecSessionEditObj* edit = dynamic_cast<cfsec::ICFSecSecSessionEditObj*>( new CFBamSecSessionEditObj( orig ));
		return( edit );
	}

	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::realizeSecSession( cfsec::ICFSecSecSessionObj* Obj ) {
	...
	void CFBamSecSessionTableObj::deepDisposeByIdIdx( const uuid_ptr_t SecSessionId ) {
		...
		while( listIter != listEnd ) {
			...
			if( cur != NULL ) {
				...
				if( classCode == cfsec::CFSecSecSessionBuff::CLASS_CODE ) {
					dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->reallyDeepDisposeSecSession( dynamic_cast<cfsec::ICFSecSecSessionObj*>( cur ) );
				}
			}
			listIter ++;
		}
	...
	void CFBamSecSessionTableObj::deepDisposeBySecUserIdx( const uuid_ptr_t SecUserId ) {
	...
	void CFBamSecSessionTableObj::deepDisposeBySecDevIdx( const uuid_ptr_t SecUserId,
	...
	void CFBamSecSessionTableObj::deepDisposeByStartIdx( const uuid_ptr_t SecUserId,
	...
	void CFBamSecSessionTableObj::deepDisposeByFinishIdx( const uuid_ptr_t SecUserId,
	...
	void CFBamSecSessionTableObj::deepDisposeBySecProxyIdx( const uuid_ptr_t SecProxyId ) {
	...
	void CFBamSecSessionTableObj::reallyDeepDisposeSecSession( cfsec::ICFSecSecSessionObj* Obj ) {
		...
		if( classCode == cfsec::CFSecSecSessionBuff::CLASS_CODE ) {
			dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->reallyDetachFromIndexesSecSession( dynamic_cast<cfsec::ICFSecSecSessionObj*>( obj ) );
		}
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::createSecSession( cfsec::ICFSecSecSessionEditObj* Obj ) {
		...
		try {
			...
			cflib::ICFLibCloneableObj* cloneable = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->createSecSession(
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readSecSession( cfsec::CFSecSecSessionPKey* pkey, bool forceRead ) {
		static const std::string S_Obj( "obj" );
		...
		if( forceRead || ( obj == NULL ) ) {
			cfsec::CFSecSecSessionBuff* readBuff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedByIdIdx( schema->getAuthorization(),
			pkey->getRequiredSecSessionId() );
			if( readBuff != NULL ) {
			obj = dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->newInstance();
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::lockSecSession( cfsec::CFSecSecSessionPKey* pkey ) {
		static const std::string S_ProcName( "lockSecSession" );
		cfsec::ICFSecSecSessionObj* locked = NULL;
		cfsec::CFSecSecSessionBuff* lockBuff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->lockDerived( schema->getAuthorization(), pkey );
		if( lockBuff != NULL ) {
			locked = dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->newInstance();
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readAllSecSession( bool forceRead ) {
		...
		if( forceRead || ( allSecSession == NULL ) ) {
			...
			allSecSession = map;
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readAllDerived( schema->getAuthorization() );
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::pageAllSecSession(const uuid_ptr_t priorSecSessionId )
	{
		static const std::string S_Realized( "realized" );
		...
		try {
			...
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->pageAllBuff( schema->getAuthorization(),
				priorSecSessionId );
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readSecSessionByIdIdx( const uuid_ptr_t SecSessionId, bool forceRead )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readSecSessionBySecUserIdx( const uuid_ptr_t SecUserId,
		bool forceRead )
	{
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedBySecUserIdx( schema->getAuthorization(),
				SecUserId );
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readSecSessionBySecDevIdx( const uuid_ptr_t SecUserId,
			const std::string* SecDevName,
		bool forceRead )
	{
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfsec::CFSecSecSessionPKey, cfsec::ICFSecSecSessionObj*>();
			cfsec::ICFSecSecSessionObj* obj;
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedBySecDevIdx( schema->getAuthorization(),
				SecUserId,
				SecDevName );
			cfsec::CFSecSecSessionBuff* buff;
			for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readSecSessionByStartIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point& Start, bool forceRead )
	{
		...
		if( forceRead || ( obj == NULL ) ) {
			cfsec::CFSecSecSessionBuff* buff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedByStartIdx( schema->getAuthorization(),
				SecUserId,
				Start );
			if( buff != NULL ) {
				obj = dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->newInstance();
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readSecSessionByFinishIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point* Finish,
		bool forceRead )
	{
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfsec::CFSecSecSessionPKey, cfsec::ICFSecSecSessionObj*>();
			cfsec::ICFSecSecSessionObj* obj;
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedByFinishIdx( schema->getAuthorization(),
				SecUserId,
				Finish );
			cfsec::CFSecSecSessionBuff* buff;
			for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readSecSessionBySecProxyIdx( const uuid_ptr_t SecProxyId,
		bool forceRead )
	{
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfsec::CFSecSecSessionPKey, cfsec::ICFSecSecSessionObj*>();
			cfsec::ICFSecSecSessionObj* obj;
			std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->readDerivedBySecProxyIdx( schema->getAuthorization(),
				SecProxyId );
			cfsec::CFSecSecSessionBuff* buff;
			for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
				buff = buffList[ idx ];
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readCachedSecSession( cfsec::CFSecSecSessionPKey* pkey ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readSecSession" );
		cfsec::ICFSecSecSessionObj* obj = NULL;
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readCachedSecSessionByIdIdx( const uuid_ptr_t SecSessionId )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readCachedSecSessionBySecUserIdx( const uuid_ptr_t SecUserId )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readCachedSecSessionBySecDevIdx( const uuid_ptr_t SecUserId,
			const std::string* SecDevName )
	{
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::readCachedSecSessionByStartIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point& Start )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readCachedSecSessionByFinishIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point* Finish )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::readCachedSecSessionBySecProxyIdx( const uuid_ptr_t SecProxyId )
	{
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::pageSecSessionBySecUserIdx( const uuid_ptr_t SecUserId,
			const uuid_ptr_t priorSecSessionId )
	{
		...
		key.setRequiredSecUserId( SecUserId );
		cfsec::ICFSecSecSessionObj* obj;
		std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->pageBuffBySecUserIdx( schema->getAuthorization(),
				SecUserId,
				priorSecSessionId );
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::pageSecSessionBySecDevIdx( const uuid_ptr_t SecUserId,
			const std::string* SecDevName,
			const uuid_ptr_t priorSecSessionId )
	{
		...
		cfsec::ICFSecSecSessionObj* obj;
		std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->pageBuffBySecDevIdx( schema->getAuthorization(),
				SecUserId,
				SecDevName,
				priorSecSessionId );
		std::vector<cfsec::ICFSecSecSessionObj*> retList;
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::pageSecSessionByFinishIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point* Finish,
			const uuid_ptr_t priorSecSessionId )
	{
		...
		cfsec::ICFSecSecSessionObj* obj;
		std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->pageBuffByFinishIdx( schema->getAuthorization(),
				SecUserId,
				Finish,
				priorSecSessionId );
		std::vector<cfsec::ICFSecSecSessionObj*> retList;
	...
	std::vector<cfsec::ICFSecSecSessionObj*> CFBamSecSessionTableObj::pageSecSessionBySecProxyIdx( const uuid_ptr_t SecProxyId,
			const uuid_ptr_t priorSecSessionId )
	{
		...
		cfsec::ICFSecSecSessionObj* obj;
		std::TCFLibOwningVector<cfsec::CFSecSecSessionBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->pageBuffBySecProxyIdx( schema->getAuthorization(),
				SecProxyId,
				priorSecSessionId );
		std::vector<cfsec::ICFSecSecSessionObj*> retList;
		retList.reserve( buffList.size() );
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::updateSecSession( cfsec::ICFSecSecSessionEditObj* Obj ) {
		...
		cfsec::ICFSecSecSessionObj* obj = dynamic_cast<cfsec::ICFSecSecSessionObj*>( Obj->getOrig() );
		try {
			cfsec::CFSecSecSessionBuff* updated = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->updateSecSession( schema->getAuthorization(),
			...
			obj = dynamic_cast<cfsec::ICFSecSecSessionObj*>( dynamic_cast<cfbam::CFBamSecSessionTableObj*>( schema->getSecSessionTableObj() )->newInstance() );
			obj->setBuff( updated );
			obj = dynamic_cast<cfsec::ICFSecSecSessionObj*>( obj->realize() );
			if( obj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
	...
	void CFBamSecSessionTableObj::deleteSecSession( cfsec::ICFSecSecSessionEditObj* Obj ) {
		cfsec::ICFSecSecSessionObj* obj = Obj;
		dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->deleteSecSession( schema->getAuthorization(),
			obj->getSecSessionBuff() );
		deepDisposeByIdIdx( obj->getRequiredSecSessionId() );
	}

	void CFBamSecSessionTableObj::deleteSecSessionByIdIdx( const uuid_ptr_t SecSessionId )
	{
		cfsec::CFSecSecSessionPKey pkey;
		pkey.setRequiredSecSessionId( SecSessionId );
	...
	void CFBamSecSessionTableObj::deleteSecSessionBySecUserIdx( const uuid_ptr_t SecUserId )
	{
	...
	void CFBamSecSessionTableObj::deleteSecSessionBySecDevIdx( const uuid_ptr_t SecUserId,
			const std::string* SecDevName )
	{
	...
	void CFBamSecSessionTableObj::deleteSecSessionByStartIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point& Start )
	{
		if( indexByStartIdx == NULL ) {
			indexByStartIdx = new std::map< cfsec::CFSecSecSessionByStartIdxKey,
		...
		auto searchIndexByStartIdx = indexByStartIdx->find( key );
		if( searchIndexByStartIdx != indexByStartIdx->end() ) {
			dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecSession()->deleteSecSessionByStartIdx( schema->getAuthorization(),
				SecUserId,
				Start );
		}
		else {
	...
	void CFBamSecSessionTableObj::deleteSecSessionByFinishIdx( const uuid_ptr_t SecUserId,
			const std::chrono::system_clock::time_point* Finish )
	{
	...
	void CFBamSecSessionTableObj::deleteSecSessionBySecProxyIdx( const uuid_ptr_t SecProxyId )
	{
	...
	void CFBamSecSessionTableObj::reallyDetachFromIndexesSecSession( cfsec::ICFSecSecSessionObj* Obj ) {
	...
	cfsec::ICFSecSecSessionObj* CFBamSecSessionTableObj::getSystemSession() {
