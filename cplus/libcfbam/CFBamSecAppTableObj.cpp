...
namespace cfbam {
	...
	const std::string CFBamSecAppTableObj::CLASS_NAME( "CFBamSecAppTableObj" );
	const std::string CFBamSecAppTableObj::TABLE_NAME( "SecApp" );
	const std::string CFBamSecAppTableObj::TABLE_DBNAME( "SecApp" );

	CFBamSecAppTableObj::CFBamSecAppTableObj() {
		schema = NULL;
	...
	CFBamSecAppTableObj::CFBamSecAppTableObj( cfsec::ICFSecSchemaObj* argSchema ) {
	...
	cfsec::ICFSecSchemaObj* CFBamSecAppTableObj::getSchema() {
		return( schema );
	}

	void CFBamSecAppTableObj::setSchema( cfsec::ICFSecSchemaObj* value ) {
		schema = dynamic_cast<cfbam::ICFBamSchemaObj*>( value );
	}

	const std::string CFBamSecAppTableObj::getTableName() {
		return( TABLE_NAME );
	}

	const std::string CFBamSecAppTableObj::getTableDbName() {
		return( TABLE_DBNAME );
	}
	const classcode_t* CFBamSecAppTableObj::getObjQualifyingClassCode() {
		return( NULL );
	}


	void CFBamSecAppTableObj::minimizeMemory() {
		if( allSecApp != NULL ) {
			allSecApp->clear();
			delete allSecApp;
			allSecApp = NULL;
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::newInstance() {
		cfsec::ICFSecSecAppObj* inst = dynamic_cast<cfsec::ICFSecSecAppObj*>( new CFBamSecAppObj( schema ) );
		return( inst );
	}

	cfsec::ICFSecSecAppEditObj* CFBamSecAppTableObj::newEditInstance( cfsec::ICFSecSecAppObj* orig ) {
		cfsec::ICFSecSecAppEditObj* edit = dynamic_cast<cfsec::ICFSecSecAppEditObj*>( new CFBamSecAppEditObj( orig ));
		return( edit );
	}

	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::realizeSecApp( cfsec::ICFSecSecAppObj* Obj ) {
	...
	void CFBamSecAppTableObj::deepDisposeByIdIdx( const int64_t ClusterId,
		...
		while( listIter != listEnd ) {
			...
			if( cur != NULL ) {
				...
				if( classCode == cfsec::CFSecSecAppBuff::CLASS_CODE ) {
					dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->reallyDeepDisposeSecApp( dynamic_cast<cfsec::ICFSecSecAppObj*>( cur ) );
				}
			}
			listIter ++;
		}
	...
	void CFBamSecAppTableObj::deepDisposeByClusterIdx( const int64_t ClusterId ) {
	...
	void CFBamSecAppTableObj::deepDisposeByUJEEMountIdx( const int64_t ClusterId,
	...
	void CFBamSecAppTableObj::reallyDeepDisposeSecApp( cfsec::ICFSecSecAppObj* Obj ) {
		...
		dynamic_cast<cfbam::CFBamSecFormTableObj*>( schema->getSecFormTableObj() )->deepDisposeBySecAppIdx( obj->getRequiredClusterId(),
			obj->getRequiredSecAppId() );
		classcode_t classCode = obj->getClassCode();
		if( classCode == cfsec::CFSecSecAppBuff::CLASS_CODE ) {
			dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->reallyDetachFromIndexesSecApp( dynamic_cast<cfsec::ICFSecSecAppObj*>( obj ) );
		}
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::createSecApp( cfsec::ICFSecSecAppEditObj* Obj ) {
		...
		try {
			...
			cflib::ICFLibCloneableObj* cloneable = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->createSecApp(
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readSecApp( cfsec::CFSecSecAppPKey* pkey, bool forceRead ) {
		static const std::string S_Obj( "obj" );
		...
		if( forceRead || ( obj == NULL ) ) {
			cfsec::CFSecSecAppBuff* readBuff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->readDerivedByIdIdx( schema->getAuthorization(),
			pkey->getRequiredClusterId(),
			pkey->getRequiredSecAppId() );
			if( readBuff != NULL ) {
			obj = dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->newInstance();
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::lockSecApp( cfsec::CFSecSecAppPKey* pkey ) {
		static const std::string S_ProcName( "lockSecApp" );
		cfsec::ICFSecSecAppObj* locked = NULL;
		cfsec::CFSecSecAppBuff* lockBuff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->lockDerived( schema->getAuthorization(), pkey );
		if( lockBuff != NULL ) {
			locked = dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->newInstance();
	...
	std::vector<cfsec::ICFSecSecAppObj*> CFBamSecAppTableObj::readAllSecApp( bool forceRead ) {
		...
		if( forceRead || ( allSecApp == NULL ) ) {
			...
			allSecApp = map;
			std::TCFLibOwningVector<cfsec::CFSecSecAppBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->readAllDerived( schema->getAuthorization() );
	...
	std::vector<cfsec::ICFSecSecAppObj*> CFBamSecAppTableObj::pageAllSecApp(const int64_t* priorClusterId,
			const int32_t* priorSecAppId )
	{
		...
		try {
			...
			std::TCFLibOwningVector<cfsec::CFSecSecAppBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->pageAllBuff( schema->getAuthorization(),
				priorClusterId,
				priorSecAppId );
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readSecAppByIdIdx( const int64_t ClusterId,
	...
	std::vector<cfsec::ICFSecSecAppObj*> CFBamSecAppTableObj::readSecAppByClusterIdx( const int64_t ClusterId,
		bool forceRead )
	{
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			std::TCFLibOwningVector<cfsec::CFSecSecAppBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->readDerivedByClusterIdx( schema->getAuthorization(),
				ClusterId );
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readSecAppByUJEEMountIdx( const int64_t ClusterId,
			const std::string& JEEMountName, bool forceRead )
	{
		...
		if( forceRead || ( obj == NULL ) ) {
			cfsec::CFSecSecAppBuff* buff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->readDerivedByUJEEMountIdx( schema->getAuthorization(),
				ClusterId,
				JEEMountName );
			if( buff != NULL ) {
				obj = dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->newInstance();
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readSecAppByLookupUJEEMountIdx( const int64_t ClusterId,
			const std::string& JEEMountName, bool forceRead )
	{
		static const std::string S_Realized( "realized" );
		static const std::string S_Obj( "obj" );
		...
		if( forceRead || ( obj == NULL ) ) {
			cfsec::CFSecSecAppBuff* buff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->readDerivedByLookupUJEEMountIdx( schema->getAuthorization(),
				ClusterId,
				JEEMountName );
			if( buff != NULL ) {
				obj = dynamic_cast<cfsec::ICFSecSecAppObj*>( dynamic_cast<cfbam::CFBamSecAppTableObj*>( schema->getSecAppTableObj() )->newInstance() );
				obj->setBuff( buff );
				realized = dynamic_cast<cfsec::ICFSecSecAppObj*>( obj->realize() );
				if( realized == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readCachedSecApp( cfsec::CFSecSecAppPKey* pkey ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readSecApp" );
		cfsec::ICFSecSecAppObj* obj = NULL;
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readCachedSecAppByIdIdx( const int64_t ClusterId,
			const int32_t SecAppId )
	{
	...
	std::vector<cfsec::ICFSecSecAppObj*> CFBamSecAppTableObj::readCachedSecAppByClusterIdx( const int64_t ClusterId )
	{
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readCachedSecAppByUJEEMountIdx( const int64_t ClusterId,
			const std::string& JEEMountName )
	{
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::readCachedSecAppByLookupUJEEMountIdx( const int64_t ClusterId,
			const std::string& JEEMountName )
	{
		static const std::string S_Realized( "realized" );
		static const std::string S_Obj( "obj" );
	...
	std::vector<cfsec::ICFSecSecAppObj*> CFBamSecAppTableObj::pageSecAppByClusterIdx( const int64_t ClusterId,
			...
			const int32_t* priorSecAppId )
	{
		...
		key.setRequiredClusterId( ClusterId );
		cfsec::ICFSecSecAppObj* obj;
		std::TCFLibOwningVector<cfsec::CFSecSecAppBuff*> buffList = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->pageBuffByClusterIdx( schema->getAuthorization(),
				ClusterId,
				priorClusterId,
				priorSecAppId );
	...
	cfsec::ICFSecSecAppObj* CFBamSecAppTableObj::updateSecApp( cfsec::ICFSecSecAppEditObj* Obj ) {
		...
		cfsec::ICFSecSecAppObj* obj = dynamic_cast<cfsec::ICFSecSecAppObj*>( Obj->getOrig() );
		try {
			cfsec::CFSecSecAppBuff* updated = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->updateSecApp( schema->getAuthorization(),
	...
	void CFBamSecAppTableObj::deleteSecApp( cfsec::ICFSecSecAppEditObj* Obj ) {
		cfsec::ICFSecSecAppObj* obj = Obj;
		dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->deleteSecApp( schema->getAuthorization(),
			obj->getSecAppBuff() );
		deepDisposeByIdIdx( obj->getRequiredClusterId(),
			obj->getRequiredSecAppId() );
	}

	void CFBamSecAppTableObj::deleteSecAppByIdIdx( const int64_t ClusterId,
			const int32_t SecAppId )
	{
		cfsec::CFSecSecAppPKey pkey;
		pkey.setRequiredClusterId( ClusterId );
	...
	void CFBamSecAppTableObj::deleteSecAppByClusterIdx( const int64_t ClusterId )
	{
	...
	void CFBamSecAppTableObj::deleteSecAppByUJEEMountIdx( const int64_t ClusterId,
			const std::string& JEEMountName )
	{
		if( indexByUJEEMountIdx == NULL ) {
			indexByUJEEMountIdx = new std::map< cfsec::CFSecSecAppByUJEEMountIdxKey,
		...
		auto searchIndexByUJEEMountIdx = indexByUJEEMountIdx->find( key );
		if( searchIndexByUJEEMountIdx != indexByUJEEMountIdx->end() ) {
			dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableSecApp()->deleteSecAppByUJEEMountIdx( schema->getAuthorization(),
				ClusterId,
				JEEMountName );
		}
		else {
	...
	void CFBamSecAppTableObj::reallyDetachFromIndexesSecApp( cfsec::ICFSecSecAppObj* Obj ) {
