// Description: C++18 base object instance implementation for CFBam TextType.
...
namespace cfbam {
	...
	const std::string CFBamTextTypeObj::CLASS_NAME( "CFBamTextTypeObj" );
	const classcode_t CFBamTextTypeObj::CLASS_CODE = 0xa842L;

	const int64_t CFBamTextTypeObj::getRequiredSchemaDefId() {
		return( getTextTypeBuff()->getRequiredSchemaDefId() );
	}

	const int64_t* CFBamTextTypeObj::getRequiredSchemaDefIdReference() {
		return( getTextTypeBuff()->getRequiredSchemaDefIdReference() );
	}


	cfbam::ICFBamSchemaDefObj* CFBamTextTypeObj::getRequiredContainerSchemaDef( bool forceRead ) {
	...
	CFBamTextTypeObj::CFBamTextTypeObj()
	: ICFBamTextTypeObj(),
	  CFBamTextDefObj()
	{
	}

	CFBamTextTypeObj::CFBamTextTypeObj( cfbam::ICFBamSchemaObj* argSchema )
	: ICFBamTextTypeObj(),
	  CFBamTextDefObj( argSchema )
	{
	}
	...
	cfbam::CFBamValueBuff* CFBamTextTypeObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfbam::CFBamTextTypeBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFBamSchema*>( schema->getBackingStore() )->getTableTextType()->readDerivedByIdIdx( dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFBamTextTypeObj::setBuff( cfbam::CFBamValueBuff* value ) {
	...
	const std::string& CFBamTextTypeObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFBamTextTypeObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFBamTextTypeObj::implementsClassCode( const classcode_t value ) const {
		if( value == cfbam::CFBamTextTypeBuff::CLASS_CODE ) {
			return( true );
		}
		else {
			return( CFBamTextDefObj::implementsClassCode( value ) );
		}
	}

	std::string CFBamTextTypeObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFBamTextTypeObj" );
		...
		ret.append( cflib::CFLibXmlUtil::formatRequiredInt64( &S_Space, S_SchemaDefId, CFBamTextTypeObj::getRequiredSchemaDefId() ) );
		ret.append( S_Postamble );
		return( ret );
	}

	std::string CFBamTextTypeObj::getObjName() {
	...
	const std::string CFBamTextTypeObj::getGenDefName() {
		return( cfbam::CFBamTextTypeBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFBamTextTypeObj::getScope() {
		cfbam::ICFBamSchemaDefObj* scope = getRequiredContainerSchemaDef();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamTextTypeObj::getObjScope() {
		cfbam::ICFBamSchemaDefObj* scope = getRequiredContainerSchemaDef();
		return( scope );
	}

	cflib::ICFLibAnyObj* CFBamTextTypeObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFBamTextTypeObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFBamTextTypeObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFBamTextTypeObj::getObjQualifiedName() {
	...
	std::string CFBamTextTypeObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfbam::ICFBamValueObj* CFBamTextTypeObj::realize() {
		cfbam::ICFBamTextTypeObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->realizeTextType(
			(cfbam::ICFBamTextTypeObj*)this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamTextTypeObj::read( bool forceRead ) {
		cfbam::ICFBamTextTypeObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->readTextTypeByIdIdx( getTextTypeBuff()->getRequiredTenantId(),
		getTextTypeBuff()->getRequiredId(), forceRead );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamTextTypeObj::moveUp() {
		cfbam::ICFBamTextTypeObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->moveUpTextType( this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueObj* CFBamTextTypeObj::moveDown() {
		cfbam::ICFBamTextTypeObj* retobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->moveDownTextType( this );
		return( dynamic_cast<cfbam::ICFBamValueObj*>( retobj ) );
	}

	cfbam::ICFBamValueEditObj* CFBamTextTypeObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfbam::ICFBamTextTypeObj* lockobj = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->lockTextType( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFBamSchemaObj*>( getSchema() )->getTextTypeTableObj()->newEditInstance( this );
		return( dynamic_cast<cfbam::ICFBamValueEditObj*>( edit ) );
	}

	cfbam::ICFBamTextTypeEditObj* CFBamTextTypeObj::getTextTypeEdit() {
		return( dynamic_cast<cfbam::ICFBamTextTypeEditObj*>( edit ) );
	}
