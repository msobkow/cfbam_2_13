...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CSharpTableInterface
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCSharpTableInterfaceHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCSharpTableInterfaceHandler::CFBamSaxLoaderTableCSharpTableInterfaceHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCSharpTableInterfaceHandler::~CFBamSaxLoaderTableCSharpTableInterfaceHandler() {
	}

	void CFBamSaxLoaderTableCSharpTableInterfaceHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CSharpTableInterface" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCSharpTableInterfaceHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CSharpTableInterface more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCsTableInterfaceNull() || origBuff->getOptionalCsTableInterfaceValue().empty() ) {
			editBuff->setOptionalCsTableInterfaceValue( attrText );
		}
		else {
