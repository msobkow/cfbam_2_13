...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeTableImplementation
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeTableImplementationHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeTableImplementationHandler::CFBamSaxLoaderTableCafeTableImplementationHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeTableImplementationHandler::~CFBamSaxLoaderTableCafeTableImplementationHandler() {
	}

	void CFBamSaxLoaderTableCafeTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeTableImplementation" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeTableImplementation more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJTableImplementationNull() || origBuff->getOptionalJTableImplementationValue().empty() ) {
			editBuff->setOptionalJTableImplementationValue( attrText );
		}
		else {
