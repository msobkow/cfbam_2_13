...
// Description: C++18 XML SAX2 Element Handler implementation for NmTokenType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderNmTokenTypeHandler::CLASS_NAME( "CFBamSaxLoaderNmTokenTypeHandler" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderNmTokenTypeHandler::S_1( "1" );

	CFBamSaxLoaderNmTokenTypeHandler::CFBamSaxLoaderNmTokenTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderNmTokenTypeHandler::~CFBamSaxLoaderNmTokenTypeHandler() {
	}

	void CFBamSaxLoaderNmTokenTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamNmTokenTypeObj* tmpBuff = NULL;
		cfbam::ICFBamNmTokenTypeEditObj* editBuff = NULL;
		cfbam::ICFBamNmTokenTypeEditObj* editNmTokenType = NULL;
		cfbam::ICFBamNmTokenTypeObj* origNmTokenType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamNmTokenTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamNmTokenTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getNmTokenTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamNmTokenTypeEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getNmTokenTypeLoaderBehaviour();
			origNmTokenType = dynamic_cast<cfbam::ICFBamNmTokenTypeObj*>( schemaObj->getNmTokenTypeTableObj()->readNmTokenTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origNmTokenType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editNmTokenType = dynamic_cast<cfbam::ICFBamNmTokenTypeEditObj*>( origNmTokenType->beginEdit() );
						editNmTokenType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editNmTokenType->setOptionalShortNameNull();
						}
						else {
							editNmTokenType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editNmTokenType->setOptionalLabelNull();
						}
						else {
							editNmTokenType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editNmTokenType->setOptionalShortDescriptionNull();
						}
						else {
							editNmTokenType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editNmTokenType->setOptionalDescriptionNull();
						}
						else {
							editNmTokenType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editNmTokenType->setOptionalDefaultXmlValueNull();
						}
						else {
							editNmTokenType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editNmTokenType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editNmTokenType->setOptionalGenerateIdNull();
						}
						else {
							editNmTokenType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editNmTokenType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editNmTokenType->setOptionalDbNameNull();
						}
						else {
							editNmTokenType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editNmTokenType->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editNmTokenType->setOptionalInitValueNull();
						}
						else {
							editNmTokenType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editNmTokenType != NULL ) {
				if( origNmTokenType != NULL ) {
					...
					origNmTokenType = dynamic_cast<cfbam::ICFBamNmTokenTypeObj*>( editNmTokenType->create() );
				}
				editNmTokenType = NULL;
			}
			...
			curContext->setObj( origNmTokenType );
...
cleanup:
	...
	void CFBamSaxLoaderNmTokenTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
