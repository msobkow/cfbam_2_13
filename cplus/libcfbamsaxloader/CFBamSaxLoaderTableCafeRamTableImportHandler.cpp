...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeRamTableImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeRamTableImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeRamTableImportHandler::CFBamSaxLoaderTableCafeRamTableImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeRamTableImportHandler::~CFBamSaxLoaderTableCafeRamTableImportHandler() {
	}

	void CFBamSaxLoaderTableCafeRamTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeRamTableImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeRamTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeRamTableImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJRamTableImportNull() || origBuff->getOptionalJRamTableImportValue().empty() ) {
			editBuff->setOptionalJRamTableImportValue( attrText );
		}
		else {
