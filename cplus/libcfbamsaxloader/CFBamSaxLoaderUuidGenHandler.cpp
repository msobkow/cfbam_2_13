...
// Description: C++18 XML SAX2 Element Handler implementation for UuidGen
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderUuidGenHandler::CLASS_NAME( "CFBamSaxLoaderUuidGenHandler" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_True( "true" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_False( "false" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_No( "no" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_0( "0" );
	const std::string CFBamSaxLoaderUuidGenHandler::S_1( "1" );

	CFBamSaxLoaderUuidGenHandler::CFBamSaxLoaderUuidGenHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderUuidGenHandler::~CFBamSaxLoaderUuidGenHandler() {
	}

	void CFBamSaxLoaderUuidGenHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamUuidGenObj* tmpBuff = NULL;
		cfbam::ICFBamUuidGenEditObj* editBuff = NULL;
		cfbam::ICFBamUuidGenEditObj* editUuidGen = NULL;
		cfbam::ICFBamUuidGenObj* origUuidGen = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamUuidGenBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamUuidGenBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getUuidGenTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamUuidGenEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getUuidGenLoaderBehaviour();
			origUuidGen = dynamic_cast<cfbam::ICFBamUuidGenObj*>( schemaObj->getUuidGenTableObj()->readUuidGenByUNameIdx( refTenant->getRequiredId(),
			...
			if( origUuidGen == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editUuidGen = dynamic_cast<cfbam::ICFBamUuidGenEditObj*>( origUuidGen->beginEdit() );
						editUuidGen->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editUuidGen->setOptionalShortNameNull();
						}
						else {
							editUuidGen->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editUuidGen->setOptionalLabelNull();
						}
						else {
							editUuidGen->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editUuidGen->setOptionalShortDescriptionNull();
						}
						else {
							editUuidGen->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editUuidGen->setOptionalDescriptionNull();
						}
						else {
							editUuidGen->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editUuidGen->setOptionalDefaultXmlValueNull();
						}
						else {
							editUuidGen->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editUuidGen->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editUuidGen->setOptionalGenerateIdNull();
						}
						else {
							editUuidGen->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editUuidGen->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editUuidGen->setOptionalDbNameNull();
						}
						else {
							editUuidGen->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editUuidGen->setOptionalInitValueNull();
						}
						else {
							editUuidGen->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editUuidGen != NULL ) {
				if( origUuidGen != NULL ) {
					...
					origUuidGen = dynamic_cast<cfbam::ICFBamUuidGenObj*>( editUuidGen->create() );
				}
				editUuidGen = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderUuidGenHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
