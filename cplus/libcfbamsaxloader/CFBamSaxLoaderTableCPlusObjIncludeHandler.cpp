...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusObjInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusObjIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusObjIncludeHandler::CFBamSaxLoaderTableCPlusObjIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusObjIncludeHandler::~CFBamSaxLoaderTableCPlusObjIncludeHandler() {
	}

	void CFBamSaxLoaderTableCPlusObjIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusObjInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusObjIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusObjInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppObjIncludeNull() || origBuff->getOptionalCppObjIncludeValue().empty() ) {
			editBuff->setOptionalCppObjIncludeValue( attrText );
		}
		else {
