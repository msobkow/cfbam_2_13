...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeObjImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeObjImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeObjImportHandler::CFBamSaxLoaderTableCafeObjImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeObjImportHandler::~CFBamSaxLoaderTableCafeObjImportHandler() {
	}

	void CFBamSaxLoaderTableCafeObjImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeObjImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeObjImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeObjImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJObjImportNull() || origBuff->getOptionalJObjImportValue().empty() ) {
			editBuff->setOptionalJObjImportValue( attrText );
		}
		else {
