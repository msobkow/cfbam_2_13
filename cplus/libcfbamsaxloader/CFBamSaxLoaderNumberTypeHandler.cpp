...
// Description: C++18 XML SAX2 Element Handler implementation for NumberType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderNumberTypeHandler::CLASS_NAME( "CFBamSaxLoaderNumberTypeHandler" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderNumberTypeHandler::S_1( "1" );

	CFBamSaxLoaderNumberTypeHandler::CFBamSaxLoaderNumberTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderNumberTypeHandler::~CFBamSaxLoaderNumberTypeHandler() {
	}

	void CFBamSaxLoaderNumberTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamNumberTypeObj* tmpBuff = NULL;
		cfbam::ICFBamNumberTypeEditObj* editBuff = NULL;
		cfbam::ICFBamNumberTypeEditObj* editNumberType = NULL;
		cfbam::ICFBamNumberTypeObj* origNumberType = NULL;
		...
		int16_t natDigits;
		int16_t natPrecis;
		cflib::TCFLibBigDecimal<31,5>* natInitValue = NULL;
		cflib::TCFLibBigDecimal<31,5>* natMinValue = NULL;
		cflib::TCFLibBigDecimal<31,5>* natMaxValue = NULL;
		try {
			if( ( uri == NULL ) || ( *uri == 0 ) ) {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamNumberTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamNumberTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getNumberTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamNumberTypeEditObj*>( tmpBuff->beginEdit() );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( *attrLocalName == S_SpecificallyId ) {
				...
				else if( *attrLocalName == S_Digits ) {
					if( attrDigits != NULL ) {
						throw cflib::CFLibUniqueIndexViolationException( CLASS_NAME,
							S_ProcName,
							*attrLocalName );
					...
					if( cppValue != NULL ) {
						attrDigits = new std::string( *cppValue );
					}
					else {
						attrDigits = NULL;
					}
			...
			if( attrDigits != NULL ) {
				curContext->putNamedTag( S_Digits, *attrDigits );
			}
			if( attrPrecis != NULL ) {
			...
			natDigits = cflib::CFLibXmlUtil::evalInt16( *attrDigits );
			...
			if( ( attrInitValue == NULL ) || ( attrInitValue->length() == 0 ) ) {
				...
				natInitValue = cflib::TCFLibBigDecimal<31,5>::parse( *attrInitValue );
			...
			if( ( attrMinValue == NULL ) || ( attrMinValue->length() == 0 ) ) {
				...
				natMinValue = cflib::TCFLibBigDecimal<31,5>::parse( *attrMinValue );
				if( natMinValue == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
			...
			if( natMinValue != NULL ) {
				editBuff->setOptionalMinValueValue( *natMinValue );
			}
			else {
			...
			if( ( attrMaxValue == NULL ) || ( attrMaxValue->length() == 0 ) ) {
				...
				natMaxValue = cflib::TCFLibBigDecimal<31,5>::parse( *attrMaxValue );
				if( natMaxValue == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
			...
			if( natMaxValue != NULL ) {
				editBuff->setOptionalMaxValueValue( *natMaxValue );
			}
			else {
			...
			loaderBehaviour = saxLoader->getNumberTypeLoaderBehaviour();
			origNumberType = dynamic_cast<cfbam::ICFBamNumberTypeObj*>( schemaObj->getNumberTypeTableObj()->readNumberTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origNumberType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editNumberType = dynamic_cast<cfbam::ICFBamNumberTypeEditObj*>( origNumberType->beginEdit() );
						editNumberType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editNumberType->setOptionalShortNameNull();
						}
						else {
							editNumberType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editNumberType->setOptionalLabelNull();
						}
						else {
							editNumberType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editNumberType->setOptionalShortDescriptionNull();
						}
						else {
							editNumberType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editNumberType->setOptionalDescriptionNull();
						}
						else {
							editNumberType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editNumberType->setOptionalDefaultXmlValueNull();
						}
						else {
							editNumberType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editNumberType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editNumberType->setOptionalGenerateIdNull();
						}
						else {
							editNumberType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editNumberType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editNumberType->setOptionalDbNameNull();
						}
						else {
							editNumberType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						...
						if( editBuff->isOptionalInitValueNull() ) {
							editNumberType->setOptionalInitValueNull();
						}
						else {
							editNumberType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalMinValueNull() ) {
							editNumberType->setOptionalMinValueNull();
						}
						else {
							editNumberType->setOptionalMinValueValue( editBuff->getOptionalMinValueValue() );
						}
						if( editBuff->isOptionalMaxValueNull() ) {
							editNumberType->setOptionalMaxValueNull();
						}
						else {
							editNumberType->setOptionalMaxValueValue( editBuff->getOptionalMaxValueValue() );
						}
			...
			if( editNumberType != NULL ) {
				if( origNumberType != NULL ) {
					...
					origNumberType = dynamic_cast<cfbam::ICFBamNumberTypeObj*>( editNumberType->create() );
				}
				editNumberType = NULL;
			}
			...
			curContext->setObj( origNumberType );
...
cleanup:
	...
	void CFBamSaxLoaderNumberTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
