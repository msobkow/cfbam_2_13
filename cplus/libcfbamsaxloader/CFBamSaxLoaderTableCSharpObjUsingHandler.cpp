...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CSharpObjUsing
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCSharpObjUsingHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCSharpObjUsingHandler::CFBamSaxLoaderTableCSharpObjUsingHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCSharpObjUsingHandler::~CFBamSaxLoaderTableCSharpObjUsingHandler() {
	}

	void CFBamSaxLoaderTableCSharpObjUsingHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CSharpObjUsing" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCSharpObjUsingHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CSharpObjUsing more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCsObjUsingNull() || origBuff->getOptionalCsObjUsingValue().empty() ) {
			editBuff->setOptionalCsObjUsingValue( attrText );
		}
		else {
