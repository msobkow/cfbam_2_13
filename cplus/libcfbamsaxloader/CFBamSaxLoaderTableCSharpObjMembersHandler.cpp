...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CSharpObjMembers
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCSharpObjMembersHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCSharpObjMembersHandler::CFBamSaxLoaderTableCSharpObjMembersHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCSharpObjMembersHandler::~CFBamSaxLoaderTableCSharpObjMembersHandler() {
	}

	void CFBamSaxLoaderTableCSharpObjMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CSharpObjMembers" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCSharpObjMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CSharpObjMembers more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCsObjMembersNull() || origBuff->getOptionalCsObjMembersValue().empty() ) {
			editBuff->setOptionalCsObjMembersValue( attrText );
		}
		else {
