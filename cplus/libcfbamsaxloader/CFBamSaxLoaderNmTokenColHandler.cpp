...
// Description: C++18 XML SAX2 Element Handler implementation for NmTokenCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderNmTokenColHandler::CLASS_NAME( "CFBamSaxLoaderNmTokenColHandler" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderNmTokenColHandler::S_1( "1" );

	CFBamSaxLoaderNmTokenColHandler::CFBamSaxLoaderNmTokenColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderNmTokenColHandler::~CFBamSaxLoaderNmTokenColHandler() {
	}

	void CFBamSaxLoaderNmTokenColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamNmTokenColObj* tmpBuff = NULL;
		cfbam::ICFBamNmTokenColEditObj* editBuff = NULL;
		cfbam::ICFBamNmTokenColEditObj* editNmTokenCol = NULL;
		cfbam::ICFBamNmTokenColObj* origNmTokenCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamNmTokenColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamNmTokenColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getNmTokenColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamNmTokenColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getNmTokenColLoaderBehaviour();
			origNmTokenCol = dynamic_cast<cfbam::ICFBamNmTokenColObj*>( schemaObj->getNmTokenColTableObj()->readNmTokenColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origNmTokenCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editNmTokenCol = dynamic_cast<cfbam::ICFBamNmTokenColEditObj*>( origNmTokenCol->beginEdit() );
						editNmTokenCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editNmTokenCol->setOptionalShortNameNull();
						}
						else {
							editNmTokenCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editNmTokenCol->setOptionalLabelNull();
						}
						else {
							editNmTokenCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editNmTokenCol->setOptionalShortDescriptionNull();
						}
						else {
							editNmTokenCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editNmTokenCol->setOptionalDescriptionNull();
						}
						else {
							editNmTokenCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editNmTokenCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editNmTokenCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editNmTokenCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editNmTokenCol->setOptionalGenerateIdNull();
						}
						else {
							editNmTokenCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editNmTokenCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editNmTokenCol->setOptionalDbNameNull();
						}
						else {
							editNmTokenCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editNmTokenCol->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editNmTokenCol->setOptionalInitValueNull();
						}
						else {
							editNmTokenCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editNmTokenCol != NULL ) {
				if( origNmTokenCol != NULL ) {
					...
					origNmTokenCol = dynamic_cast<cfbam::ICFBamNmTokenColObj*>( editNmTokenCol->create() );
				}
				editNmTokenCol = NULL;
			}
			...
			curContext->setObj( origNmTokenCol );
...
cleanup:
	...
	void CFBamSaxLoaderNmTokenColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
