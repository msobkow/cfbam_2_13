...
// Description: C++18 XML SAX2 Element Handler implementation for BlobCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderBlobColHandler::CLASS_NAME( "CFBamSaxLoaderBlobColHandler" );
	const std::string CFBamSaxLoaderBlobColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderBlobColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderBlobColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderBlobColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderBlobColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderBlobColHandler::S_1( "1" );

	CFBamSaxLoaderBlobColHandler::CFBamSaxLoaderBlobColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderBlobColHandler::~CFBamSaxLoaderBlobColHandler() {
	}

	void CFBamSaxLoaderBlobColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamBlobColObj* tmpBuff = NULL;
		cfbam::ICFBamBlobColEditObj* editBuff = NULL;
		cfbam::ICFBamBlobColEditObj* editBlobCol = NULL;
		cfbam::ICFBamBlobColObj* origBlobCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamBlobColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamBlobColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getBlobColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamBlobColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getBlobColLoaderBehaviour();
			origBlobCol = dynamic_cast<cfbam::ICFBamBlobColObj*>( schemaObj->getBlobColTableObj()->readBlobColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origBlobCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editBlobCol = dynamic_cast<cfbam::ICFBamBlobColEditObj*>( origBlobCol->beginEdit() );
						editBlobCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editBlobCol->setOptionalShortNameNull();
						}
						else {
							editBlobCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editBlobCol->setOptionalLabelNull();
						}
						else {
							editBlobCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editBlobCol->setOptionalShortDescriptionNull();
						}
						else {
							editBlobCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editBlobCol->setOptionalDescriptionNull();
						}
						else {
							editBlobCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editBlobCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editBlobCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editBlobCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editBlobCol->setOptionalGenerateIdNull();
						}
						else {
							editBlobCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editBlobCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editBlobCol->setOptionalDbNameNull();
						}
						else {
							editBlobCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editBlobCol->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editBlobCol->setOptionalInitValueNull();
						}
						else {
							editBlobCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editBlobCol != NULL ) {
				if( origBlobCol != NULL ) {
					...
					origBlobCol = dynamic_cast<cfbam::ICFBamBlobColObj*>( editBlobCol->create() );
				}
				editBlobCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderBlobColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
