...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeTableImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeTableImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeTableImportHandler::CFBamSaxLoaderTableCafeTableImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeTableImportHandler::~CFBamSaxLoaderTableCafeTableImportHandler() {
	}

	void CFBamSaxLoaderTableCafeTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeTableImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeTableImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJTableImportNull() || origBuff->getOptionalJTableImportValue().empty() ) {
			editBuff->setOptionalJTableImportValue( attrText );
		}
		else {
