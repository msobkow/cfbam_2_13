...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusMSSqlTableInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler::CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler::~CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler() {
	}

	void CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusMSSqlTableInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusMSSqlTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusMSSqlTableInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppMSSqlTableIncludeNull() || origBuff->getOptionalCppMSSqlTableIncludeValue().empty() ) {
			editBuff->setOptionalCppMSSqlTableIncludeValue( attrText );
		}
		else {
