...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusEditObjInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusEditObjIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusEditObjIncludeHandler::CFBamSaxLoaderTableCPlusEditObjIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusEditObjIncludeHandler::~CFBamSaxLoaderTableCPlusEditObjIncludeHandler() {
	}

	void CFBamSaxLoaderTableCPlusEditObjIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusEditObjInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusEditObjIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusEditObjInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppEditObjIncludeNull() || origBuff->getOptionalCppEditObjIncludeValue().empty() ) {
			editBuff->setOptionalCppEditObjIncludeValue( attrText );
		}
		else {
