...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeEditObjImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeEditObjImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeEditObjImportHandler::CFBamSaxLoaderTableCafeEditObjImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeEditObjImportHandler::~CFBamSaxLoaderTableCafeEditObjImportHandler() {
	}

	void CFBamSaxLoaderTableCafeEditObjImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeEditObjImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeEditObjImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeEditObjImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJEditObjImportNull() || origBuff->getOptionalJEditObjImportValue().empty() ) {
			editBuff->setOptionalJEditObjImportValue( attrText );
		}
		else {
