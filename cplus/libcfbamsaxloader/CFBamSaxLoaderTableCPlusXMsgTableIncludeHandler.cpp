...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusXMsgTableInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler::CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler::~CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler() {
	}

	void CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusXMsgTableInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusXMsgTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusXMsgTableInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppXMsgTableIncludeNull() || origBuff->getOptionalCppXMsgTableIncludeValue().empty() ) {
			editBuff->setOptionalCppXMsgTableIncludeValue( attrText );
		}
		else {
