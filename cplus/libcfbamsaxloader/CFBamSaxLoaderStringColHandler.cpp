...
// Description: C++18 XML SAX2 Element Handler implementation for StringCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderStringColHandler::CLASS_NAME( "CFBamSaxLoaderStringColHandler" );
	const std::string CFBamSaxLoaderStringColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderStringColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderStringColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderStringColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderStringColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderStringColHandler::S_1( "1" );

	CFBamSaxLoaderStringColHandler::CFBamSaxLoaderStringColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderStringColHandler::~CFBamSaxLoaderStringColHandler() {
	}

	void CFBamSaxLoaderStringColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamStringColObj* tmpBuff = NULL;
		cfbam::ICFBamStringColEditObj* editBuff = NULL;
		cfbam::ICFBamStringColEditObj* editStringCol = NULL;
		cfbam::ICFBamStringColObj* origStringCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamStringColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamStringColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getStringColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamStringColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getStringColLoaderBehaviour();
			origStringCol = dynamic_cast<cfbam::ICFBamStringColObj*>( schemaObj->getStringColTableObj()->readStringColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origStringCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editStringCol = dynamic_cast<cfbam::ICFBamStringColEditObj*>( origStringCol->beginEdit() );
						editStringCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editStringCol->setOptionalShortNameNull();
						}
						else {
							editStringCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editStringCol->setOptionalLabelNull();
						}
						else {
							editStringCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editStringCol->setOptionalShortDescriptionNull();
						}
						else {
							editStringCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editStringCol->setOptionalDescriptionNull();
						}
						else {
							editStringCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editStringCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editStringCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editStringCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editStringCol->setOptionalGenerateIdNull();
						}
						else {
							editStringCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editStringCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editStringCol->setOptionalDbNameNull();
						}
						else {
							editStringCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						...
						if( editBuff->isOptionalInitValueNull() ) {
							editStringCol->setOptionalInitValueNull();
						}
						else {
							editStringCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editStringCol != NULL ) {
				if( origStringCol != NULL ) {
					...
					origStringCol = dynamic_cast<cfbam::ICFBamStringColObj*>( editStringCol->create() );
				}
				editStringCol = NULL;
			}
			...
			curContext->setObj( origStringCol );
...
cleanup:
	...
	void CFBamSaxLoaderStringColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
