...
// Description: C++18 XML SAX2 Element Handler implementation for ServerProc
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderServerProcHandler::CLASS_NAME( "CFBamSaxLoaderServerProcHandler" );
	const std::string CFBamSaxLoaderServerProcHandler::S_True( "true" );
	const std::string CFBamSaxLoaderServerProcHandler::S_False( "false" );
	const std::string CFBamSaxLoaderServerProcHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderServerProcHandler::S_No( "no" );
	const std::string CFBamSaxLoaderServerProcHandler::S_0( "0" );
	const std::string CFBamSaxLoaderServerProcHandler::S_1( "1" );

	CFBamSaxLoaderServerProcHandler::CFBamSaxLoaderServerProcHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderServerProcHandler::~CFBamSaxLoaderServerProcHandler() {
	}

	void CFBamSaxLoaderServerProcHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamServerProcObj* tmpBuff = NULL;
		cfbam::ICFBamServerProcEditObj* editBuff = NULL;
		cfbam::ICFBamServerProcEditObj* editServerProc = NULL;
		cfbam::ICFBamServerProcObj* origServerProc = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamServerProcBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamServerProcBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getServerProcTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamServerProcEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getServerProcLoaderBehaviour();
			origServerProc = dynamic_cast<cfbam::ICFBamServerProcObj*>( schemaObj->getServerProcTableObj()->readServerProcByUNameIdx( refForTable->getRequiredTenantId(),
			...
			if( origServerProc == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editServerProc = dynamic_cast<cfbam::ICFBamServerProcEditObj*>( origServerProc->beginEdit() );
						editServerProc->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editServerProc->setOptionalShortNameNull();
						}
						else {
							editServerProc->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editServerProc->setOptionalLabelNull();
						}
						else {
							editServerProc->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editServerProc->setOptionalShortDescriptionNull();
						}
						else {
							editServerProc->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editServerProc->setOptionalDescriptionNull();
						}
						else {
							editServerProc->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalSuffixNull() ) {
							editServerProc->setOptionalSuffixNull();
						}
						else {
							editServerProc->setOptionalSuffixValue( editBuff->getOptionalSuffixValue() );
						}
						...
						if( editBuff->isOptionalDummyNull() ) {
							editServerProc->setOptionalDummyNull();
						}
						else {
							editServerProc->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editServerProc != NULL ) {
				if( origServerProc != NULL ) {
					...
					origServerProc = dynamic_cast<cfbam::ICFBamServerProcObj*>( editServerProc->create() );
				}
				editServerProc = NULL;
			}
			...
			curContext->setObj( origServerProc );
...
cleanup:
	...
	void CFBamSaxLoaderServerProcHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
