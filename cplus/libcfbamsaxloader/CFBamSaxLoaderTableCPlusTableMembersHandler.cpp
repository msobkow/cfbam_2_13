...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusTableMembers
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusTableMembersHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusTableMembersHandler::CFBamSaxLoaderTableCPlusTableMembersHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusTableMembersHandler::~CFBamSaxLoaderTableCPlusTableMembersHandler() {
	}

	void CFBamSaxLoaderTableCPlusTableMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusTableMembers" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusTableMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusTableMembers more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppTableMembersNull() || origBuff->getOptionalCppTableMembersValue().empty() ) {
			editBuff->setOptionalCppTableMembersValue( attrText );
		}
		else {
