...
// Description: C++18 XML SAX2 Element Handler implementation for DelDep
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderDelDepHandler::CLASS_NAME( "CFBamSaxLoaderDelDepHandler" );
	const std::string CFBamSaxLoaderDelDepHandler::S_True( "true" );
	const std::string CFBamSaxLoaderDelDepHandler::S_False( "false" );
	const std::string CFBamSaxLoaderDelDepHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderDelDepHandler::S_No( "no" );
	const std::string CFBamSaxLoaderDelDepHandler::S_0( "0" );
	const std::string CFBamSaxLoaderDelDepHandler::S_1( "1" );

	CFBamSaxLoaderDelDepHandler::CFBamSaxLoaderDelDepHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderDelDepHandler::~CFBamSaxLoaderDelDepHandler() {
	}

	void CFBamSaxLoaderDelDepHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamDelDepEditObj* editDelDep = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamDelDepBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamDelDepBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getDelDepTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamDelDepEditObj*>( tmpBuff->beginEdit() );
			...
			editDelDep = editBuff;
			editBuff = NULL;
			origDelDep = dynamic_cast<cfbam::ICFBamDelDepObj*>( editDelDep->create() );
			editDelDep = NULL;
			tmpBuff = NULL;

			curContext->setObj( origDelDep );
...
cleanup:
	...
	void CFBamSaxLoaderDelDepHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
