...
// Description: C++18 XML SAX2 Element Handler implementation for TableCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableColHandler::CLASS_NAME( "CFBamSaxLoaderTableColHandler" );
	const std::string CFBamSaxLoaderTableColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTableColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTableColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTableColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTableColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTableColHandler::S_1( "1" );

	CFBamSaxLoaderTableColHandler::CFBamSaxLoaderTableColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableColHandler::~CFBamSaxLoaderTableColHandler() {
	}

	void CFBamSaxLoaderTableColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		static const std::string S_DataType( "DataType" );
		...
		cfbam::ICFBamTableColObj* tmpBuff = NULL;
		cfbam::ICFBamTableColEditObj* editBuff = NULL;
		cfbam::ICFBamTableColEditObj* editTableCol = NULL;
		cfbam::ICFBamTableColObj* origTableCol = NULL;
		...
		std::string* attrXmlElementName = NULL;
		std::string* attrDataType = NULL;
		// TableCol References
		cfbam::ICFBamTableObj* refTable = NULL;
		cfbam::ICFBamValueObj* refDataType = NULL;
		...
		std::string* natXmlElementName = NULL;
		try {
			if( ( uri == NULL ) || ( *uri == 0 ) ) {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTableColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTableColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTableColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTableColEditObj*>( tmpBuff->beginEdit() );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( *attrLocalName == S_SpecificallyId ) {
				...
				else if( *attrLocalName == S_XmlElementName ) {
					...
					if( cppValue != NULL ) {
						attrXmlElementName = new std::string( *cppValue );
					}
					else {
						attrXmlElementName = NULL;
					}
				}
				else if( *attrLocalName == S_DataType ) {
					...
					if( cppValue != NULL ) {
						attrDataType = new std::string( *cppValue );
					}
					else {
						attrDataType = NULL;
					}
			...
			if( attrXmlElementName != NULL ) {
				curContext->putNamedTag( S_XmlElementName, *attrXmlElementName );
			}
			if( attrDataType != NULL ) {
				curContext->putNamedTag( S_DataType, *attrDataType );
			}
			...
			if( attrXmlElementName != NULL ) {
				natXmlElementName = new std::string( *attrXmlElementName );
			}
			else {
				natXmlElementName = NULL;
			}
			if( natXmlElementName != NULL ) {
				editBuff->setOptionalXmlElementNameValue( *natXmlElementName );
			}
			else {
				editBuff->setOptionalXmlElementNameNull();
			}
			...
			if( ( attrDataType != NULL ) && ( attrDataType->length() > 0 ) ) {
				refDataType = dynamic_cast<cfbam::ICFBamValueObj*>( editBuff->getNamedObject( schemaObj->getValueTableObj()->getObjQualifyingClassCode(),
					*attrDataType ) );
				if( refDataType == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			loaderBehaviour = saxLoader->getTableColLoaderBehaviour();
			origTableCol = dynamic_cast<cfbam::ICFBamTableColObj*>( schemaObj->getTableColTableObj()->readTableColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTableCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTableCol = dynamic_cast<cfbam::ICFBamTableColEditObj*>( origTableCol->beginEdit() );
						editTableCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTableCol->setOptionalShortNameNull();
						}
						else {
							editTableCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTableCol->setOptionalLabelNull();
						}
						else {
							editTableCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTableCol->setOptionalShortDescriptionNull();
						}
						else {
							editTableCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTableCol->setOptionalDescriptionNull();
						}
						else {
							editTableCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTableCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTableCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTableCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTableCol->setOptionalGenerateIdNull();
						}
						else {
							editTableCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTableCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTableCol->setOptionalDbNameNull();
						}
						else {
							editTableCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalXmlElementNameNull() ) {
							editTableCol->setOptionalXmlElementNameNull();
						}
						else {
							editTableCol->setOptionalXmlElementNameValue( editBuff->getOptionalXmlElementNameValue() );
						}
			...
			if( editTableCol != NULL ) {
				if( origTableCol != NULL ) {
					...
					origTableCol = dynamic_cast<cfbam::ICFBamTableColObj*>( editTableCol->create() );
				}
				editTableCol = NULL;
			}
			...
			curContext->setObj( origTableCol );
...
cleanup:
	...
	void CFBamSaxLoaderTableColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
