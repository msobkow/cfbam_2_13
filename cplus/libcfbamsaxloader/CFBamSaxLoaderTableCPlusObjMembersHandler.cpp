...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusObjMembers
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusObjMembersHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusObjMembersHandler::CFBamSaxLoaderTableCPlusObjMembersHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusObjMembersHandler::~CFBamSaxLoaderTableCPlusObjMembersHandler() {
	}

	void CFBamSaxLoaderTableCPlusObjMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusObjMembers" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusObjMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusObjMembers more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppObjMembersNull() || origBuff->getOptionalCppObjMembersValue().empty() ) {
			editBuff->setOptionalCppObjMembersValue( attrText );
		}
		else {
