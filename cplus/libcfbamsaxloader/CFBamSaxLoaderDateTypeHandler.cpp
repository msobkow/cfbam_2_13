...
// Description: C++18 XML SAX2 Element Handler implementation for DateType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderDateTypeHandler::CLASS_NAME( "CFBamSaxLoaderDateTypeHandler" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderDateTypeHandler::S_1( "1" );

	CFBamSaxLoaderDateTypeHandler::CFBamSaxLoaderDateTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderDateTypeHandler::~CFBamSaxLoaderDateTypeHandler() {
	}

	void CFBamSaxLoaderDateTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamDateTypeEditObj* editDateType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamDateTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamDateTypeBuff::GENDEFNAME + "'" );
			...
			loaderBehaviour = saxLoader->getDateTypeLoaderBehaviour();
			origDateType = dynamic_cast<cfbam::ICFBamDateTypeObj*>( schemaObj->getDateTypeTableObj()->readDateTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origDateType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editDateType = dynamic_cast<cfbam::ICFBamDateTypeEditObj*>( origDateType->beginEdit() );
						editDateType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editDateType->setOptionalShortNameNull();
						}
						else {
							editDateType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editDateType->setOptionalLabelNull();
						}
						else {
							editDateType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editDateType->setOptionalShortDescriptionNull();
						}
						else {
							editDateType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editDateType->setOptionalDescriptionNull();
						}
						else {
							editDateType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editDateType->setOptionalDefaultXmlValueNull();
						}
						else {
							editDateType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editDateType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editDateType->setOptionalGenerateIdNull();
						}
						else {
							editDateType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editDateType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editDateType->setOptionalDbNameNull();
						}
						else {
							editDateType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editDateType->setOptionalDummyNull();
						}
						else {
							editDateType->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editDateType != NULL ) {
				if( origDateType != NULL ) {
					...
					origDateType = dynamic_cast<cfbam::ICFBamDateTypeObj*>( editDateType->create() );
				}
				editDateType = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderDateTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
