...
// Description: C++18 XML SAX2 Element Handler implementation for BoolType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderBoolTypeHandler::CLASS_NAME( "CFBamSaxLoaderBoolTypeHandler" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderBoolTypeHandler::S_1( "1" );

	CFBamSaxLoaderBoolTypeHandler::CFBamSaxLoaderBoolTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderBoolTypeHandler::~CFBamSaxLoaderBoolTypeHandler() {
	}

	void CFBamSaxLoaderBoolTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamBoolTypeObj* tmpBuff = NULL;
		cfbam::ICFBamBoolTypeEditObj* editBuff = NULL;
		cfbam::ICFBamBoolTypeEditObj* editBoolType = NULL;
		cfbam::ICFBamBoolTypeObj* origBoolType = NULL;
		...
		std::string* attrFalseString = NULL;
		std::string* attrTrueString = NULL;
		std::string* attrNullString = NULL;
		// BoolDef References
		// BoolType Attributes
		// BoolType References
		...
		cflib::CFLibNullableBool natInitValue = NULL;
		std::string* natFalseString = NULL;
		std::string* natTrueString = NULL;
		std::string* natNullString = NULL;
		try {
			if( ( uri == NULL ) || ( *uri == 0 ) ) {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamBoolTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamBoolTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getBoolTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamBoolTypeEditObj*>( tmpBuff->beginEdit() );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( *attrLocalName == S_SpecificallyId ) {
				...
				else if( *attrLocalName == S_FalseString ) {
					...
					if( cppValue != NULL ) {
						attrFalseString = new std::string( *cppValue );
					}
					else {
						attrFalseString = NULL;
					}
				}
				else if( *attrLocalName == S_TrueString ) {
					...
					if( cppValue != NULL ) {
						attrTrueString = new std::string( *cppValue );
					}
					else {
						attrTrueString = NULL;
					}
				}
				else if( *attrLocalName == S_NullString ) {
					...
					if( cppValue != NULL ) {
						attrNullString = new std::string( *cppValue );
					}
					else {
						attrNullString = NULL;
					}
			...
			if( attrFalseString != NULL ) {
				curContext->putNamedTag( S_FalseString, *attrFalseString );
			}
			if( attrTrueString != NULL ) {
				curContext->putNamedTag( S_TrueString, *attrTrueString );
			}
			if( attrNullString != NULL ) {
				curContext->putNamedTag( S_NullString, *attrNullString );
			}
			...
			else if( ( *attrInitValue == S_True ) || ( *attrInitValue == S_Yes ) || ( *attrInitValue == S_1 ) ) {
				natInitValue.setValue( true );
			}
			else if( ( *attrInitValue == S_False ) || ( *attrInitValue == S_No ) || ( *attrInitValue == S_0 ) ) {
				natInitValue.setValue( false );
			}
			else {
				throw cflib::CFLibInvalidArgumentException( CLASS_NAME,
					S_ProcName,
			...
			if( attrFalseString != NULL ) {
				natFalseString = new std::string( *attrFalseString );
			}
			else {
				natFalseString = NULL;
			}
			if( natFalseString != NULL ) {
				editBuff->setOptionalFalseStringValue( *natFalseString );
			}
			else {
				editBuff->setOptionalFalseStringNull();
			}

			if( attrTrueString != NULL ) {
				natTrueString = new std::string( *attrTrueString );
			}
			else {
				natTrueString = NULL;
			}
			if( natTrueString != NULL ) {
				editBuff->setOptionalTrueStringValue( *natTrueString );
			}
			else {
				editBuff->setOptionalTrueStringNull();
			}

			if( attrNullString != NULL ) {
				natNullString = new std::string( *attrNullString );
			}
			else {
				natNullString = NULL;
			}
			if( natNullString != NULL ) {
				editBuff->setOptionalNullStringValue( *natNullString );
			}
			else {
				editBuff->setOptionalNullStringNull();
			}
			...
			loaderBehaviour = saxLoader->getBoolTypeLoaderBehaviour();
			origBoolType = dynamic_cast<cfbam::ICFBamBoolTypeObj*>( schemaObj->getBoolTypeTableObj()->readBoolTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origBoolType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editBoolType = dynamic_cast<cfbam::ICFBamBoolTypeEditObj*>( origBoolType->beginEdit() );
						editBoolType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editBoolType->setOptionalShortNameNull();
						}
						else {
							editBoolType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editBoolType->setOptionalLabelNull();
						}
						else {
							editBoolType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editBoolType->setOptionalShortDescriptionNull();
						}
						else {
							editBoolType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editBoolType->setOptionalDescriptionNull();
						}
						else {
							editBoolType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editBoolType->setOptionalDefaultXmlValueNull();
						}
						else {
							editBoolType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editBoolType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editBoolType->setOptionalGenerateIdNull();
						}
						else {
							editBoolType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editBoolType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editBoolType->setOptionalDbNameNull();
						}
						else {
							editBoolType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editBoolType->setOptionalInitValueNull();
						}
						else {
							editBoolType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalFalseStringNull() ) {
							editBoolType->setOptionalFalseStringNull();
						}
						else {
							editBoolType->setOptionalFalseStringValue( editBuff->getOptionalFalseStringValue() );
						}
						if( editBuff->isOptionalTrueStringNull() ) {
							editBoolType->setOptionalTrueStringNull();
						}
						else {
							editBoolType->setOptionalTrueStringValue( editBuff->getOptionalTrueStringValue() );
						}
						if( editBuff->isOptionalNullStringNull() ) {
							editBoolType->setOptionalNullStringNull();
						}
						else {
							editBoolType->setOptionalNullStringValue( editBuff->getOptionalNullStringValue() );
						}
			...
			if( editBoolType != NULL ) {
				if( origBoolType != NULL ) {
					...
					origBoolType = dynamic_cast<cfbam::ICFBamBoolTypeObj*>( editBoolType->create() );
				}
				editBoolType = NULL;
			}
			...
			curContext->setObj( origBoolType );
...
cleanup:
		...
		if( natFalseString != NULL ) {
			delete natFalseString;
			natFalseString = NULL;
		}

		if( natTrueString != NULL ) {
			delete natTrueString;
			natTrueString = NULL;
		}

		if( natNullString != NULL ) {
			delete natNullString;
			natNullString = NULL;
		}
		...
		if( attrFalseString != NULL ) {
			delete attrFalseString;
			attrFalseString = NULL;
		}

		if( attrTrueString != NULL ) {
			delete attrTrueString;
			attrTrueString = NULL;
		}

		if( attrNullString != NULL ) {
			delete attrNullString;
			attrNullString = NULL;
		}
	...
	void CFBamSaxLoaderBoolTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
