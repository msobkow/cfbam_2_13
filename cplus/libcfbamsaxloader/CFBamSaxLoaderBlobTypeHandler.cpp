...
// Description: C++18 XML SAX2 Element Handler implementation for BlobType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderBlobTypeHandler::CLASS_NAME( "CFBamSaxLoaderBlobTypeHandler" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderBlobTypeHandler::S_1( "1" );

	CFBamSaxLoaderBlobTypeHandler::CFBamSaxLoaderBlobTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderBlobTypeHandler::~CFBamSaxLoaderBlobTypeHandler() {
	}

	void CFBamSaxLoaderBlobTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamBlobTypeObj* tmpBuff = NULL;
		cfbam::ICFBamBlobTypeEditObj* editBuff = NULL;
		cfbam::ICFBamBlobTypeEditObj* editBlobType = NULL;
		cfbam::ICFBamBlobTypeObj* origBlobType = NULL;
		...
		int32_t natMaxLen;
		std::vector<cflib::BYTE>* natInitValue = NULL;
		try {
			if( ( uri == NULL ) || ( *uri == 0 ) ) {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamBlobTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamBlobTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getBlobTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamBlobTypeEditObj*>( tmpBuff->beginEdit() );
			...
			if( ( attrInitValue == NULL ) || ( attrInitValue->length() == 0 ) ) {
				...
				natInitValue = cflib::CFLibXmlUtil::parseBlob( *attrInitValue );
			...
			loaderBehaviour = saxLoader->getBlobTypeLoaderBehaviour();
			origBlobType = dynamic_cast<cfbam::ICFBamBlobTypeObj*>( schemaObj->getBlobTypeTableObj()->readBlobTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origBlobType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editBlobType = dynamic_cast<cfbam::ICFBamBlobTypeEditObj*>( origBlobType->beginEdit() );
						editBlobType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editBlobType->setOptionalShortNameNull();
						}
						else {
							editBlobType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editBlobType->setOptionalLabelNull();
						}
						else {
							editBlobType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editBlobType->setOptionalShortDescriptionNull();
						}
						else {
							editBlobType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editBlobType->setOptionalDescriptionNull();
						}
						else {
							editBlobType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editBlobType->setOptionalDefaultXmlValueNull();
						}
						else {
							editBlobType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editBlobType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editBlobType->setOptionalGenerateIdNull();
						}
						else {
							editBlobType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editBlobType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editBlobType->setOptionalDbNameNull();
						}
						else {
							editBlobType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editBlobType->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editBlobType->setOptionalInitValueNull();
						}
						else {
							editBlobType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editBlobType != NULL ) {
				if( origBlobType != NULL ) {
					...
					origBlobType = dynamic_cast<cfbam::ICFBamBlobTypeObj*>( editBlobType->create() );
				}
				editBlobType = NULL;
			}
			...
			curContext->setObj( origBlobType );
...
cleanup:
	...
	void CFBamSaxLoaderBlobTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
