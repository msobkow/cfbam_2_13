...
// Description: C++18 XML SAX2 Element Handler implementation for PopDep
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderPopDepHandler::CLASS_NAME( "CFBamSaxLoaderPopDepHandler" );
	const std::string CFBamSaxLoaderPopDepHandler::S_True( "true" );
	const std::string CFBamSaxLoaderPopDepHandler::S_False( "false" );
	const std::string CFBamSaxLoaderPopDepHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderPopDepHandler::S_No( "no" );
	const std::string CFBamSaxLoaderPopDepHandler::S_0( "0" );
	const std::string CFBamSaxLoaderPopDepHandler::S_1( "1" );

	CFBamSaxLoaderPopDepHandler::CFBamSaxLoaderPopDepHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderPopDepHandler::~CFBamSaxLoaderPopDepHandler() {
	}

	void CFBamSaxLoaderPopDepHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamPopDepObj* tmpBuff = NULL;
		cfbam::ICFBamPopDepEditObj* editBuff = NULL;
		cfbam::ICFBamPopDepEditObj* editPopDep = NULL;
		cfbam::ICFBamPopDepObj* origPopDep = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamPopDepBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamPopDepBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getPopDepTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamPopDepEditObj*>( tmpBuff->beginEdit() );
			...
			editPopDep = editBuff;
			editBuff = NULL;
			origPopDep = dynamic_cast<cfbam::ICFBamPopDepObj*>( editPopDep->create() );
			editPopDep = NULL;
			tmpBuff = NULL;

			curContext->setObj( origPopDep );
...
cleanup:
	...
	void CFBamSaxLoaderPopDepHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
