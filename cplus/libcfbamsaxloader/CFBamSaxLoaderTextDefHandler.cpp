...
// Description: C++18 XML SAX2 Element Handler implementation for TextDef
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTextDefHandler::CLASS_NAME( "CFBamSaxLoaderTextDefHandler" );
	const std::string CFBamSaxLoaderTextDefHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTextDefHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTextDefHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTextDefHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTextDefHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTextDefHandler::S_1( "1" );

	CFBamSaxLoaderTextDefHandler::CFBamSaxLoaderTextDefHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTextDefHandler::~CFBamSaxLoaderTextDefHandler() {
	}

	void CFBamSaxLoaderTextDefHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTextDefObj* tmpBuff = NULL;
		cfbam::ICFBamTextDefEditObj* editBuff = NULL;
		cfbam::ICFBamTextDefEditObj* editTextDef = NULL;
		cfbam::ICFBamTextDefObj* origTextDef = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTextDefBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTextDefBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTextDefTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTextDefEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTextDefLoaderBehaviour();
			origTextDef = dynamic_cast<cfbam::ICFBamTextDefObj*>( schemaObj->getTextDefTableObj()->readTextDefByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTextDef == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTextDef = dynamic_cast<cfbam::ICFBamTextDefEditObj*>( origTextDef->beginEdit() );
						editTextDef->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTextDef->setOptionalShortNameNull();
						}
						else {
							editTextDef->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTextDef->setOptionalLabelNull();
						}
						else {
							editTextDef->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTextDef->setOptionalShortDescriptionNull();
						}
						else {
							editTextDef->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTextDef->setOptionalDescriptionNull();
						}
						else {
							editTextDef->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTextDef->setOptionalDefaultXmlValueNull();
						}
						else {
							editTextDef->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTextDef->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTextDef->setOptionalGenerateIdNull();
						}
						else {
							editTextDef->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTextDef->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTextDef->setOptionalDbNameNull();
						}
						else {
							editTextDef->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editTextDef->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editTextDef->setOptionalInitValueNull();
						}
						else {
							editTextDef->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalXmlElementNameNull() ) {
							editTextDef->setOptionalXmlElementNameNull();
						}
						else {
							editTextDef->setOptionalXmlElementNameValue( editBuff->getOptionalXmlElementNameValue() );
						}
			...
			if( editTextDef != NULL ) {
				if( origTextDef != NULL ) {
					...
					origTextDef = dynamic_cast<cfbam::ICFBamTextDefObj*>( editTextDef->create() );
				}
				editTextDef = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderTextDefHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
