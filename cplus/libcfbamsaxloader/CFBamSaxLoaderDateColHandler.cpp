...
// Description: C++18 XML SAX2 Element Handler implementation for DateCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderDateColHandler::CLASS_NAME( "CFBamSaxLoaderDateColHandler" );
	const std::string CFBamSaxLoaderDateColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderDateColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderDateColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderDateColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderDateColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderDateColHandler::S_1( "1" );

	CFBamSaxLoaderDateColHandler::CFBamSaxLoaderDateColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderDateColHandler::~CFBamSaxLoaderDateColHandler() {
	}

	void CFBamSaxLoaderDateColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamDateColObj* tmpBuff = NULL;
		cfbam::ICFBamDateColEditObj* editBuff = NULL;
		cfbam::ICFBamDateColEditObj* editDateCol = NULL;
		cfbam::ICFBamDateColObj* origDateCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamDateColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamDateColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getDateColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamDateColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getDateColLoaderBehaviour();
			origDateCol = dynamic_cast<cfbam::ICFBamDateColObj*>( schemaObj->getDateColTableObj()->readDateColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origDateCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editDateCol = dynamic_cast<cfbam::ICFBamDateColEditObj*>( origDateCol->beginEdit() );
						editDateCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editDateCol->setOptionalShortNameNull();
						}
						else {
							editDateCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editDateCol->setOptionalLabelNull();
						}
						else {
							editDateCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editDateCol->setOptionalShortDescriptionNull();
						}
						else {
							editDateCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editDateCol->setOptionalDescriptionNull();
						}
						else {
							editDateCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editDateCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editDateCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editDateCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editDateCol->setOptionalGenerateIdNull();
						}
						else {
							editDateCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editDateCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editDateCol->setOptionalDbNameNull();
						}
						else {
							editDateCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editDateCol->setOptionalDummyNull();
						}
						else {
							editDateCol->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editDateCol != NULL ) {
				if( origDateCol != NULL ) {
					...
					origDateCol = dynamic_cast<cfbam::ICFBamDateColObj*>( editDateCol->create() );
				}
				editDateCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderDateColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
