...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CSharpTableUsing
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCSharpTableUsingHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCSharpTableUsingHandler::CFBamSaxLoaderTableCSharpTableUsingHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCSharpTableUsingHandler::~CFBamSaxLoaderTableCSharpTableUsingHandler() {
	}

	void CFBamSaxLoaderTableCSharpTableUsingHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CSharpTableUsing" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCSharpTableUsingHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CSharpTableUsing more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCsTableUsingNull() || origBuff->getOptionalCsTableUsingValue().empty() ) {
			editBuff->setOptionalCsTableUsingValue( attrText );
		}
		else {
