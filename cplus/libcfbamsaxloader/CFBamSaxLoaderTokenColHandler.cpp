...
// Description: C++18 XML SAX2 Element Handler implementation for TokenCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTokenColHandler::CLASS_NAME( "CFBamSaxLoaderTokenColHandler" );
	const std::string CFBamSaxLoaderTokenColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTokenColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTokenColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTokenColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTokenColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTokenColHandler::S_1( "1" );

	CFBamSaxLoaderTokenColHandler::CFBamSaxLoaderTokenColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTokenColHandler::~CFBamSaxLoaderTokenColHandler() {
	}

	void CFBamSaxLoaderTokenColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTokenColObj* tmpBuff = NULL;
		cfbam::ICFBamTokenColEditObj* editBuff = NULL;
		cfbam::ICFBamTokenColEditObj* editTokenCol = NULL;
		cfbam::ICFBamTokenColObj* origTokenCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTokenColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTokenColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTokenColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTokenColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTokenColLoaderBehaviour();
			origTokenCol = dynamic_cast<cfbam::ICFBamTokenColObj*>( schemaObj->getTokenColTableObj()->readTokenColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTokenCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTokenCol = dynamic_cast<cfbam::ICFBamTokenColEditObj*>( origTokenCol->beginEdit() );
						editTokenCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTokenCol->setOptionalShortNameNull();
						}
						else {
							editTokenCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTokenCol->setOptionalLabelNull();
						}
						else {
							editTokenCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTokenCol->setOptionalShortDescriptionNull();
						}
						else {
							editTokenCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTokenCol->setOptionalDescriptionNull();
						}
						else {
							editTokenCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTokenCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTokenCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTokenCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTokenCol->setOptionalGenerateIdNull();
						}
						else {
							editTokenCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTokenCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTokenCol->setOptionalDbNameNull();
						}
						else {
							editTokenCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editTokenCol->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editTokenCol->setOptionalInitValueNull();
						}
						else {
							editTokenCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editTokenCol != NULL ) {
				if( origTokenCol != NULL ) {
					...
					origTokenCol = dynamic_cast<cfbam::ICFBamTokenColObj*>( editTokenCol->create() );
				}
				editTokenCol = NULL;
			}
			...
			curContext->setObj( origTokenCol );
...
cleanup:
	...
	void CFBamSaxLoaderTokenColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
