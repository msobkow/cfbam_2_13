...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CSharpPgSqlTableUsing
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler::CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler::~CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler() {
	}

	void CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CSharpPgSqlTableUsing" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCSharpPgSqlTableUsingHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CSharpPgSqlTableUsing more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCsPgSqlTableUsingNull() || origBuff->getOptionalCsPgSqlTableUsingValue().empty() ) {
			editBuff->setOptionalCsPgSqlTableUsingValue( attrText );
		}
		else {
