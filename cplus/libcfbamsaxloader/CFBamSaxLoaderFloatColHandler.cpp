...
// Description: C++18 XML SAX2 Element Handler implementation for FloatCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderFloatColHandler::CLASS_NAME( "CFBamSaxLoaderFloatColHandler" );
	const std::string CFBamSaxLoaderFloatColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderFloatColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderFloatColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderFloatColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderFloatColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderFloatColHandler::S_1( "1" );

	CFBamSaxLoaderFloatColHandler::CFBamSaxLoaderFloatColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderFloatColHandler::~CFBamSaxLoaderFloatColHandler() {
	}

	void CFBamSaxLoaderFloatColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamFloatColObj* tmpBuff = NULL;
		cfbam::ICFBamFloatColEditObj* editBuff = NULL;
		cfbam::ICFBamFloatColEditObj* editFloatCol = NULL;
		cfbam::ICFBamFloatColObj* origFloatCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamFloatColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamFloatColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getFloatColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamFloatColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getFloatColLoaderBehaviour();
			origFloatCol = dynamic_cast<cfbam::ICFBamFloatColObj*>( schemaObj->getFloatColTableObj()->readFloatColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origFloatCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editFloatCol = dynamic_cast<cfbam::ICFBamFloatColEditObj*>( origFloatCol->beginEdit() );
						editFloatCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editFloatCol->setOptionalShortNameNull();
						}
						else {
							editFloatCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editFloatCol->setOptionalLabelNull();
						}
						else {
							editFloatCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editFloatCol->setOptionalShortDescriptionNull();
						}
						else {
							editFloatCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editFloatCol->setOptionalDescriptionNull();
						}
						else {
							editFloatCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editFloatCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editFloatCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editFloatCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editFloatCol->setOptionalGenerateIdNull();
						}
						else {
							editFloatCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editFloatCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editFloatCol->setOptionalDbNameNull();
						}
						else {
							editFloatCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editFloatCol->setOptionalInitValueNull();
						}
						else {
							editFloatCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalMinValueNull() ) {
							editFloatCol->setOptionalMinValueNull();
						}
						else {
							editFloatCol->setOptionalMinValueValue( editBuff->getOptionalMinValueValue() );
						}
						if( editBuff->isOptionalMaxValueNull() ) {
							editFloatCol->setOptionalMaxValueNull();
						}
						else {
							editFloatCol->setOptionalMaxValueValue( editBuff->getOptionalMaxValueValue() );
						}
			...
			if( editFloatCol != NULL ) {
				if( origFloatCol != NULL ) {
					...
					origFloatCol = dynamic_cast<cfbam::ICFBamFloatColObj*>( editFloatCol->create() );
				}
				editFloatCol = NULL;
			}
			...
			curContext->setObj( origFloatCol );
...
cleanup:
	...
	void CFBamSaxLoaderFloatColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
