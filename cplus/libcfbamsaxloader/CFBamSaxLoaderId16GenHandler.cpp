...
// Description: C++18 XML SAX2 Element Handler implementation for Id16Gen
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderId16GenHandler::CLASS_NAME( "CFBamSaxLoaderId16GenHandler" );
	const std::string CFBamSaxLoaderId16GenHandler::S_True( "true" );
	const std::string CFBamSaxLoaderId16GenHandler::S_False( "false" );
	const std::string CFBamSaxLoaderId16GenHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderId16GenHandler::S_No( "no" );
	const std::string CFBamSaxLoaderId16GenHandler::S_0( "0" );
	const std::string CFBamSaxLoaderId16GenHandler::S_1( "1" );

	CFBamSaxLoaderId16GenHandler::CFBamSaxLoaderId16GenHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderId16GenHandler::~CFBamSaxLoaderId16GenHandler() {
	}

	void CFBamSaxLoaderId16GenHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamId16GenObj* tmpBuff = NULL;
		cfbam::ICFBamId16GenEditObj* editBuff = NULL;
		cfbam::ICFBamId16GenEditObj* editId16Gen = NULL;
		cfbam::ICFBamId16GenObj* origId16Gen = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamId16GenBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamId16GenBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getId16GenTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamId16GenEditObj*>( tmpBuff->beginEdit() );
			...
			natBlockSize = cflib::CFLibXmlUtil::evalInt16( *attrBlockSize );
			...
			loaderBehaviour = saxLoader->getId16GenLoaderBehaviour();
			origId16Gen = dynamic_cast<cfbam::ICFBamId16GenObj*>( schemaObj->getId16GenTableObj()->readId16GenByUNameIdx( refTenant->getRequiredId(),
			...
			if( origId16Gen == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editId16Gen = dynamic_cast<cfbam::ICFBamId16GenEditObj*>( origId16Gen->beginEdit() );
						editId16Gen->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editId16Gen->setOptionalShortNameNull();
						}
						else {
							editId16Gen->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editId16Gen->setOptionalLabelNull();
						}
						else {
							editId16Gen->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editId16Gen->setOptionalShortDescriptionNull();
						}
						else {
							editId16Gen->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editId16Gen->setOptionalDescriptionNull();
						}
						else {
							editId16Gen->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editId16Gen->setOptionalDefaultXmlValueNull();
						}
						else {
							editId16Gen->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editId16Gen->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editId16Gen->setOptionalGenerateIdNull();
						}
						else {
							editId16Gen->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editId16Gen->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editId16Gen->setOptionalDbNameNull();
						}
						else {
							editId16Gen->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editId16Gen->setOptionalInitValueNull();
						}
						else {
							editId16Gen->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalMinValueNull() ) {
							editId16Gen->setOptionalMinValueNull();
						}
						else {
							editId16Gen->setOptionalMinValueValue( editBuff->getOptionalMinValueValue() );
						}
						if( editBuff->isOptionalMaxValueNull() ) {
							editId16Gen->setOptionalMaxValueNull();
						}
						else {
							editId16Gen->setOptionalMaxValueValue( editBuff->getOptionalMaxValueValue() );
						}
			...
			if( editId16Gen != NULL ) {
				if( origId16Gen != NULL ) {
					...
					origId16Gen = dynamic_cast<cfbam::ICFBamId16GenObj*>( editId16Gen->create() );
				}
				editId16Gen = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderId16GenHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
