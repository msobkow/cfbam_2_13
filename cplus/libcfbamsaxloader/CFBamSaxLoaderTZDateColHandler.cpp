...
// Description: C++18 XML SAX2 Element Handler implementation for TZDateCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTZDateColHandler::CLASS_NAME( "CFBamSaxLoaderTZDateColHandler" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTZDateColHandler::S_1( "1" );

	CFBamSaxLoaderTZDateColHandler::CFBamSaxLoaderTZDateColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTZDateColHandler::~CFBamSaxLoaderTZDateColHandler() {
	}

	void CFBamSaxLoaderTZDateColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTZDateColObj* tmpBuff = NULL;
		cfbam::ICFBamTZDateColEditObj* editBuff = NULL;
		cfbam::ICFBamTZDateColEditObj* editTZDateCol = NULL;
		cfbam::ICFBamTZDateColObj* origTZDateCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTZDateColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTZDateColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTZDateColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTZDateColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTZDateColLoaderBehaviour();
			origTZDateCol = dynamic_cast<cfbam::ICFBamTZDateColObj*>( schemaObj->getTZDateColTableObj()->readTZDateColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTZDateCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTZDateCol = dynamic_cast<cfbam::ICFBamTZDateColEditObj*>( origTZDateCol->beginEdit() );
						editTZDateCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTZDateCol->setOptionalShortNameNull();
						}
						else {
							editTZDateCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTZDateCol->setOptionalLabelNull();
						}
						else {
							editTZDateCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTZDateCol->setOptionalShortDescriptionNull();
						}
						else {
							editTZDateCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTZDateCol->setOptionalDescriptionNull();
						}
						else {
							editTZDateCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTZDateCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTZDateCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTZDateCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTZDateCol->setOptionalGenerateIdNull();
						}
						else {
							editTZDateCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTZDateCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTZDateCol->setOptionalDbNameNull();
						}
						else {
							editTZDateCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTZDateCol->setOptionalDummyNull();
						}
						else {
							editTZDateCol->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTZDateCol != NULL ) {
				if( origTZDateCol != NULL ) {
					...
					origTZDateCol = dynamic_cast<cfbam::ICFBamTZDateColObj*>( editTZDateCol->create() );
				}
				editTZDateCol = NULL;
			}
			...
			curContext->setObj( origTZDateCol );
...
cleanup:
	...
	void CFBamSaxLoaderTZDateColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
