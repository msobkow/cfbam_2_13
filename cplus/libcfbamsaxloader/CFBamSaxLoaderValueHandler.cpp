...
// Description: C++18 XML SAX2 Element Handler implementation for Value
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderValueHandler::CLASS_NAME( "CFBamSaxLoaderValueHandler" );
	const std::string CFBamSaxLoaderValueHandler::S_True( "true" );
	const std::string CFBamSaxLoaderValueHandler::S_False( "false" );
	const std::string CFBamSaxLoaderValueHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderValueHandler::S_No( "no" );
	const std::string CFBamSaxLoaderValueHandler::S_0( "0" );
	const std::string CFBamSaxLoaderValueHandler::S_1( "1" );

	CFBamSaxLoaderValueHandler::CFBamSaxLoaderValueHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderValueHandler::~CFBamSaxLoaderValueHandler() {
	}

	void CFBamSaxLoaderValueHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamValueObj* tmpBuff = NULL;
		cfbam::ICFBamValueEditObj* editBuff = NULL;
		cfbam::ICFBamValueEditObj* editValue = NULL;
		cfbam::ICFBamValueObj* origValue = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamValueBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamValueBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getValueTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamValueEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getValueLoaderBehaviour();
			origValue = dynamic_cast<cfbam::ICFBamValueObj*>( schemaObj->getValueTableObj()->readValueByUNameIdx( refTenant->getRequiredId(),
			...
			if( origValue == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editValue = dynamic_cast<cfbam::ICFBamValueEditObj*>( origValue->beginEdit() );
						editValue->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editValue->setOptionalShortNameNull();
						}
						else {
							editValue->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editValue->setOptionalLabelNull();
						}
						else {
							editValue->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editValue->setOptionalShortDescriptionNull();
						}
						else {
							editValue->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editValue->setOptionalDescriptionNull();
						}
						else {
							editValue->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editValue->setOptionalDefaultXmlValueNull();
						}
						else {
							editValue->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editValue->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editValue->setOptionalGenerateIdNull();
						}
						else {
							editValue->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
			...
			if( editValue != NULL ) {
				if( origValue != NULL ) {
					...
					origValue = dynamic_cast<cfbam::ICFBamValueObj*>( editValue->create() );
				}
				editValue = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderValueHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
