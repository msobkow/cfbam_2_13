...
// Description: C++18 XML SAX2 Element Handler implementation for TimestampCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTimestampColHandler::CLASS_NAME( "CFBamSaxLoaderTimestampColHandler" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTimestampColHandler::S_1( "1" );

	CFBamSaxLoaderTimestampColHandler::CFBamSaxLoaderTimestampColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTimestampColHandler::~CFBamSaxLoaderTimestampColHandler() {
	}

	void CFBamSaxLoaderTimestampColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTimestampColObj* tmpBuff = NULL;
		cfbam::ICFBamTimestampColEditObj* editBuff = NULL;
		cfbam::ICFBamTimestampColEditObj* editTimestampCol = NULL;
		cfbam::ICFBamTimestampColObj* origTimestampCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTimestampColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTimestampColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTimestampColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTimestampColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTimestampColLoaderBehaviour();
			origTimestampCol = dynamic_cast<cfbam::ICFBamTimestampColObj*>( schemaObj->getTimestampColTableObj()->readTimestampColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTimestampCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTimestampCol = dynamic_cast<cfbam::ICFBamTimestampColEditObj*>( origTimestampCol->beginEdit() );
						editTimestampCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTimestampCol->setOptionalShortNameNull();
						}
						else {
							editTimestampCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTimestampCol->setOptionalLabelNull();
						}
						else {
							editTimestampCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTimestampCol->setOptionalShortDescriptionNull();
						}
						else {
							editTimestampCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTimestampCol->setOptionalDescriptionNull();
						}
						else {
							editTimestampCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTimestampCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTimestampCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTimestampCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTimestampCol->setOptionalGenerateIdNull();
						}
						else {
							editTimestampCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTimestampCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTimestampCol->setOptionalDbNameNull();
						}
						else {
							editTimestampCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTimestampCol->setOptionalDummyNull();
						}
						else {
							editTimestampCol->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTimestampCol != NULL ) {
				if( origTimestampCol != NULL ) {
					...
					origTimestampCol = dynamic_cast<cfbam::ICFBamTimestampColObj*>( editTimestampCol->create() );
				}
				editTimestampCol = NULL;
			}
			...
			curContext->setObj( origTimestampCol );
...
cleanup:
	...
	void CFBamSaxLoaderTimestampColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
