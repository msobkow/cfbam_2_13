...
// Description: C++18 XML SAX2 Element Handler implementation for ServerObjFunc
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderServerObjFuncHandler::CLASS_NAME( "CFBamSaxLoaderServerObjFuncHandler" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_True( "true" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_False( "false" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_No( "no" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_0( "0" );
	const std::string CFBamSaxLoaderServerObjFuncHandler::S_1( "1" );

	CFBamSaxLoaderServerObjFuncHandler::CFBamSaxLoaderServerObjFuncHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderServerObjFuncHandler::~CFBamSaxLoaderServerObjFuncHandler() {
	}

	void CFBamSaxLoaderServerObjFuncHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		static const std::string S_RetTable( "RetTable" );
		...
		cfbam::ICFBamServerObjFuncObj* tmpBuff = NULL;
		cfbam::ICFBamServerObjFuncEditObj* editBuff = NULL;
		cfbam::ICFBamServerObjFuncEditObj* editServerObjFunc = NULL;
		cfbam::ICFBamServerObjFuncObj* origServerObjFunc = NULL;
		...
		std::string* attrRetTable = NULL;
		// ServerObjFunc References
		cfbam::ICFBamTableObj* refRetTable = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamServerObjFuncBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamServerObjFuncBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getServerObjFuncTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamServerObjFuncEditObj*>( tmpBuff->beginEdit() );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( *attrLocalName == S_SpecificallyId ) {
				...
				else if( *attrLocalName == S_RetTable ) {
					...
					if( cppValue != NULL ) {
						attrRetTable = new std::string( *cppValue );
					}
					else {
						attrRetTable = NULL;
					}
			...
			if( attrRetTable != NULL ) {
				curContext->putNamedTag( S_RetTable, *attrRetTable );
			}
			...
			if( ( attrRetTable != NULL ) && ( attrRetTable->length() > 0 ) ) {
				refRetTable = dynamic_cast<cfbam::ICFBamTableObj*>( editBuff->getNamedObject( schemaObj->getTableTableObj()->getObjQualifyingClassCode(),
					*attrRetTable ) );
				if( refRetTable == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			loaderBehaviour = saxLoader->getServerObjFuncLoaderBehaviour();
			origServerObjFunc = dynamic_cast<cfbam::ICFBamServerObjFuncObj*>( schemaObj->getServerObjFuncTableObj()->readServerObjFuncByUNameIdx( refForTable->getRequiredTenantId(),
			...
			if( origServerObjFunc == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editServerObjFunc = dynamic_cast<cfbam::ICFBamServerObjFuncEditObj*>( origServerObjFunc->beginEdit() );
						editServerObjFunc->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editServerObjFunc->setOptionalShortNameNull();
						}
						else {
							editServerObjFunc->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editServerObjFunc->setOptionalLabelNull();
						}
						else {
							editServerObjFunc->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editServerObjFunc->setOptionalShortDescriptionNull();
						}
						else {
							editServerObjFunc->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editServerObjFunc->setOptionalDescriptionNull();
						}
						else {
							editServerObjFunc->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalSuffixNull() ) {
							editServerObjFunc->setOptionalSuffixNull();
						}
						else {
							editServerObjFunc->setOptionalSuffixValue( editBuff->getOptionalSuffixValue() );
						}
			...
			if( editServerObjFunc != NULL ) {
				if( origServerObjFunc != NULL ) {
					...
					origServerObjFunc = dynamic_cast<cfbam::ICFBamServerObjFuncObj*>( editServerObjFunc->create() );
				}
				editServerObjFunc = NULL;
			}
			...
			curContext->setObj( origServerObjFunc );
...
cleanup:
	...
	void CFBamSaxLoaderServerObjFuncHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
