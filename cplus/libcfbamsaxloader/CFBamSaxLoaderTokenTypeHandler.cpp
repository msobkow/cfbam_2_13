...
// Description: C++18 XML SAX2 Element Handler implementation for TokenType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTokenTypeHandler::CLASS_NAME( "CFBamSaxLoaderTokenTypeHandler" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTokenTypeHandler::S_1( "1" );

	CFBamSaxLoaderTokenTypeHandler::CFBamSaxLoaderTokenTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTokenTypeHandler::~CFBamSaxLoaderTokenTypeHandler() {
	}

	void CFBamSaxLoaderTokenTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTokenTypeObj* tmpBuff = NULL;
		cfbam::ICFBamTokenTypeEditObj* editBuff = NULL;
		cfbam::ICFBamTokenTypeEditObj* editTokenType = NULL;
		cfbam::ICFBamTokenTypeObj* origTokenType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTokenTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTokenTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTokenTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTokenTypeEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTokenTypeLoaderBehaviour();
			origTokenType = dynamic_cast<cfbam::ICFBamTokenTypeObj*>( schemaObj->getTokenTypeTableObj()->readTokenTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTokenType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTokenType = dynamic_cast<cfbam::ICFBamTokenTypeEditObj*>( origTokenType->beginEdit() );
						editTokenType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTokenType->setOptionalShortNameNull();
						}
						else {
							editTokenType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTokenType->setOptionalLabelNull();
						}
						else {
							editTokenType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTokenType->setOptionalShortDescriptionNull();
						}
						else {
							editTokenType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTokenType->setOptionalDescriptionNull();
						}
						else {
							editTokenType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTokenType->setOptionalDefaultXmlValueNull();
						}
						else {
							editTokenType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTokenType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTokenType->setOptionalGenerateIdNull();
						}
						else {
							editTokenType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTokenType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTokenType->setOptionalDbNameNull();
						}
						else {
							editTokenType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editTokenType->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editTokenType->setOptionalInitValueNull();
						}
						else {
							editTokenType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editTokenType != NULL ) {
				if( origTokenType != NULL ) {
					...
					origTokenType = dynamic_cast<cfbam::ICFBamTokenTypeObj*>( editTokenType->create() );
				}
				editTokenType = NULL;
			}
			...
			curContext->setObj( origTokenType );
...
cleanup:
	...
	void CFBamSaxLoaderTokenTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
