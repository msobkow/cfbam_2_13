...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element HPlusXMsgTableInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler::CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler::~CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler() {
	}

	void CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "HPlusXMsgTableInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableHPlusXMsgTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element HPlusXMsgTableInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalHppXMsgTableIncludeNull() || origBuff->getOptionalHppXMsgTableIncludeValue().empty() ) {
			editBuff->setOptionalHppXMsgTableIncludeValue( attrText );
		}
		else {
