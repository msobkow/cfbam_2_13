...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element HPlusObjMembers
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableHPlusObjMembersHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableHPlusObjMembersHandler::CFBamSaxLoaderTableHPlusObjMembersHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableHPlusObjMembersHandler::~CFBamSaxLoaderTableHPlusObjMembersHandler() {
	}

	void CFBamSaxLoaderTableHPlusObjMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "HPlusObjMembers" );
...
cleanup:
	...
	void CFBamSaxLoaderTableHPlusObjMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element HPlusObjMembers more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalHppObjMembersNull() || origBuff->getOptionalHppObjMembersValue().empty() ) {
			editBuff->setOptionalHppObjMembersValue( attrText );
		}
		else {
