...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusObjInterface
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusObjInterfaceHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusObjInterfaceHandler::CFBamSaxLoaderTableCPlusObjInterfaceHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusObjInterfaceHandler::~CFBamSaxLoaderTableCPlusObjInterfaceHandler() {
	}

	void CFBamSaxLoaderTableCPlusObjInterfaceHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusObjInterface" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusObjInterfaceHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusObjInterface more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppObjInterfaceNull() || origBuff->getOptionalCppObjInterfaceValue().empty() ) {
			editBuff->setOptionalCppObjInterfaceValue( attrText );
		}
		else {
