...
// Description: C++18 XML SAX2 Element Handler implementation for TZTimeType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTZTimeTypeHandler::CLASS_NAME( "CFBamSaxLoaderTZTimeTypeHandler" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTZTimeTypeHandler::S_1( "1" );

	CFBamSaxLoaderTZTimeTypeHandler::CFBamSaxLoaderTZTimeTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTZTimeTypeHandler::~CFBamSaxLoaderTZTimeTypeHandler() {
	}

	void CFBamSaxLoaderTZTimeTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTZTimeTypeObj* tmpBuff = NULL;
		cfbam::ICFBamTZTimeTypeEditObj* editBuff = NULL;
		cfbam::ICFBamTZTimeTypeEditObj* editTZTimeType = NULL;
		cfbam::ICFBamTZTimeTypeObj* origTZTimeType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTZTimeTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTZTimeTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTZTimeTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTZTimeTypeEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTZTimeTypeLoaderBehaviour();
			origTZTimeType = dynamic_cast<cfbam::ICFBamTZTimeTypeObj*>( schemaObj->getTZTimeTypeTableObj()->readTZTimeTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTZTimeType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTZTimeType = dynamic_cast<cfbam::ICFBamTZTimeTypeEditObj*>( origTZTimeType->beginEdit() );
						editTZTimeType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTZTimeType->setOptionalShortNameNull();
						}
						else {
							editTZTimeType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTZTimeType->setOptionalLabelNull();
						}
						else {
							editTZTimeType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTZTimeType->setOptionalShortDescriptionNull();
						}
						else {
							editTZTimeType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTZTimeType->setOptionalDescriptionNull();
						}
						else {
							editTZTimeType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTZTimeType->setOptionalDefaultXmlValueNull();
						}
						else {
							editTZTimeType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTZTimeType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTZTimeType->setOptionalGenerateIdNull();
						}
						else {
							editTZTimeType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTZTimeType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTZTimeType->setOptionalDbNameNull();
						}
						else {
							editTZTimeType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTZTimeType->setOptionalDummyNull();
						}
						else {
							editTZTimeType->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTZTimeType != NULL ) {
				if( origTZTimeType != NULL ) {
					...
					origTZTimeType = dynamic_cast<cfbam::ICFBamTZTimeTypeObj*>( editTZTimeType->create() );
				}
				editTZTimeType = NULL;
			}
			...
			curContext->setObj( origTZTimeType );
...
cleanup:
	...
	void CFBamSaxLoaderTZTimeTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
