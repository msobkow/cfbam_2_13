...
// Description: C++18 XML SAX2 Element Handler implementation for Atom
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderAtomHandler::CLASS_NAME( "CFBamSaxLoaderAtomHandler" );
	const std::string CFBamSaxLoaderAtomHandler::S_True( "true" );
	const std::string CFBamSaxLoaderAtomHandler::S_False( "false" );
	const std::string CFBamSaxLoaderAtomHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderAtomHandler::S_No( "no" );
	const std::string CFBamSaxLoaderAtomHandler::S_0( "0" );
	const std::string CFBamSaxLoaderAtomHandler::S_1( "1" );

	CFBamSaxLoaderAtomHandler::CFBamSaxLoaderAtomHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderAtomHandler::~CFBamSaxLoaderAtomHandler() {
	}

	void CFBamSaxLoaderAtomHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamAtomObj* tmpBuff = NULL;
		cfbam::ICFBamAtomEditObj* editBuff = NULL;
		cfbam::ICFBamAtomEditObj* editAtom = NULL;
		cfbam::ICFBamAtomObj* origAtom = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamAtomBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamAtomBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getAtomTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamAtomEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getAtomLoaderBehaviour();
			origAtom = dynamic_cast<cfbam::ICFBamAtomObj*>( schemaObj->getAtomTableObj()->readAtomByUNameIdx( refTenant->getRequiredId(),
			...
			if( origAtom == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editAtom = dynamic_cast<cfbam::ICFBamAtomEditObj*>( origAtom->beginEdit() );
						editAtom->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editAtom->setOptionalShortNameNull();
						}
						else {
							editAtom->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editAtom->setOptionalLabelNull();
						}
						else {
							editAtom->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editAtom->setOptionalShortDescriptionNull();
						}
						else {
							editAtom->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editAtom->setOptionalDescriptionNull();
						}
						else {
							editAtom->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editAtom->setOptionalDefaultXmlValueNull();
						}
						else {
							editAtom->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editAtom->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editAtom->setOptionalGenerateIdNull();
						}
						else {
							editAtom->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editAtom->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editAtom->setOptionalDbNameNull();
						}
						else {
							editAtom->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
			...
			if( editAtom != NULL ) {
				if( origAtom != NULL ) {
					...
					origAtom = dynamic_cast<cfbam::ICFBamAtomObj*>( editAtom->create() );
				}
				editAtom = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderAtomHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
