...
// Description: C++18 XML SAX2 Element Handler implementation for TimeCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTimeColHandler::CLASS_NAME( "CFBamSaxLoaderTimeColHandler" );
	const std::string CFBamSaxLoaderTimeColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTimeColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTimeColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTimeColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTimeColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTimeColHandler::S_1( "1" );

	CFBamSaxLoaderTimeColHandler::CFBamSaxLoaderTimeColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTimeColHandler::~CFBamSaxLoaderTimeColHandler() {
	}

	void CFBamSaxLoaderTimeColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTimeColObj* tmpBuff = NULL;
		cfbam::ICFBamTimeColEditObj* editBuff = NULL;
		cfbam::ICFBamTimeColEditObj* editTimeCol = NULL;
		cfbam::ICFBamTimeColObj* origTimeCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTimeColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTimeColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTimeColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTimeColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTimeColLoaderBehaviour();
			origTimeCol = dynamic_cast<cfbam::ICFBamTimeColObj*>( schemaObj->getTimeColTableObj()->readTimeColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTimeCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTimeCol = dynamic_cast<cfbam::ICFBamTimeColEditObj*>( origTimeCol->beginEdit() );
						editTimeCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTimeCol->setOptionalShortNameNull();
						}
						else {
							editTimeCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTimeCol->setOptionalLabelNull();
						}
						else {
							editTimeCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTimeCol->setOptionalShortDescriptionNull();
						}
						else {
							editTimeCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTimeCol->setOptionalDescriptionNull();
						}
						else {
							editTimeCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTimeCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTimeCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTimeCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTimeCol->setOptionalGenerateIdNull();
						}
						else {
							editTimeCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTimeCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTimeCol->setOptionalDbNameNull();
						}
						else {
							editTimeCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTimeCol->setOptionalDummyNull();
						}
						else {
							editTimeCol->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTimeCol != NULL ) {
				if( origTimeCol != NULL ) {
					...
					origTimeCol = dynamic_cast<cfbam::ICFBamTimeColObj*>( editTimeCol->create() );
				}
				editTimeCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderTimeColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
