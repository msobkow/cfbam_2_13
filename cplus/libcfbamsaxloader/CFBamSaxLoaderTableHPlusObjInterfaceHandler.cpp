...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element HPlusObjInterface
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableHPlusObjInterfaceHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableHPlusObjInterfaceHandler::CFBamSaxLoaderTableHPlusObjInterfaceHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableHPlusObjInterfaceHandler::~CFBamSaxLoaderTableHPlusObjInterfaceHandler() {
	}

	void CFBamSaxLoaderTableHPlusObjInterfaceHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "HPlusObjInterface" );
...
cleanup:
	...
	void CFBamSaxLoaderTableHPlusObjInterfaceHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element HPlusObjInterface more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalHppObjInterfaceNull() || origBuff->getOptionalHppObjInterfaceValue().empty() ) {
			editBuff->setOptionalHppObjInterfaceValue( attrText );
		}
		else {
