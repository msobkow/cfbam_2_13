...
// Description: C++18 XML SAX2 Element Handler implementation for NmTokensDef
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderNmTokensDefHandler::CLASS_NAME( "CFBamSaxLoaderNmTokensDefHandler" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_True( "true" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_False( "false" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_No( "no" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_0( "0" );
	const std::string CFBamSaxLoaderNmTokensDefHandler::S_1( "1" );

	CFBamSaxLoaderNmTokensDefHandler::CFBamSaxLoaderNmTokensDefHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderNmTokensDefHandler::~CFBamSaxLoaderNmTokensDefHandler() {
	}

	void CFBamSaxLoaderNmTokensDefHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamNmTokensDefObj* tmpBuff = NULL;
		cfbam::ICFBamNmTokensDefEditObj* editBuff = NULL;
		cfbam::ICFBamNmTokensDefEditObj* editNmTokensDef = NULL;
		cfbam::ICFBamNmTokensDefObj* origNmTokensDef = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamNmTokensDefBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamNmTokensDefBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getNmTokensDefTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamNmTokensDefEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getNmTokensDefLoaderBehaviour();
			origNmTokensDef = dynamic_cast<cfbam::ICFBamNmTokensDefObj*>( schemaObj->getNmTokensDefTableObj()->readNmTokensDefByUNameIdx( refTenant->getRequiredId(),
			...
			if( origNmTokensDef == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editNmTokensDef = dynamic_cast<cfbam::ICFBamNmTokensDefEditObj*>( origNmTokensDef->beginEdit() );
						editNmTokensDef->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editNmTokensDef->setOptionalShortNameNull();
						}
						else {
							editNmTokensDef->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editNmTokensDef->setOptionalLabelNull();
						}
						else {
							editNmTokensDef->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editNmTokensDef->setOptionalShortDescriptionNull();
						}
						else {
							editNmTokensDef->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editNmTokensDef->setOptionalDescriptionNull();
						}
						else {
							editNmTokensDef->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editNmTokensDef->setOptionalDefaultXmlValueNull();
						}
						else {
							editNmTokensDef->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editNmTokensDef->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editNmTokensDef->setOptionalGenerateIdNull();
						}
						else {
							editNmTokensDef->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editNmTokensDef->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editNmTokensDef->setOptionalDbNameNull();
						}
						else {
							editNmTokensDef->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editNmTokensDef->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editNmTokensDef->setOptionalInitValueNull();
						}
						else {
							editNmTokensDef->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editNmTokensDef != NULL ) {
				if( origNmTokensDef != NULL ) {
					...
					origNmTokensDef = dynamic_cast<cfbam::ICFBamNmTokensDefObj*>( editNmTokensDef->create() );
				}
				editNmTokensDef = NULL;
			}
			...
			curContext->setObj( origNmTokensDef );
...
cleanup:
	...
	void CFBamSaxLoaderNmTokensDefHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
