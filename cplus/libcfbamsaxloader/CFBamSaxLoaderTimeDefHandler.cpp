...
// Description: C++18 XML SAX2 Element Handler implementation for TimeDef
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTimeDefHandler::CLASS_NAME( "CFBamSaxLoaderTimeDefHandler" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTimeDefHandler::S_1( "1" );

	CFBamSaxLoaderTimeDefHandler::CFBamSaxLoaderTimeDefHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTimeDefHandler::~CFBamSaxLoaderTimeDefHandler() {
	}

	void CFBamSaxLoaderTimeDefHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTimeDefObj* tmpBuff = NULL;
		cfbam::ICFBamTimeDefEditObj* editBuff = NULL;
		cfbam::ICFBamTimeDefEditObj* editTimeDef = NULL;
		cfbam::ICFBamTimeDefObj* origTimeDef = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTimeDefBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTimeDefBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTimeDefTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTimeDefEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTimeDefLoaderBehaviour();
			origTimeDef = dynamic_cast<cfbam::ICFBamTimeDefObj*>( schemaObj->getTimeDefTableObj()->readTimeDefByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTimeDef == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTimeDef = dynamic_cast<cfbam::ICFBamTimeDefEditObj*>( origTimeDef->beginEdit() );
						editTimeDef->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTimeDef->setOptionalShortNameNull();
						}
						else {
							editTimeDef->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTimeDef->setOptionalLabelNull();
						}
						else {
							editTimeDef->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTimeDef->setOptionalShortDescriptionNull();
						}
						else {
							editTimeDef->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTimeDef->setOptionalDescriptionNull();
						}
						else {
							editTimeDef->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTimeDef->setOptionalDefaultXmlValueNull();
						}
						else {
							editTimeDef->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTimeDef->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTimeDef->setOptionalGenerateIdNull();
						}
						else {
							editTimeDef->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTimeDef->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTimeDef->setOptionalDbNameNull();
						}
						else {
							editTimeDef->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTimeDef->setOptionalDummyNull();
						}
						else {
							editTimeDef->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTimeDef != NULL ) {
				if( origTimeDef != NULL ) {
					...
					origTimeDef = dynamic_cast<cfbam::ICFBamTimeDefObj*>( editTimeDef->create() );
				}
				editTimeDef = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderTimeDefHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
