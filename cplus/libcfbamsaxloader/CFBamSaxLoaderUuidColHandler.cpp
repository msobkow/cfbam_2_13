...
// Description: C++18 XML SAX2 Element Handler implementation for UuidCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderUuidColHandler::CLASS_NAME( "CFBamSaxLoaderUuidColHandler" );
	const std::string CFBamSaxLoaderUuidColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderUuidColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderUuidColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderUuidColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderUuidColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderUuidColHandler::S_1( "1" );

	CFBamSaxLoaderUuidColHandler::CFBamSaxLoaderUuidColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderUuidColHandler::~CFBamSaxLoaderUuidColHandler() {
	}

	void CFBamSaxLoaderUuidColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamUuidColObj* tmpBuff = NULL;
		cfbam::ICFBamUuidColEditObj* editBuff = NULL;
		cfbam::ICFBamUuidColEditObj* editUuidCol = NULL;
		cfbam::ICFBamUuidColObj* origUuidCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamUuidColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamUuidColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getUuidColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamUuidColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getUuidColLoaderBehaviour();
			origUuidCol = dynamic_cast<cfbam::ICFBamUuidColObj*>( schemaObj->getUuidColTableObj()->readUuidColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origUuidCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editUuidCol = dynamic_cast<cfbam::ICFBamUuidColEditObj*>( origUuidCol->beginEdit() );
						editUuidCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editUuidCol->setOptionalShortNameNull();
						}
						else {
							editUuidCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editUuidCol->setOptionalLabelNull();
						}
						else {
							editUuidCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editUuidCol->setOptionalShortDescriptionNull();
						}
						else {
							editUuidCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editUuidCol->setOptionalDescriptionNull();
						}
						else {
							editUuidCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editUuidCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editUuidCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editUuidCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editUuidCol->setOptionalGenerateIdNull();
						}
						else {
							editUuidCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editUuidCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editUuidCol->setOptionalDbNameNull();
						}
						else {
							editUuidCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editUuidCol->setOptionalInitValueNull();
						}
						else {
							editUuidCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editUuidCol != NULL ) {
				if( origUuidCol != NULL ) {
					...
					origUuidCol = dynamic_cast<cfbam::ICFBamUuidColObj*>( editUuidCol->create() );
				}
				editUuidCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderUuidColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
