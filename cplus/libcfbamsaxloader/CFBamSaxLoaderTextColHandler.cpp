...
// Description: C++18 XML SAX2 Element Handler implementation for TextCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTextColHandler::CLASS_NAME( "CFBamSaxLoaderTextColHandler" );
	const std::string CFBamSaxLoaderTextColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTextColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTextColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTextColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTextColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTextColHandler::S_1( "1" );

	CFBamSaxLoaderTextColHandler::CFBamSaxLoaderTextColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTextColHandler::~CFBamSaxLoaderTextColHandler() {
	}

	void CFBamSaxLoaderTextColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTextColObj* tmpBuff = NULL;
		cfbam::ICFBamTextColEditObj* editBuff = NULL;
		cfbam::ICFBamTextColEditObj* editTextCol = NULL;
		cfbam::ICFBamTextColObj* origTextCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTextColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTextColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTextColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTextColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTextColLoaderBehaviour();
			origTextCol = dynamic_cast<cfbam::ICFBamTextColObj*>( schemaObj->getTextColTableObj()->readTextColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTextCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTextCol = dynamic_cast<cfbam::ICFBamTextColEditObj*>( origTextCol->beginEdit() );
						editTextCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTextCol->setOptionalShortNameNull();
						}
						else {
							editTextCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTextCol->setOptionalLabelNull();
						}
						else {
							editTextCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTextCol->setOptionalShortDescriptionNull();
						}
						else {
							editTextCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTextCol->setOptionalDescriptionNull();
						}
						else {
							editTextCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTextCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTextCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTextCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTextCol->setOptionalGenerateIdNull();
						}
						else {
							editTextCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTextCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTextCol->setOptionalDbNameNull();
						}
						else {
							editTextCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editTextCol->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editTextCol->setOptionalInitValueNull();
						}
						else {
							editTextCol->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalXmlElementNameNull() ) {
							editTextCol->setOptionalXmlElementNameNull();
						}
						else {
							editTextCol->setOptionalXmlElementNameValue( editBuff->getOptionalXmlElementNameValue() );
						}
			...
			if( editTextCol != NULL ) {
				if( origTextCol != NULL ) {
					...
					origTextCol = dynamic_cast<cfbam::ICFBamTextColObj*>( editTextCol->create() );
				}
				editTextCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderTextColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
