...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeRamTableImplementation
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeRamTableImplementationHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeRamTableImplementationHandler::CFBamSaxLoaderTableCafeRamTableImplementationHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeRamTableImplementationHandler::~CFBamSaxLoaderTableCafeRamTableImplementationHandler() {
	}

	void CFBamSaxLoaderTableCafeRamTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeRamTableImplementation" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeRamTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeRamTableImplementation more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJRamTableImplementationNull() || origBuff->getOptionalJRamTableImplementationValue().empty() ) {
			editBuff->setOptionalJRamTableImplementationValue( attrText );
		}
		else {
