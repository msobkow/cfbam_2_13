...
// Description: C++18 XML SAX2 Element Handler implementation for TZTimeCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTZTimeColHandler::CLASS_NAME( "CFBamSaxLoaderTZTimeColHandler" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTZTimeColHandler::S_1( "1" );

	CFBamSaxLoaderTZTimeColHandler::CFBamSaxLoaderTZTimeColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTZTimeColHandler::~CFBamSaxLoaderTZTimeColHandler() {
	}

	void CFBamSaxLoaderTZTimeColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTZTimeColObj* tmpBuff = NULL;
		cfbam::ICFBamTZTimeColEditObj* editBuff = NULL;
		cfbam::ICFBamTZTimeColEditObj* editTZTimeCol = NULL;
		cfbam::ICFBamTZTimeColObj* origTZTimeCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTZTimeColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTZTimeColBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTZTimeColTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTZTimeColEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTZTimeColLoaderBehaviour();
			origTZTimeCol = dynamic_cast<cfbam::ICFBamTZTimeColObj*>( schemaObj->getTZTimeColTableObj()->readTZTimeColByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTZTimeCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTZTimeCol = dynamic_cast<cfbam::ICFBamTZTimeColEditObj*>( origTZTimeCol->beginEdit() );
						editTZTimeCol->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTZTimeCol->setOptionalShortNameNull();
						}
						else {
							editTZTimeCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTZTimeCol->setOptionalLabelNull();
						}
						else {
							editTZTimeCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTZTimeCol->setOptionalShortDescriptionNull();
						}
						else {
							editTZTimeCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTZTimeCol->setOptionalDescriptionNull();
						}
						else {
							editTZTimeCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTZTimeCol->setOptionalDefaultXmlValueNull();
						}
						else {
							editTZTimeCol->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTZTimeCol->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTZTimeCol->setOptionalGenerateIdNull();
						}
						else {
							editTZTimeCol->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTZTimeCol->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTZTimeCol->setOptionalDbNameNull();
						}
						else {
							editTZTimeCol->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTZTimeCol->setOptionalDummyNull();
						}
						else {
							editTZTimeCol->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTZTimeCol != NULL ) {
				if( origTZTimeCol != NULL ) {
					...
					origTZTimeCol = dynamic_cast<cfbam::ICFBamTZTimeColObj*>( editTZTimeCol->create() );
				}
				editTZTimeCol = NULL;
			}
			...
			curContext->setObj( origTZTimeCol );
...
cleanup:
	...
	void CFBamSaxLoaderTZTimeColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
