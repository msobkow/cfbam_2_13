...
// Description: C++18 XML SAX2 Element Handler implementation for Chain
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderChainHandler::CLASS_NAME( "CFBamSaxLoaderChainHandler" );
	const std::string CFBamSaxLoaderChainHandler::S_True( "true" );
	const std::string CFBamSaxLoaderChainHandler::S_False( "false" );
	const std::string CFBamSaxLoaderChainHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderChainHandler::S_No( "no" );
	const std::string CFBamSaxLoaderChainHandler::S_0( "0" );
	const std::string CFBamSaxLoaderChainHandler::S_1( "1" );

	CFBamSaxLoaderChainHandler::CFBamSaxLoaderChainHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderChainHandler::~CFBamSaxLoaderChainHandler() {
	}

	void CFBamSaxLoaderChainHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamChainEditObj* editChain = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamChainBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamChainBuff::GENDEFNAME + "'" );
			...
			if( attrPrevRel != NULL ) {
				curContext->putNamedTag( S_PrevRel, *attrPrevRel );
			}
			if( attrNextRel != NULL ) {
				curContext->putNamedTag( S_NextRel, *attrNextRel );
			}
			...
			if( ( attrPrevRel != NULL ) && ( attrPrevRel->length() > 0 ) ) {
				refPrevRel = dynamic_cast<cfbam::ICFBamRelationObj*>( editBuff->getNamedObject( schemaObj->getRelationTableObj()->getObjQualifyingClassCode(),
					*attrPrevRel ) );
				if( refPrevRel == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			if( ( attrNextRel != NULL ) && ( attrNextRel->length() > 0 ) ) {
				refNextRel = dynamic_cast<cfbam::ICFBamRelationObj*>( editBuff->getNamedObject( schemaObj->getRelationTableObj()->getObjQualifyingClassCode(),
					*attrNextRel ) );
				if( refNextRel == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			loaderBehaviour = saxLoader->getChainLoaderBehaviour();
			origChain = dynamic_cast<cfbam::ICFBamChainObj*>( schemaObj->getChainTableObj()->readChainByUNameIdx( refTenant->getRequiredId(),
			...
			if( origChain == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editChain = dynamic_cast<cfbam::ICFBamChainEditObj*>( origChain->beginEdit() );
						editChain->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editChain->setOptionalShortNameNull();
						}
						else {
							editChain->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editChain->setOptionalLabelNull();
						}
						else {
							editChain->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editChain->setOptionalShortDescriptionNull();
						}
						else {
							editChain->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editChain->setOptionalDescriptionNull();
						}
						else {
							editChain->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalSuffixNull() ) {
							editChain->setOptionalSuffixNull();
						}
						else {
							editChain->setOptionalSuffixValue( editBuff->getOptionalSuffixValue() );
						}
			...
			if( editChain != NULL ) {
				if( origChain != NULL ) {
					...
					origChain = dynamic_cast<cfbam::ICFBamChainObj*>( editChain->create() );
				}
				editChain = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderChainHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
