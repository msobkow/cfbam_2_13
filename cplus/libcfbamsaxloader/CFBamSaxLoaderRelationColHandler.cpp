...
// Description: C++18 XML SAX2 Element Handler implementation for RelationCol
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderRelationColHandler::CLASS_NAME( "CFBamSaxLoaderRelationColHandler" );
	const std::string CFBamSaxLoaderRelationColHandler::S_True( "true" );
	const std::string CFBamSaxLoaderRelationColHandler::S_False( "false" );
	const std::string CFBamSaxLoaderRelationColHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderRelationColHandler::S_No( "no" );
	const std::string CFBamSaxLoaderRelationColHandler::S_0( "0" );
	const std::string CFBamSaxLoaderRelationColHandler::S_1( "1" );

	CFBamSaxLoaderRelationColHandler::CFBamSaxLoaderRelationColHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderRelationColHandler::~CFBamSaxLoaderRelationColHandler() {
	}

	void CFBamSaxLoaderRelationColHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		static const std::string S_FromCol( "FromCol" );
		...
		cfbam::ICFBamRelationColEditObj* editRelationCol = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamRelationColBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamRelationColBuff::GENDEFNAME + "'" );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( *attrLocalName == S_SpecificallyId ) {
				...
				else if( *attrLocalName == S_FromCol ) {
					if( attrFromCol != NULL ) {
						throw cflib::CFLibUniqueIndexViolationException( CLASS_NAME,
							S_ProcName,
							*attrLocalName );
					...
					if( cppValue != NULL ) {
						attrFromCol = new std::string( *cppValue );
					}
					else {
						attrFromCol = NULL;
					}
			...
			if( attrFromCol != NULL ) {
				curContext->putNamedTag( S_FromCol, *attrFromCol );
			}
			if( attrToCol != NULL ) {
				curContext->putNamedTag( S_ToCol, *attrToCol );
			}
			...
			if( ( attrFromCol != NULL ) && ( attrFromCol->length() > 0 ) ) {
				refFromCol = dynamic_cast<cfbam::ICFBamIndexColObj*>( editBuff->getNamedObject( schemaObj->getIndexColTableObj()->getObjQualifyingClassCode(),
					*attrFromCol ) );
				if( refFromCol == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			if( ( attrToCol != NULL ) && ( attrToCol->length() > 0 ) ) {
				refToCol = dynamic_cast<cfbam::ICFBamIndexColObj*>( editBuff->getNamedObject( schemaObj->getIndexColTableObj()->getObjQualifyingClassCode(),
					*attrToCol ) );
				if( refToCol == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
			...
			loaderBehaviour = saxLoader->getRelationColLoaderBehaviour();
			origRelationCol = dynamic_cast<cfbam::ICFBamRelationColObj*>( schemaObj->getRelationColTableObj()->readRelationColByUNameIdx( refRelation->getRequiredTenantId(),
			...
			if( origRelationCol == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editRelationCol = dynamic_cast<cfbam::ICFBamRelationColEditObj*>( origRelationCol->beginEdit() );
						...
						if( editBuff->isOptionalShortNameNull() ) {
							...
							editRelationCol->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editRelationCol->setOptionalLabelNull();
						}
						else {
							editRelationCol->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editRelationCol->setOptionalShortDescriptionNull();
						}
						else {
							editRelationCol->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editRelationCol->setOptionalDescriptionNull();
						}
						else {
							editRelationCol->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
			...
			if( editRelationCol != NULL ) {
				if( origRelationCol != NULL ) {
					...
					origRelationCol = dynamic_cast<cfbam::ICFBamRelationColObj*>( editRelationCol->create() );
				}
				editRelationCol = NULL;
			}
...
cleanup:
	...
	void CFBamSaxLoaderRelationColHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
