...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusTableInterface
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusTableInterfaceHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusTableInterfaceHandler::CFBamSaxLoaderTableCPlusTableInterfaceHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusTableInterfaceHandler::~CFBamSaxLoaderTableCPlusTableInterfaceHandler() {
	}

	void CFBamSaxLoaderTableCPlusTableInterfaceHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusTableInterface" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusTableInterfaceHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusTableInterface more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppTableInterfaceNull() || origBuff->getOptionalCppTableInterfaceValue().empty() ) {
			editBuff->setOptionalCppTableInterfaceValue( attrText );
		}
		else {
