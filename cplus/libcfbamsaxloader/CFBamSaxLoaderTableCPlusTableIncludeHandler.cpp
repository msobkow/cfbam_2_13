...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusTableInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusTableIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusTableIncludeHandler::CFBamSaxLoaderTableCPlusTableIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusTableIncludeHandler::~CFBamSaxLoaderTableCPlusTableIncludeHandler() {
	}

	void CFBamSaxLoaderTableCPlusTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusTableInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusTableInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppTableIncludeNull() || origBuff->getOptionalCppTableIncludeValue().empty() ) {
			editBuff->setOptionalCppTableIncludeValue( attrText );
		}
		else {
