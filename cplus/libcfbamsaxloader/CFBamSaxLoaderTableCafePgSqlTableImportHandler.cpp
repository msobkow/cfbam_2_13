...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafePgSqlTableImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafePgSqlTableImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafePgSqlTableImportHandler::CFBamSaxLoaderTableCafePgSqlTableImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafePgSqlTableImportHandler::~CFBamSaxLoaderTableCafePgSqlTableImportHandler() {
	}

	void CFBamSaxLoaderTableCafePgSqlTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafePgSqlTableImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafePgSqlTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafePgSqlTableImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJPgSqlTableImportNull() || origBuff->getOptionalJPgSqlTableImportValue().empty() ) {
			editBuff->setOptionalJPgSqlTableImportValue( attrText );
		}
		else {
