...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusEditObjInterace
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusEditObjInteraceHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusEditObjInteraceHandler::CFBamSaxLoaderTableCPlusEditObjInteraceHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusEditObjInteraceHandler::~CFBamSaxLoaderTableCPlusEditObjInteraceHandler() {
	}

	void CFBamSaxLoaderTableCPlusEditObjInteraceHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusEditObjInterace" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusEditObjInteraceHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusEditObjInterace more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppEditObjInterfaceNull() || origBuff->getOptionalCppEditObjInterfaceValue().empty() ) {
			editBuff->setOptionalCppEditObjInterfaceValue( attrText );
		}
		else {
