...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element HPlusTableInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableHPlusTableIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableHPlusTableIncludeHandler::CFBamSaxLoaderTableHPlusTableIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableHPlusTableIncludeHandler::~CFBamSaxLoaderTableHPlusTableIncludeHandler() {
	}

	void CFBamSaxLoaderTableHPlusTableIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "HPlusTableInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableHPlusTableIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element HPlusTableInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalHppTableIncludeNull() || origBuff->getOptionalHppTableIncludeValue().empty() ) {
			editBuff->setOptionalHppTableIncludeValue( attrText );
		}
		else {
