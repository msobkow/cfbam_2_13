...
// Description: C++18 XML SAX2 Element Handler implementation for TimestampType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTimestampTypeHandler::CLASS_NAME( "CFBamSaxLoaderTimestampTypeHandler" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTimestampTypeHandler::S_1( "1" );

	CFBamSaxLoaderTimestampTypeHandler::CFBamSaxLoaderTimestampTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTimestampTypeHandler::~CFBamSaxLoaderTimestampTypeHandler() {
	}

	void CFBamSaxLoaderTimestampTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTimestampTypeObj* tmpBuff = NULL;
		cfbam::ICFBamTimestampTypeEditObj* editBuff = NULL;
		cfbam::ICFBamTimestampTypeEditObj* editTimestampType = NULL;
		cfbam::ICFBamTimestampTypeObj* origTimestampType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTimestampTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTimestampTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTimestampTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTimestampTypeEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTimestampTypeLoaderBehaviour();
			origTimestampType = dynamic_cast<cfbam::ICFBamTimestampTypeObj*>( schemaObj->getTimestampTypeTableObj()->readTimestampTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTimestampType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTimestampType = dynamic_cast<cfbam::ICFBamTimestampTypeEditObj*>( origTimestampType->beginEdit() );
						editTimestampType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTimestampType->setOptionalShortNameNull();
						}
						else {
							editTimestampType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTimestampType->setOptionalLabelNull();
						}
						else {
							editTimestampType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTimestampType->setOptionalShortDescriptionNull();
						}
						else {
							editTimestampType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTimestampType->setOptionalDescriptionNull();
						}
						else {
							editTimestampType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTimestampType->setOptionalDefaultXmlValueNull();
						}
						else {
							editTimestampType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTimestampType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTimestampType->setOptionalGenerateIdNull();
						}
						else {
							editTimestampType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTimestampType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTimestampType->setOptionalDbNameNull();
						}
						else {
							editTimestampType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalDummyNull() ) {
							editTimestampType->setOptionalDummyNull();
						}
						else {
							editTimestampType->setOptionalDummyValue( editBuff->getOptionalDummyValue() );
						}
			...
			if( editTimestampType != NULL ) {
				if( origTimestampType != NULL ) {
					...
					origTimestampType = dynamic_cast<cfbam::ICFBamTimestampTypeObj*>( editTimestampType->create() );
				}
				editTimestampType = NULL;
			}
			...
			curContext->setObj( origTimestampType );
...
cleanup:
	...
	void CFBamSaxLoaderTimestampTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
