...
// Description: C++18 XML SAX2 Element Handler implementation for UuidType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderUuidTypeHandler::CLASS_NAME( "CFBamSaxLoaderUuidTypeHandler" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderUuidTypeHandler::S_1( "1" );

	CFBamSaxLoaderUuidTypeHandler::CFBamSaxLoaderUuidTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderUuidTypeHandler::~CFBamSaxLoaderUuidTypeHandler() {
	}

	void CFBamSaxLoaderUuidTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamUuidTypeObj* tmpBuff = NULL;
		cfbam::ICFBamUuidTypeEditObj* editBuff = NULL;
		cfbam::ICFBamUuidTypeEditObj* editUuidType = NULL;
		cfbam::ICFBamUuidTypeObj* origUuidType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamUuidTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamUuidTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getUuidTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamUuidTypeEditObj*>( tmpBuff->beginEdit() );
			...
			if( ( attrInitValue == NULL ) || ( attrInitValue->length() == 0 ) ) {
				...
				cflib::CFLib::uuidFromString( natInitValue, attrInitValue->data() );
			}
			if( ( attrInitValue != NULL ) && ( attrInitValue->length() > 0 ) ) {
			...
			loaderBehaviour = saxLoader->getUuidTypeLoaderBehaviour();
			origUuidType = dynamic_cast<cfbam::ICFBamUuidTypeObj*>( schemaObj->getUuidTypeTableObj()->readUuidTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origUuidType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editUuidType = dynamic_cast<cfbam::ICFBamUuidTypeEditObj*>( origUuidType->beginEdit() );
						editUuidType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editUuidType->setOptionalShortNameNull();
						}
						else {
							editUuidType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editUuidType->setOptionalLabelNull();
						}
						else {
							editUuidType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editUuidType->setOptionalShortDescriptionNull();
						}
						else {
							editUuidType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editUuidType->setOptionalDescriptionNull();
						}
						else {
							editUuidType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editUuidType->setOptionalDefaultXmlValueNull();
						}
						else {
							editUuidType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editUuidType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editUuidType->setOptionalGenerateIdNull();
						}
						else {
							editUuidType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editUuidType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editUuidType->setOptionalDbNameNull();
						}
						else {
							editUuidType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						if( editBuff->isOptionalInitValueNull() ) {
							editUuidType->setOptionalInitValueNull();
						}
						else {
							editUuidType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
			...
			if( editUuidType != NULL ) {
				if( origUuidType != NULL ) {
					...
					origUuidType = dynamic_cast<cfbam::ICFBamUuidTypeObj*>( editUuidType->create() );
				}
				editUuidType = NULL;
			}
			...
			curContext->setObj( origUuidType );
...
cleanup:
	...
	void CFBamSaxLoaderUuidTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
