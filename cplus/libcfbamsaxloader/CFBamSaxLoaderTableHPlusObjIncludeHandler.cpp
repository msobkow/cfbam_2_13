...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element HPlusObjInclude
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableHPlusObjIncludeHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableHPlusObjIncludeHandler::CFBamSaxLoaderTableHPlusObjIncludeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableHPlusObjIncludeHandler::~CFBamSaxLoaderTableHPlusObjIncludeHandler() {
	}

	void CFBamSaxLoaderTableHPlusObjIncludeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "HPlusObjInclude" );
...
cleanup:
	...
	void CFBamSaxLoaderTableHPlusObjIncludeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element HPlusObjInclude more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalHppObjIncludeNull() || origBuff->getOptionalHppObjIncludeValue().empty() ) {
			editBuff->setOptionalHppObjIncludeValue( attrText );
		}
		else {
