...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeTableMembers
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeTableMembersHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeTableMembersHandler::CFBamSaxLoaderTableCafeTableMembersHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeTableMembersHandler::~CFBamSaxLoaderTableCafeTableMembersHandler() {
	}

	void CFBamSaxLoaderTableCafeTableMembersHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeTableMembers" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeTableMembersHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeTableMembers more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJTableMembersNull() || origBuff->getOptionalJTableMembersValue().empty() ) {
			editBuff->setOptionalJTableMembersValue( attrText );
		}
		else {
