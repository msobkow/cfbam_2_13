...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CPlusTableImplementation
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCPlusTableImplementationHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCPlusTableImplementationHandler::CFBamSaxLoaderTableCPlusTableImplementationHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCPlusTableImplementationHandler::~CFBamSaxLoaderTableCPlusTableImplementationHandler() {
	}

	void CFBamSaxLoaderTableCPlusTableImplementationHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CPlusTableImplementation" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCPlusTableImplementationHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CPlusTableImplementation more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalCppTableImplementationNull() || origBuff->getOptionalCppTableImplementationValue().empty() ) {
			editBuff->setOptionalCppTableImplementationValue( attrText );
		}
		else {
