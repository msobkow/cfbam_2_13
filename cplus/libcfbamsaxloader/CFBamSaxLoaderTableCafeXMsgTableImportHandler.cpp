...
// Description: C++18 XML SAX2 Element Handler implementation for Table sub-element CafeXMsgTableImport
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTableCafeXMsgTableImportHandler::CLASS_NAME( "CFBamSaxLoaderTableHandler" );

	CFBamSaxLoaderTableCafeXMsgTableImportHandler::CFBamSaxLoaderTableCafeXMsgTableImportHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTableCafeXMsgTableImportHandler::~CFBamSaxLoaderTableCafeXMsgTableImportHandler() {
	}

	void CFBamSaxLoaderTableCafeXMsgTableImportHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		const static std::string S_XmlElementName( "CafeXMsgTableImport" );
...
cleanup:
	...
	void CFBamSaxLoaderTableCafeXMsgTableImportHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
		...
		static const std::string S_OnlySetOnceMsg( "Cannot specify Table sub-element CafeXMsgTableImport more than once per object" );
		CFLIB_EXCEPTION_DECLINFO
		cflib::CFLibXmlCoreContext* curContext = NULL;
		cfbam::CFBamSaxLoader* saxLoader = NULL;
		cfbam::ICFBamSchemaObj* schemaObj = NULL;
		...
		if( origBuff->isOptionalJXMsgTableImportNull() || origBuff->getOptionalJXMsgTableImportValue().empty() ) {
			editBuff->setOptionalJXMsgTableImportValue( attrText );
		}
		else {
