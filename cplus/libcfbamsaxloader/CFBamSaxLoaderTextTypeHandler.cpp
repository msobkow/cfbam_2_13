...
// Description: C++18 XML SAX2 Element Handler implementation for TextType
...
namespace cfbam {
	...
	const std::string CFBamSaxLoaderTextTypeHandler::CLASS_NAME( "CFBamSaxLoaderTextTypeHandler" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_True( "true" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_False( "false" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_Yes( "yes" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_No( "no" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_0( "0" );
	const std::string CFBamSaxLoaderTextTypeHandler::S_1( "1" );

	CFBamSaxLoaderTextTypeHandler::CFBamSaxLoaderTextTypeHandler( cfbam::CFBamSaxLoader* saxLoader )
	: cflib::CFLibXmlCoreElementHandler( saxLoader )
	{
	}

	CFBamSaxLoaderTextTypeHandler::~CFBamSaxLoaderTextTypeHandler() {
	}

	void CFBamSaxLoaderTextTypeHandler::startElement (
		...
		const xercesc::Attributes& attrs )
	{
		...
		cfbam::ICFBamTextTypeObj* tmpBuff = NULL;
		cfbam::ICFBamTextTypeEditObj* editBuff = NULL;
		cfbam::ICFBamTextTypeEditObj* editTextType = NULL;
		cfbam::ICFBamTextTypeObj* origTextType = NULL;
		...
		try {
			...
			cQName = NULL;
			if( cppQName != cfbam::CFBamTextTypeBuff::GENDEFNAME ) {
				std::string Msg( "Expected QName to be '" + cfbam::CFBamTextTypeBuff::GENDEFNAME + "'" );
			...
			tmpBuff = schemaObj->getTextTypeTableObj()->newInstance();
			editBuff = dynamic_cast<cfbam::ICFBamTextTypeEditObj*>( tmpBuff->beginEdit() );
			...
			loaderBehaviour = saxLoader->getTextTypeLoaderBehaviour();
			origTextType = dynamic_cast<cfbam::ICFBamTextTypeObj*>( schemaObj->getTextTypeTableObj()->readTextTypeByUNameIdx( refTenant->getRequiredId(),
			...
			if( origTextType == NULL ) {
				...
				switch( loaderBehaviour ) {
					...
					case cfbam::CFBamSaxLoader::LoaderBehaviourEnum::Update:
						editTextType = dynamic_cast<cfbam::ICFBamTextTypeEditObj*>( origTextType->beginEdit() );
						editTextType->setRequiredName( editBuff->getRequiredName() );
						if( editBuff->isOptionalShortNameNull() ) {
							editTextType->setOptionalShortNameNull();
						}
						else {
							editTextType->setOptionalShortNameValue( editBuff->getOptionalShortNameValue() );
						}
						if( editBuff->isOptionalLabelNull() ) {
							editTextType->setOptionalLabelNull();
						}
						else {
							editTextType->setOptionalLabelValue( editBuff->getOptionalLabelValue() );
						}
						if( editBuff->isOptionalShortDescriptionNull() ) {
							editTextType->setOptionalShortDescriptionNull();
						}
						else {
							editTextType->setOptionalShortDescriptionValue( editBuff->getOptionalShortDescriptionValue() );
						}
						if( editBuff->isOptionalDescriptionNull() ) {
							editTextType->setOptionalDescriptionNull();
						}
						else {
							editTextType->setOptionalDescriptionValue( editBuff->getOptionalDescriptionValue() );
						}
						if( editBuff->isOptionalDefaultXmlValueNull() ) {
							editTextType->setOptionalDefaultXmlValueNull();
						}
						else {
							editTextType->setOptionalDefaultXmlValueValue( editBuff->getOptionalDefaultXmlValueValue() );
						}
						editTextType->setRequiredIsNullable( editBuff->getRequiredIsNullable() );
						if( editBuff->isOptionalGenerateIdNull() ) {
							editTextType->setOptionalGenerateIdNull();
						}
						else {
							editTextType->setOptionalGenerateIdValue( editBuff->getOptionalGenerateIdValue() );
						}
						editTextType->setRequiredImplementsPolymorph( editBuff->getRequiredImplementsPolymorph() );
						if( editBuff->isOptionalDbNameNull() ) {
							editTextType->setOptionalDbNameNull();
						}
						else {
							editTextType->setOptionalDbNameValue( editBuff->getOptionalDbNameValue() );
						}
						editTextType->setRequiredMaxLen( editBuff->getRequiredMaxLen() );
						if( editBuff->isOptionalInitValueNull() ) {
							editTextType->setOptionalInitValueNull();
						}
						else {
							editTextType->setOptionalInitValueValue( editBuff->getOptionalInitValueValue() );
						}
						if( editBuff->isOptionalXmlElementNameNull() ) {
							editTextType->setOptionalXmlElementNameNull();
						}
						else {
							editTextType->setOptionalXmlElementNameValue( editBuff->getOptionalXmlElementNameValue() );
						}
			...
			if( editTextType != NULL ) {
				if( origTextType != NULL ) {
					...
					origTextType = dynamic_cast<cfbam::ICFBamTextTypeObj*>( editTextType->create() );
				}
				editTextType = NULL;
			}
			...
			curContext->setObj( origTextType );
...
cleanup:
	...
	void CFBamSaxLoaderTextTypeHandler::endElement(
		const XMLCh* const uri,
		const XMLCh* const localname,
		const XMLCh* const qname )
	{
