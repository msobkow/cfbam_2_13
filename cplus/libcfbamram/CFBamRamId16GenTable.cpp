...
// Description: C++18 Implementation for an in-memory RAM DbIO for Id16Gen.
...
namespace cfbam {
	...
	const std::string CFBamRamId16GenTable::CLASS_NAME( "CFBamRamId16GenTable" );

	CFBamRamId16GenTable::CFBamRamId16GenTable( ICFBamSchema* argSchema )
	: cfbam::ICFBamId16GenTable()
	{
		schema = argSchema;
		dictByPKey = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>();
		dictByDispIdx = new std::map<cfbam::CFBamId16GenByDispIdxKey,
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamId16GenBuff*>*>();
	}

	CFBamRamId16GenTable::~CFBamRamId16GenTable() {
		if( dictByDispIdx != NULL ) {
			for( auto iterDict = dictByDispIdx->begin(); iterDict != dictByDispIdx->end(); iterDict ++ ) {
				delete iterDict->second;
				iterDict->second = NULL;
			}
			delete dictByDispIdx;
			dictByDispIdx = NULL;
		}
		if( dictByPKey != NULL ) {
			delete dictByPKey;
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::createId16Gen( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamId16GenBuff* Buff )
	{
		static const std::string S_ProcName( "createId16Gen" );
		cfbam::CFBamValueBuff* tail = NULL;
		if( Buff->getClassCode() == cfbam::CFBamId16GenBuff::CLASS_CODE ) {
		...
		if( Buff->isOptionalDispenserTenantIdNull() ) {
			keyDispIdx.setOptionalDispenserTenantIdNull();
		}
		else {
			keyDispIdx.setOptionalDispenserTenantIdValue( Buff->getOptionalDispenserTenantIdValue() );
		}
		if( Buff->isOptionalDispenserIdNull() ) {
			keyDispIdx.setOptionalDispenserIdNull();
		}
		else {
			keyDispIdx.setOptionalDispenserIdValue( Buff->getOptionalDispenserIdValue() );
		}
		...
		cfbam::CFBamId16GenBuff* buff = dynamic_cast<cfbam::CFBamId16GenBuff*>( schema->getTableInt16Type()->createInt16Type( Authorization,
			Buff ) );
		pkey.setRequiredTenantId( buff->getRequiredTenantId() );
		pkey.setRequiredId( buff->getRequiredId() );
		dictByPKey->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>::value_type( pkey, buff ) );

		std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>* subdictDispIdx;
		auto searchDictByDispIdx = dictByDispIdx->find( keyDispIdx );
		if( searchDictByDispIdx != dictByDispIdx->end() ) {
			subdictDispIdx = searchDictByDispIdx->second;
		}
		else {
			subdictDispIdx = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>();
			dictByDispIdx->insert( std::map<cfbam::CFBamId16GenByDispIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamId16GenBuff*>*>::value_type( keyDispIdx, subdictDispIdx ) );
		}
		subdictDispIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>::value_type( pkey, buff ) );
		...
		if( buff->getClassCode() == cfbam::CFBamId16GenBuff::CLASS_CODE ) {
			buff = dynamic_cast<cfbam::CFBamId16GenBuff*>( buff->clone() );
		}
		return( buff );
	}

	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readDerived( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		...
		auto searchDictByPKey = dictByPKey->find( *PKey );
		if( searchDictByPKey != dictByPKey->end() ) {
			buff = dynamic_cast<cfbam::CFBamId16GenBuff*>( searchDictByPKey->second );
			if( buff != NULL ) {
				buff = dynamic_cast<cfbam::CFBamId16GenBuff*>( buff->clone() );
			}
		}
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::lockDerived( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readAllDerived( const cfsec::CFSecAuthorization* Authorization ) {
		static const std::string S_ProcName( "readAllDerived" );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> retVec;
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamId16GenBuff*>::iterator iter = dictByPKey->begin();
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamId16GenBuff*>::iterator end = dictByPKey->end();
		cfbam::CFBamId16GenBuff* clone;
		while( iter != end ) {
			clone = dynamic_cast<cfbam::CFBamId16GenBuff*>( iter->second->clone() );
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readDerivedByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			...
			const std::string& Name )
	{
		...
		else if( buff->implementsClassCode( cfbam::CFBamId16GenBuff::CLASS_CODE ) ) {
			return( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff ) );
		}
		else {
			delete buff;
			buff = NULL;
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId )
	{
		...
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		...
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			if( ( buff != NULL ) && ( buff->implementsClassCode( cfbam::CFBamId16GenBuff::CLASS_CODE ) ) ) {
				*iterBuffList = NULL;
				filteredList.push_back( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff ) );
			}
			iterBuffList ++;
		}
		return( filteredList );
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByNextIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedBySchemaIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t SchemaDefId )
	{
		static const std::string S_ProcName( "readDerivedBySchemaIdx" );
		std::TCFLibOwningVector<cfbam::CFBamInt16TypeBuff*> buffList = schema->getTableInt16Type()->readDerivedBySchemaIdx( Authorization,
			TenantId,
			SchemaDefId );
		cfbam::CFBamInt16TypeBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamInt16TypeBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamInt16TypeBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			if( ( buff != NULL ) && ( buff->implementsClassCode( cfbam::CFBamId16GenBuff::CLASS_CODE ) ) ) {
				*iterBuffList = NULL;
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readDerivedByDispIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DispenserTenantId,
			const int64_t* DispenserId )
	{
		static const std::string S_ProcName( "readDerivedByDispIdx" );
		...
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> recVec;
		cfbam::CFBamId16GenBuff* clone;
		auto searchDictByDispIdx = dictByDispIdx->find( key );
		if( searchDictByDispIdx != dictByDispIdx->end() ) {
			std::map<cfbam::CFBamValuePKey,
				 cfbam::CFBamId16GenBuff*>* subdictDispIdx = searchDictByDispIdx->second;
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamId16GenBuff*>::iterator iter = subdictDispIdx->begin();
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamId16GenBuff*>::iterator end = subdictDispIdx->end();
			while( iter != end ) {
				clone = dynamic_cast<cfbam::CFBamId16GenBuff*>( iter->second->clone() );
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readDerivedByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id )
	{
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readBuff( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		static const std::string S_ProcName( "readBuff" );
		cfbam::CFBamId16GenBuff* buff = readDerived( Authorization, PKey );
		if( buff != NULL ) {
			if( buff->getClassCode() != cfbam::CFBamId16GenBuff::CLASS_CODE ) {
				delete buff;
				buff = NULL;
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::lockBuff( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		static const std::string S_ProcName( "lockBuff" );
		cfbam::CFBamId16GenBuff* buff = lockDerived( Authorization, PKey );
		return( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff ) );
	}

	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readAllBuff( const cfsec::CFSecAuthorization* Authorization )
	{
		static const std::string S_ProcName( "readAllBuff" );
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readAllDerived( Authorization );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			iterBuffList ++;
			if( ( buff != NULL ) && ( buff->getClassCode() == cfbam::CFBamId16GenBuff::CLASS_CODE ) ) {
				filteredList.push_back( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff->clone() ) );
			}
		}
		return( filteredList );
	}

	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readBuffByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id )
	{
		static const std::string S_ProcName( "readBuffByIdIdx" );
		cfbam::CFBamId16GenBuff* buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		if( buff == NULL ) {
			return( NULL );
		}
		else if( buff->getClassCode() == cfbam::CFBamId16GenBuff::CLASS_CODE ) {
			return( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff ) );
		}
		else {
			delete buff;
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::readBuffByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name )
	{
		static const std::string S_ProcName( "readBuffByUNameIdx" );
		cfbam::CFBamId16GenBuff* buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( buff == NULL ) {
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId )
	{
		static const std::string S_ProcName( "readBuffByValTentIdx" );
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* PrevTenantId,
			const int64_t* PrevId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* NextTenantId,
			const int64_t* NextId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffBySchemaIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t SchemaDefId )
	{
		...
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedBySchemaIdx( Authorization,
			TenantId,
			SchemaDefId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			iterBuffList ++;
			if( ( buff != NULL ) && ( buff->getClassCode() == cfbam::CFBamInt16TypeBuff::CLASS_CODE ) ) {
				filteredList.push_back( dynamic_cast<cfbam::CFBamId16GenBuff*>( buff->clone() ) );
			}
		}
		return( filteredList );
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::readBuffByDispIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DispenserTenantId,
			const int64_t* DispenserId )
	{
		static const std::string S_ProcName( "readBuffByDispIdx" );
		cfbam::CFBamId16GenBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> buffList = readDerivedByDispIdx( Authorization,
			DispenserTenantId,
			DispenserId );
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::pageBuffBySchemaIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t SchemaDefId,
	...
	std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> CFBamRamId16GenTable::pageBuffByDispIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DispenserTenantId,
			const int64_t* DispenserId,
			const int64_t* priorTenantId,
			const int64_t* priorId )
	{
		static const std::string S_ProcName( "pageBuffByDispIdx" );
		// MSS TODO WORKING
		throw cflib::CFLibNotImplementedYetException( CLASS_NAME, S_ProcName );
	}

	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::moveBuffUp( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id,
		int32_t revision )
	{
		...
		if( ( cur->isOptionalPrevTenantIdNull() )
			...
			return( dynamic_cast<cfbam::CFBamId16GenBuff*>( cur ) );
		}
		...
		if( grandprev != NULL ) {
			...
			if( classCode == cfbam::CFBamValueBuff::CLASS_CODE ) {
				editGrandprev = schema->getTableValue()->updateValue( Authorization, dynamic_cast<cfbam::CFBamId16GenBuff*>( grandprev ) );
			}
		...
		return( dynamic_cast<cfbam::CFBamId16GenBuff*>( editCur ) );
	}

	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::moveBuffDown( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id,
		int32_t revision )
	{
		...
		if( editGrandnext != NULL ) {
			...
			if( classCode == cfbam::CFBamValueBuff::CLASS_CODE ) {
				editGrandnext = schema->getTableValue()->updateValue( Authorization, dynamic_cast<cfbam::CFBamId16GenBuff*>( editGrandnext ) );
			}
			else if( classCode == cfbam::CFBamAtomBuff::CLASS_CODE ) {
	...
	cfbam::CFBamId16GenBuff* CFBamRamId16GenTable::updateId16Gen( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamId16GenBuff* Buff )
	{
		...
		if( searchExisting == dictByPKey->end() ) {
			delete Buff;
			std::string Msg( "Existing record not found in Id16Gen for key " + pkey.toString() );
			throw cflib::CFLibStaleCacheDetectedException( CLASS_NAME,
				S_ProcName,
				Msg );
		}

		cfbam::CFBamId16GenBuff* existing = searchExisting->second;
		...
		cfbam::CFBamId16GenByDispIdxKey existingKeyDispIdx;
		if( existing->isOptionalDispenserTenantIdNull() ) {
			existingKeyDispIdx.setOptionalDispenserTenantIdNull();
		}
		else {
			existingKeyDispIdx.setOptionalDispenserTenantIdValue( existing->getOptionalDispenserTenantIdValue() );
		}
		if( existing->isOptionalDispenserIdNull() ) {
			existingKeyDispIdx.setOptionalDispenserIdNull();
		}
		else {
			existingKeyDispIdx.setOptionalDispenserIdValue( existing->getOptionalDispenserIdValue() );
		}
		cfbam::CFBamId16GenByDispIdxKey newKeyDispIdx;
		if( Buff->isOptionalDispenserTenantIdNull() ) {
			newKeyDispIdx.setOptionalDispenserTenantIdNull();
		}
		else {
			newKeyDispIdx.setOptionalDispenserTenantIdValue( Buff->getOptionalDispenserTenantIdValue() );
		}
		if( Buff->isOptionalDispenserIdNull() ) {
			newKeyDispIdx.setOptionalDispenserIdNull();
		}
		else {
			newKeyDispIdx.setOptionalDispenserIdValue( Buff->getOptionalDispenserIdValue() );
		}
...

		{
			...
			if( allNull ) {
				cfbam::CFBamInt16TypeBuff* chk = dynamic_cast<cfbam::CFBamInt16TypeBuff*>( schema->getTableInt16Type()->readDerivedByIdIdx( Authorization,
						Buff->getRequiredTenantId(),
						Buff->getRequiredId() ) );
				if( chk == NULL ) {
					delete Buff;
					std::string Msg( "Could not resolve Superclass relationship SuperClass to table Int16Type" );
		...
		// Update is valid
		existing = dynamic_cast<cfbam::CFBamId16GenBuff*>( schema->getTableInt16Type()->updateInt16Type( Authorization,
			Buff ) );
		std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>* subdict;
		auto removalDictByPKey = dictByPKey->find( pkey );
		if( removalDictByPKey != dictByPKey->end() ) {
			dictByPKey->erase( removalDictByPKey );
		}

		dictByPKey->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>::value_type( pkey, existing ) );

		std::map<cfbam::CFBamValuePKey,
			 cfbam::CFBamId16GenBuff*>* subdictDispIdx;
		auto searchDictByDispIdx = dictByDispIdx->find( existingKeyDispIdx );
		if( searchDictByDispIdx != dictByDispIdx->end() ) {
			subdictDispIdx = searchDictByDispIdx->second;
			auto searchSubDict = subdictDispIdx->find( pkey );
			if( searchSubDict != subdictDispIdx->end() ) {
				subdictDispIdx->erase( searchSubDict );
			}
			if( subdictDispIdx->size() <= 0 ) {
				delete subdictDispIdx;
				dictByDispIdx->erase( searchDictByDispIdx );
			}
			subdictDispIdx = NULL;
		}
		auto searchNewKeyDictByDispIdx = dictByDispIdx->find( newKeyDispIdx );
		if( searchNewKeyDictByDispIdx != dictByDispIdx->end() ) {
			subdictDispIdx = searchNewKeyDictByDispIdx->second;
		}
		else {
			subdictDispIdx = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>();
			dictByDispIdx->insert( std::map<cfbam::CFBamId16GenByDispIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamId16GenBuff*>*>::value_type( newKeyDispIdx, subdictDispIdx ) );
		}
		subdictDispIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamId16GenBuff*>::value_type( pkey, existing ) );
		...
		cfbam::CFBamId16GenBuff* buff;
		if( existing->getClassCode() == cfbam::CFBamId16GenBuff::CLASS_CODE ) {
			delete Buff;
			Buff = NULL;
			buff = dynamic_cast<cfbam::CFBamId16GenBuff*>( existing->clone() );
		}
		else {
			buff = existing;
		}
	...
	void CFBamRamId16GenTable::deleteId16Gen( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamId16GenBuff* Buff )
	{
		static const std::string S_ProcName( "deleteId16Gen" );
		...
		if( existing->isOptionalDispenserTenantIdNull() ) {
			...
			keyDispIdx.setOptionalDispenserTenantIdValue( existing->getOptionalDispenserTenantIdValue() );
		}
		if( existing->isOptionalDispenserIdNull() ) {
			keyDispIdx.setOptionalDispenserIdNull();
		}
		else {
			keyDispIdx.setOptionalDispenserIdValue( existing->getOptionalDispenserIdValue() );
		}
	...
	void CFBamRamId16GenTable::deleteId16GenByDispIdx( const cfsec::CFSecAuthorization* Authorization,
			...
			const int64_t* argDispenserId )
	{
		...
		if( argDispenserTenantId == NULL ) {
			...
			key.setOptionalDispenserTenantIdValue( *argDispenserTenantId );
		}
		if( argDispenserId == NULL ) {
			key.setOptionalDispenserIdNull();
		}
		else {
			key.setOptionalDispenserIdValue( *argDispenserId );
		}
	...
	{
		cfbam::CFBamId16GenBuff* cur = NULL;
		cfbam::CFBamId16GenBuff* rereadCur = NULL;
		bool anyNotNull = false;
		if( argKey->isOptionalDispenserTenantIdNull() ) {
			anyNotNull = true;
		}
		if( argKey->isOptionalDispenserIdNull() ) {
			anyNotNull = true;
		}
		if( ! anyNotNull ) {
			return;
		}
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*> matchSet;
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamId16GenBuff*>::iterator iter = dictByPKey->begin();
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamId16GenBuff*>::iterator end = dictByPKey->end();
		while( iter != end ) {
			cur = iter->second;
			iter ++;
			if( *dynamic_cast<cfbam::CFBamId16GenBuff*>( cur ) == *argKey ) {
				matchSet.push_back( dynamic_cast<cfbam::CFBamId16GenBuff*>( cur->clone() ) );
			}
		}
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterMatch = matchSet.begin();
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator matchEnd = matchSet.end();
	...
	void CFBamRamId16GenTable::deleteId16GenBySchemaIdx( const cfsec::CFSecAuthorization* Authorization,
		...
		const cfbam::CFBamInt16TypeBySchemaIdxKey* argKey )
	{
		...
		while( iter != end ) {
			cur = iter->second;
			iter ++;
			if( *dynamic_cast<cfbam::CFBamInt16TypeBuff*>( cur ) == *argKey ) {
				matchSet.push_back( dynamic_cast<cfbam::CFBamId16GenBuff*>( cur->clone() ) );
			}
		}
		std::TCFLibOwningVector<cfbam::CFBamId16GenBuff*>::iterator iterMatch = matchSet.begin();
	...
	void CFBamRamId16GenTable::deleteId16GenByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argId )
	{
		cfbam::CFBamValuePKey key;
	...
	void CFBamRamId16GenTable::deleteId16GenByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
	...
	void CFBamRamId16GenTable::deleteId16GenByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId )
	{
	...
	void CFBamRamId16GenTable::deleteId16GenByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId )
	{
	...
	void CFBamRamId16GenTable::deleteId16GenByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamId16GenTable::deleteId16GenByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamId16GenTable::deleteId16GenByNextIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamId16GenTable::deleteId16GenByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId,
			const int64_t* argPrevId )
	{
	...
	void CFBamRamId16GenTable::deleteId16GenByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId,
			const int64_t* argNextId )
	{
	...
	void CFBamRamId16GenTable::releasePreparedStatements() {
	}
