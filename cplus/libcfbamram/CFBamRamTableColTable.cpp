...
// Description: C++18 Implementation for an in-memory RAM DbIO for TableCol.
...
namespace cfbam {
	...
	const std::string CFBamRamTableColTable::CLASS_NAME( "CFBamRamTableColTable" );

	CFBamRamTableColTable::CFBamRamTableColTable( ICFBamSchema* argSchema )
	: cfbam::ICFBamTableColTable()
	{
		schema = argSchema;
		dictByPKey = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>();
		dictByTableIdx = new std::map<cfbam::CFBamTableColByTableIdxKey,
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>*>();
		dictByDataIdx = new std::map<cfbam::CFBamTableColByDataIdxKey,
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>*>();
	}

	CFBamRamTableColTable::~CFBamRamTableColTable() {
		...
		if( dictByDataIdx != NULL ) {
			for( auto iterDict = dictByDataIdx->begin(); iterDict != dictByDataIdx->end(); iterDict ++ ) {
				delete iterDict->second;
				iterDict->second = NULL;
			}
			delete dictByDataIdx;
			dictByDataIdx = NULL;
		}
		if( dictByPKey != NULL ) {
			delete dictByPKey;
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::createTableCol( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamTableColBuff* Buff )
	{
		static const std::string S_ProcName( "createTableCol" );
		cfbam::CFBamValueBuff* tail = NULL;
		if( Buff->getClassCode() == cfbam::CFBamTableColBuff::CLASS_CODE ) {
		...
		if( Buff->isOptionalDataTenantIdNull() ) {
			keyDataIdx.setOptionalDataTenantIdNull();
		}
		else {
			keyDataIdx.setOptionalDataTenantIdValue( Buff->getOptionalDataTenantIdValue() );
		}
		if( Buff->isOptionalDataIdNull() ) {
			keyDataIdx.setOptionalDataIdNull();
		}
		else {
			keyDataIdx.setOptionalDataIdValue( Buff->getOptionalDataIdValue() );
		}
		...
		cfbam::CFBamTableColBuff* buff = dynamic_cast<cfbam::CFBamTableColBuff*>( schema->getTableValue()->createValue( Authorization,
			Buff ) );
		pkey.setRequiredTenantId( buff->getRequiredTenantId() );
		pkey.setRequiredId( buff->getRequiredId() );
		dictByPKey->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, buff ) );

		std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>* subdictTableIdx;
		...
		if( searchDictByTableIdx != dictByTableIdx->end() ) {
			...
			subdictTableIdx = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>();
			dictByTableIdx->insert( std::map<cfbam::CFBamTableColByTableIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamTableColBuff*>*>::value_type( keyTableIdx, subdictTableIdx ) );
		}
		subdictTableIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, buff ) );

		std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>* subdictDataIdx;
		auto searchDictByDataIdx = dictByDataIdx->find( keyDataIdx );
		if( searchDictByDataIdx != dictByDataIdx->end() ) {
			subdictDataIdx = searchDictByDataIdx->second;
		}
		else {
			subdictDataIdx = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>();
			dictByDataIdx->insert( std::map<cfbam::CFBamTableColByDataIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamTableColBuff*>*>::value_type( keyDataIdx, subdictDataIdx ) );
		}
		subdictDataIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, buff ) );
		...
		if( buff->getClassCode() == cfbam::CFBamTableColBuff::CLASS_CODE ) {
			buff = dynamic_cast<cfbam::CFBamTableColBuff*>( buff->clone() );
		}
		return( buff );
	}

	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readDerived( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		...
		auto searchDictByPKey = dictByPKey->find( *PKey );
		if( searchDictByPKey != dictByPKey->end() ) {
			buff = dynamic_cast<cfbam::CFBamTableColBuff*>( searchDictByPKey->second );
			if( buff != NULL ) {
				buff = dynamic_cast<cfbam::CFBamTableColBuff*>( buff->clone() );
			}
		}
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::lockDerived( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readAllDerived( const cfsec::CFSecAuthorization* Authorization ) {
		static const std::string S_ProcName( "readAllDerived" );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> retVec;
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamTableColBuff*>::iterator iter = dictByPKey->begin();
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamTableColBuff*>::iterator end = dictByPKey->end();
		cfbam::CFBamTableColBuff* clone;
		while( iter != end ) {
			clone = dynamic_cast<cfbam::CFBamTableColBuff*>( iter->second->clone() );
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readDerivedByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			...
			const std::string& Name )
	{
		...
		else if( buff->implementsClassCode( cfbam::CFBamTableColBuff::CLASS_CODE ) ) {
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId )
	{
		...
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		...
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			if( ( buff != NULL ) && ( buff->implementsClassCode( cfbam::CFBamTableColBuff::CLASS_CODE ) ) ) {
				*iterBuffList = NULL;
				filteredList.push_back( dynamic_cast<cfbam::CFBamTableColBuff*>( buff ) );
			}
			iterBuffList ++;
		}
		return( filteredList );
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByNextIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId )
	{
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByTableIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t TableId )
	{
		...
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> recVec;
		cfbam::CFBamTableColBuff* clone;
		auto searchDictByTableIdx = dictByTableIdx->find( key );
		if( searchDictByTableIdx != dictByTableIdx->end() ) {
			std::map<cfbam::CFBamValuePKey,
				 cfbam::CFBamTableColBuff*>* subdictTableIdx = searchDictByTableIdx->second;
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>::iterator iter = subdictTableIdx->begin();
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>::iterator end = subdictTableIdx->end();
			while( iter != end ) {
				clone = dynamic_cast<cfbam::CFBamTableColBuff*>( iter->second->clone() );
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readDerivedByDataIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DataTenantId,
			const int64_t* DataId )
	{
		static const std::string S_ProcName( "readDerivedByDataIdx" );
		...
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> recVec;
		cfbam::CFBamTableColBuff* clone;
		auto searchDictByDataIdx = dictByDataIdx->find( key );
		if( searchDictByDataIdx != dictByDataIdx->end() ) {
			std::map<cfbam::CFBamValuePKey,
				 cfbam::CFBamTableColBuff*>* subdictDataIdx = searchDictByDataIdx->second;
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>::iterator iter = subdictDataIdx->begin();
			std::map<cfbam::CFBamValuePKey,
				cfbam::CFBamTableColBuff*>::iterator end = subdictDataIdx->end();
			while( iter != end ) {
				clone = dynamic_cast<cfbam::CFBamTableColBuff*>( iter->second->clone() );
				recVec.push_back( clone );
				iter ++;
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readDerivedByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id )
	{
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readBuff( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		static const std::string S_ProcName( "readBuff" );
		cfbam::CFBamTableColBuff* buff = readDerived( Authorization, PKey );
		if( buff != NULL ) {
			if( buff->getClassCode() != cfbam::CFBamTableColBuff::CLASS_CODE ) {
				delete buff;
				buff = NULL;
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::lockBuff( const cfsec::CFSecAuthorization* Authorization,
		const cfbam::CFBamValuePKey* PKey )
	{
		static const std::string S_ProcName( "lockBuff" );
		cfbam::CFBamTableColBuff* buff = lockDerived( Authorization, PKey );
		return( dynamic_cast<cfbam::CFBamTableColBuff*>( buff ) );
	}

	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readAllBuff( const cfsec::CFSecAuthorization* Authorization )
	{
		static const std::string S_ProcName( "readAllBuff" );
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readAllDerived( Authorization );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
			buff = *iterBuffList;
			iterBuffList ++;
			if( ( buff != NULL ) && ( buff->getClassCode() == cfbam::CFBamTableColBuff::CLASS_CODE ) ) {
				filteredList.push_back( dynamic_cast<cfbam::CFBamTableColBuff*>( buff->clone() ) );
			}
		}
		return( filteredList );
	}

	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readBuffByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id )
	{
		static const std::string S_ProcName( "readBuffByIdIdx" );
		cfbam::CFBamTableColBuff* buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		if( buff == NULL ) {
			return( NULL );
		}
		else if( buff->getClassCode() == cfbam::CFBamTableColBuff::CLASS_CODE ) {
			return( dynamic_cast<cfbam::CFBamTableColBuff*>( buff ) );
		}
		else {
			delete buff;
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::readBuffByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const std::string& Name )
	{
		static const std::string S_ProcName( "readBuffByUNameIdx" );
		cfbam::CFBamTableColBuff* buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( buff == NULL ) {
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId )
	{
		static const std::string S_ProcName( "readBuffByValTentIdx" );
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
		while( iterBuffList != endBuffList ) {
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DefSchemaTenantId,
			const int64_t* DefSchemaId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* PrevTenantId,
			const int64_t* PrevId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* NextTenantId,
			const int64_t* NextId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* PrevId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t ScopeId,
			const int64_t* NextId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByTableIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t TableId )
	{
		...
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByTableIdx( Authorization,
			TenantId,
			TableId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::readBuffByDataIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DataTenantId,
			const int64_t* DataId )
	{
		static const std::string S_ProcName( "readBuffByDataIdx" );
		cfbam::CFBamTableColBuff* buff;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> filteredList;
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> buffList = readDerivedByDataIdx( Authorization,
			DataTenantId,
			DataId );
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterBuffList = buffList.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator endBuffList = buffList.end();
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::pageBuffByTableIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t TableId,
	...
	std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> CFBamRamTableColTable::pageBuffByDataIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t* DataTenantId,
			const int64_t* DataId,
			const int64_t* priorTenantId,
			const int64_t* priorId )
	{
		static const std::string S_ProcName( "pageBuffByDataIdx" );
		// MSS TODO WORKING
		throw cflib::CFLibNotImplementedYetException( CLASS_NAME, S_ProcName );
	}

	cfbam::CFBamTableColBuff* CFBamRamTableColTable::moveBuffUp( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id,
		int32_t revision )
	{
		...
		if( ( cur->isOptionalPrevTenantIdNull() )
			...
			return( dynamic_cast<cfbam::CFBamTableColBuff*>( cur ) );
		}
		...
		if( grandprev != NULL ) {
			...
			if( classCode == cfbam::CFBamValueBuff::CLASS_CODE ) {
				editGrandprev = schema->getTableValue()->updateValue( Authorization, dynamic_cast<cfbam::CFBamTableColBuff*>( grandprev ) );
			}
		...
		return( dynamic_cast<cfbam::CFBamTableColBuff*>( editCur ) );
	}

	cfbam::CFBamTableColBuff* CFBamRamTableColTable::moveBuffDown( const cfsec::CFSecAuthorization* Authorization,
			const int64_t TenantId,
			const int64_t Id,
		int32_t revision )
	{
		...
		if( editGrandnext != NULL ) {
			...
			if( classCode == cfbam::CFBamValueBuff::CLASS_CODE ) {
				editGrandnext = schema->getTableValue()->updateValue( Authorization, dynamic_cast<cfbam::CFBamTableColBuff*>( editGrandnext ) );
			}
			else if( classCode == cfbam::CFBamAtomBuff::CLASS_CODE ) {
	...
	cfbam::CFBamTableColBuff* CFBamRamTableColTable::updateTableCol( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamTableColBuff* Buff )
	{
		...
		if( searchExisting == dictByPKey->end() ) {
			delete Buff;
			std::string Msg( "Existing record not found in TableCol for key " + pkey.toString() );
			throw cflib::CFLibStaleCacheDetectedException( CLASS_NAME,
				S_ProcName,
				Msg );
		}

		cfbam::CFBamTableColBuff* existing = searchExisting->second;
		...
		cfbam::CFBamTableColByTableIdxKey existingKeyTableIdx;
		...
		cfbam::CFBamTableColByTableIdxKey newKeyTableIdx;
		newKeyTableIdx.setRequiredTenantId( Buff->getRequiredTenantId() );
		newKeyTableIdx.setRequiredTableId( Buff->getRequiredTableId() );
		cfbam::CFBamTableColByDataIdxKey existingKeyDataIdx;
		if( existing->isOptionalDataTenantIdNull() ) {
			existingKeyDataIdx.setOptionalDataTenantIdNull();
		}
		else {
			existingKeyDataIdx.setOptionalDataTenantIdValue( existing->getOptionalDataTenantIdValue() );
		}
		if( existing->isOptionalDataIdNull() ) {
			existingKeyDataIdx.setOptionalDataIdNull();
		}
		else {
			existingKeyDataIdx.setOptionalDataIdValue( existing->getOptionalDataIdValue() );
		}
		cfbam::CFBamTableColByDataIdxKey newKeyDataIdx;
		if( Buff->isOptionalDataTenantIdNull() ) {
			newKeyDataIdx.setOptionalDataTenantIdNull();
		}
		else {
			newKeyDataIdx.setOptionalDataTenantIdValue( Buff->getOptionalDataTenantIdValue() );
		}
		if( Buff->isOptionalDataIdNull() ) {
			newKeyDataIdx.setOptionalDataIdNull();
		}
		else {
			newKeyDataIdx.setOptionalDataIdValue( Buff->getOptionalDataIdValue() );
		}
		...
		// Update is valid
		existing = dynamic_cast<cfbam::CFBamTableColBuff*>( schema->getTableValue()->updateValue( Authorization,
			Buff ) );
		std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>* subdict;
		auto removalDictByPKey = dictByPKey->find( pkey );
		if( removalDictByPKey != dictByPKey->end() ) {
			dictByPKey->erase( removalDictByPKey );
		}

		dictByPKey->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, existing ) );

		std::map<cfbam::CFBamValuePKey,
			 cfbam::CFBamTableColBuff*>* subdictTableIdx;
		...
		if( searchNewKeyDictByTableIdx != dictByTableIdx->end() ) {
			...
			dictByTableIdx->insert( std::map<cfbam::CFBamTableColByTableIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamTableColBuff*>*>::value_type( newKeyTableIdx, subdictTableIdx ) );
		}
		subdictTableIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, existing ) );

		std::map<cfbam::CFBamValuePKey,
			 cfbam::CFBamTableColBuff*>* subdictDataIdx;
		auto searchDictByDataIdx = dictByDataIdx->find( existingKeyDataIdx );
		if( searchDictByDataIdx != dictByDataIdx->end() ) {
			subdictDataIdx = searchDictByDataIdx->second;
			auto searchSubDict = subdictDataIdx->find( pkey );
			if( searchSubDict != subdictDataIdx->end() ) {
				subdictDataIdx->erase( searchSubDict );
			}
			if( subdictDataIdx->size() <= 0 ) {
				delete subdictDataIdx;
				dictByDataIdx->erase( searchDictByDataIdx );
			}
			subdictDataIdx = NULL;
		}
		auto searchNewKeyDictByDataIdx = dictByDataIdx->find( newKeyDataIdx );
		if( searchNewKeyDictByDataIdx != dictByDataIdx->end() ) {
			subdictDataIdx = searchNewKeyDictByDataIdx->second;
		}
		else {
			subdictDataIdx = new std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>();
			dictByDataIdx->insert( std::map<cfbam::CFBamTableColByDataIdxKey,
				std::map<cfbam::CFBamValuePKey,
					cfbam::CFBamTableColBuff*>*>::value_type( newKeyDataIdx, subdictDataIdx ) );
		}
		subdictDataIdx->insert( std::map<cfbam::CFBamValuePKey, cfbam::CFBamTableColBuff*>::value_type( pkey, existing ) );
		...
		cfbam::CFBamTableColBuff* buff;
		if( existing->getClassCode() == cfbam::CFBamTableColBuff::CLASS_CODE ) {
			delete Buff;
			Buff = NULL;
			buff = dynamic_cast<cfbam::CFBamTableColBuff*>( existing->clone() );
		}
		else {
			buff = existing;
		}
	...
	void CFBamRamTableColTable::deleteTableCol( const cfsec::CFSecAuthorization* Authorization,
		cfbam::CFBamTableColBuff* Buff )
	{
		static const std::string S_ProcName( "deleteTableCol" );
		...
		if( existing->isOptionalDataTenantIdNull() ) {
			...
			keyDataIdx.setOptionalDataTenantIdValue( existing->getOptionalDataTenantIdValue() );
		}
		if( existing->isOptionalDataIdNull() ) {
			keyDataIdx.setOptionalDataIdNull();
		}
		else {
			keyDataIdx.setOptionalDataIdValue( existing->getOptionalDataIdValue() );
		}
	...
	void CFBamRamTableColTable::deleteTableColByTableIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	{
		cfbam::CFBamTableColBuff* cur = NULL;
		cfbam::CFBamTableColBuff* rereadCur = NULL;
		...
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*> matchSet;
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamTableColBuff*>::iterator iter = dictByPKey->begin();
		std::map<cfbam::CFBamValuePKey,
			cfbam::CFBamTableColBuff*>::iterator end = dictByPKey->end();
		while( iter != end ) {
			cur = iter->second;
			iter ++;
			if( *dynamic_cast<cfbam::CFBamTableColBuff*>( cur ) == *argKey ) {
				matchSet.push_back( dynamic_cast<cfbam::CFBamTableColBuff*>( cur->clone() ) );
			}
		}
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator iterMatch = matchSet.begin();
		std::TCFLibOwningVector<cfbam::CFBamTableColBuff*>::iterator matchEnd = matchSet.end();
	...
	void CFBamRamTableColTable::deleteTableColByDataIdx( const cfsec::CFSecAuthorization* Authorization,
			...
			const int64_t* argDataId )
	{
		...
		if( argDataTenantId == NULL ) {
			...
			key.setOptionalDataTenantIdValue( *argDataTenantId );
		}
		if( argDataId == NULL ) {
			key.setOptionalDataIdNull();
		}
		else {
			key.setOptionalDataIdValue( *argDataId );
		}
		...
		const cfbam::CFBamTableColByDataIdxKey* argKey )
	{
		...
		cfbam::CFBamTableColBuff* rereadCur = NULL;
		bool anyNotNull = false;
		if( argKey->isOptionalDataTenantIdNull() ) {
			anyNotNull = true;
		}
		if( argKey->isOptionalDataIdNull() ) {
			anyNotNull = true;
		}
		if( ! anyNotNull ) {
			return;
	...
	void CFBamRamTableColTable::deleteTableColByIdIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argId )
	{
		cfbam::CFBamValuePKey key;
	...
	void CFBamRamTableColTable::deleteTableColByUNameIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
	...
	void CFBamRamTableColTable::deleteTableColByValTentIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId )
	{
	...
	void CFBamRamTableColTable::deleteTableColByScopeIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId )
	{
	...
	void CFBamRamTableColTable::deleteTableColByDefSchemaIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamTableColTable::deleteTableColByPrevIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamTableColTable::deleteTableColByNextIdx( const cfsec::CFSecAuthorization* Authorization,
	...
	void CFBamRamTableColTable::deleteTableColByContPrevIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId,
			const int64_t* argPrevId )
	{
	...
	void CFBamRamTableColTable::deleteTableColByContNextIdx( const cfsec::CFSecAuthorization* Authorization,
			const int64_t argTenantId,
			const int64_t argScopeId,
			const int64_t* argNextId )
	{
	...
	void CFBamRamTableColTable::releasePreparedStatements() {
	}
