...
namespace cfbam {
	...
	protected:
		...
		std::map<cfbam::CFBamRamTenantId32Gen, cfbam::CFBamRamTenantId32Gen*>* id32Generator;
		std::map<cfbam::CFBamRamTenantId64Gen, cfbam::CFBamRamTenantId64Gen*>* id64Generator;
	...
	public:
		...
		CFBamRamTenantTable( cfbam::ICFBamSchema* argSchema );

		virtual ~CFBamRamTenantTable();
		...
		virtual int64_t nextChainIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextChainIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextEnumTagIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextEnumTagIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextIndexColIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextIndexColIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextParamIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextParamIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextRelationColIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextRelationColIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextScopeIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );

		virtual int64_t nextScopeIdGen( const cfsec::CFSecAuthorization* Authorization,
			int64_t argId );

		virtual int64_t nextValueIdGen( const cfsec::CFSecAuthorization* Authorization,
			const cfsec::CFSecTenantPKey* pkey );
