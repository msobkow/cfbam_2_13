...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f libcfbamsaxloader*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
pushd libcfbamsaxloader
	if [ -f Makefile ]; then
		make distclean
	fi
popd
pushd libcfbamsaxloader
	...
	if [ ! -d /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamsaxloader ]; then
		sudo mkdir /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamsaxloader
	fi
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamsaxloader
	autoreconf -fi
	aclocal
	autoconf
	automake
	...
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}
	sudo chmod 755 /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamsaxloader
	sudo sh -c "cd /usr/include/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfbamsaxloader && chmod 644 *.hpp"
	#sudo chmod 755 /usr/bin/*cfbam*saxloader
	sudo chmod 755 /usr/lib/*cfbamsaxloader.la
	sudo chmod 755 /usr/lib/*cfbamsaxloader.so*
	...
	sudo chmod 644 /opt/msscf/${MAJOR_VERSION}.0.${MINOR_VERSION}/xsd/*cfbam*.xsd
	sudo ldconfig
popd
